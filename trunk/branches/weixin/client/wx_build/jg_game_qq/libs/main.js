...
window.SDKVersion = qq.getSystemInfoSync().SDKVersion;
console.log("QQ基础库版本："+window.SDKVersion);
...
window.loadMain1 = function() {
  console.log("Main1 分包加载");
  var loadMainTask1 = qq.loadSubpackage({
    name: 'main1',
    success: function(res) {
      console.log("Main1 分包加载成功");
      console.log(res);
      // if (res && res.errMsg == "loadSubpackage:ok") {
         window.loadMain2(); 
  ...
  loadMainTask1 && loadMainTask1.onProgressUpdate(res => {
    // console.log('Main 下载进度:' + res.progress + '%, 已经下载的数据长度', res.totalBytesWritten, '预期需要下载的数据总长度', res.totalBytesExpectedToWrite);
  });
}
window.loadMain2 = function() {
  console.log("Main2 分包加载");
  var loadMainTask2 = qq.loadSubpackage({
    name: 'main2',
    success: function(res) {
      console.log("Main2 分包加载成功");
      console.log(res);
      // if (res && res.errMsg == "loadSubpackage:ok") {
  ...
  loadMainTask2 && loadMainTask2.onProgressUpdate(res => {
    // console.log('Main 下载进度:' + res.progress + '%, 已经下载的数据长度', res.totalBytesWritten, '预期需要下载的数据总长度', res.totalBytesExpectedToWrite);
  });
}

window.loadSubpackages = function() {
  if (window.compareVersion(window.SDKVersion, '0.0.1') >= 0) {   //分包wx.loadSubpackage：0.0.1，SDk的wx.createUserInfoButton：2.0.1
    console.log("QQ基础库版本符合最低版本要求："+window.SDKVersion+">=0.0.1");
    // wxShowLoading({ title: '正在加载游戏' });
    window.sdkInit();
    window.loadProbuf(); 
    window.loadMain1();
  } else {
    window.reqRecordInfo("QQ基础库版本过低:" + window.SDKVersion);
    qq.showModal({
      title: '提示',
...
qq.getSystemInfo({
  success (res) {
    window.systemInfo = "品牌："+res.brand+"，型号："+res.model+"，QQ版本号："+res.version+"，系统及版本："+res.system+"，客户端平台："+res.platform+"，基础库版本："+res.SDKVersion+"，设备性能等级："+res.benchmarkLevel;
