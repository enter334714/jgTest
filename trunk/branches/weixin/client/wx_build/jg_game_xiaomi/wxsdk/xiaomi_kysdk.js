...
function mainSDK() {
    ...
    return {
        ...
        userInfo:function(callback){
            callbacks['userInfo'] = typeof callback == 'function' ? callback : null;
            console.log("[SDK]获取用户信息开始");
            qg.getUserInfo({
                success: function(res) {
                    ...
                    callbacks['userInfo'] && callbacks['userInfo'](0, res.userInfo);
                },
                fail: function(){
                    qg.setStorageSync({
                        key: '9130_plat_username',
        ...
        login: function (data, callback) {
            ...
            if (qg.getSystemInfoSync().platformVersionCode >= 1063) {
                callbacks['login'] && callbacks['login'](1, {errMsg: "吊起是吧"});
                qg.login().then((res) => {
                    console.log("[SDK]登录回调", res);
                    if (res.data.token) {
                        partner_data = {
                            session : res.data.session,
                            appAccountId: res.data.appAccountId,
                        };
                        //发起网络请求
                        ...
                        qg.request({
                            ...
                            data: public_data,
                            success: function (res) {
                                console.log("[SDK]auth验证结果：", res);
                                if(res.statusCode === 200){
                                    var data = res.data;
                                    if(data.state){
                                        var userData = {
                                            appAccountId: data.data.partner_uid,
                                            session: partner_data.session,
                                            adult: data.data.adult,
                                        };
                                        try {
                                            qg.setStorageSync({
                                                key: '9130_plat_sdk_token',
                                                value: partner_data.session
                                            });
                                            qg.setStorageSync({
                                                key: '9130_plat_uid',
                                                value: data.data.partner_uid
                                            });
                                            if(data.data.ext){
                                                qg.setStorageSync({
                    ...
                    callbacks['login'] && callbacks['login'](1, {errMsg: 'SDK登录失败'});
                });
            }else{
                callbacks['login'] && callbacks['login'](1, {errMsg: 'SDK登录失败'});
            }
        ...
        startPay: function(data, callback){
            ...
            //先下单
            this_pay_order = 0;
            var session = qg.getStorageSync({
                key: '9130_plat_sdk_token'
            });
            console.log(session,1010)
            var appAccountId = qg.getStorageSync({
                key: '9130_plat_uid'
            });
            if(!session && ! !appAccountId){
