...
function mainSDK() {
    ...
    return {
        ...
        init: function (ops, callback) {
            ...
            //console.log("广告回传参数");
            //console.log(info.query.gdt_vid);
            //console.log(info.query.weixinadinfo);
            ...
            if(game_ver){
                this.checkGameVersion(game_ver, function (data) {
                    config.is_auth = data.is_auth;
                    config.btn_bg = data.auth_bg_image;
                    config.btn_img = data.auth_button_image;
                    callback && callback(data);   //init的参数callback回调函数
                });
            }
                ...
                wx.getSetting({
                    success: function (res) {
                        if(res.authSetting['scope.userInfo']){
                            ...
                            // image2.src = 'images/btn2.gif';
                            image2.src = config.btn_img;
                            image2.onload = function() {
                                btn_width = screen_width
                                btn_height = screen_height * .15
                                // btn_width = image2.width
                                // btn_height = image2.height
                                var btn_left = (screen_width - btn_width) / 2;
                                var btn_top = screen_height / 1.35;
                                var button = wx.createUserInfoButton({
                                    type: 'image',
                                    // image: 'https://pic.9187.cn/uploads/2006/171139132529.png',
                                    image: image2.src,
                                    style: {
                                        left: btn_left,
                                        top: btn_top,
                    ...
                    if (res.code) {
                        ...
                        wx.getUserInfo({
                            success: function(res6) {
                                console.log("getUserInfo:" + JSON.stringify(res6));
                                var encryptedData = res6.encryptedData;
                                var iv = res6.iv;
                                var signature = res6.signature;
                                public_data['encryptedData'] = encodeURIComponent(encryptedData);
                                public_data['iv'] = encodeURIComponent(iv);
                            ...
                            fail: function (res6) {
                                console.log("微信登录错误:"+res6.errMsg);
                            }
                        });
                    } else {
        ...
        checkGameVersion: function (game_ver, callback) {
            ...
            wx.request({
                ...
                success: function (res) {
                    ...
                    if(res.statusCode == 200){
                        var data = res.data;
                        // wx.setStorageSync('url', data.data.auth_button_image)
                        if(data.state){
                            ...
                            if(typeof wx.requestPayment == 'undefined'){
                                if(data.data.platform == 'android'){
                                    if(data.data.is_android_pay){
                                        if(data.data.is_min_pay){
                                            self.jumpPay(data.data);
                                        }else{
        ...
        jumpPay: function (data) {
            ...
            var app_id = data.min_pay_app_id;
            wx.navigateToMiniProgram({
                appId: app_id,
                path: '/pages/pay/pay?order_id='+order_id+'&money='+money,
                extraData: {
        ...
        gamePay: function (data) {
            ...
            if(data.buyQuantity <= data.balance){
                ...
                wx.requestMidasPayment({
                    ...
                    fail: function (res) {
                        console.log("[SDK]米大师支付失败");
                        console.log(res);
                        if(res.errMsg.indexOf('用户取消') !== -1){
                            callbacks['pay'] && callbacks['pay'](2, {errMsg: "用户取消支付"});
                        }else{
                            callbacks['pay'] && callbacks['pay'](1, {errMsg: "支付失败:" + res.errCode + "," + res.errMsg});
                        }
                    },
                    complete: function (res) {
            ...
            wx.showModal({
                ...
                success: function () {
                    wx.openCustomerServiceConversation({
                        ...
                        sendMessageTitle:'回复"cz"或"充值"获取订单链接',
                        sendMessagePath:'https://pic.9187.cn/uploads/2010/171354165962.jpg',
                        sendMessageImg:'https://pic.9187.cn/uploads/2010/171354165962.jpg',
                    });
                }
            });
        ...
        downloadClient: function () {
            wx.openCustomerServiceConversation({
                showMessageCard:true,
                sendMessageTitle:'绑定微端领取豪礼',
                sendMessagePath:'https://pic.9187.cn/uploads/2105/111727402151.jpg',
                sendMessageImg:'https://pic.9187.cn/uploads/2105/111727402151.jpg',
            });
        },
        adClick: function (data) {
            var ad_game_id = data.game_id;
            var ad_query = data.query;
            var log_data = {
                event: 'mingame_ad_click',
                ext1: ad_game_id,
                ext2: ad_query
            };
            this.log('event', log_data);
        }
    }
}
