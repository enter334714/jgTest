...
//class Laya
    var ___Laya = (function () {
        ...
        Laya._getUrlPath = function () {
            ...
            pathName = pathName && pathName.charAt(2) == ':' ? pathName.substring(1) : pathName;
...
//class laya.events.EventDispatcher
    var EventDispatcher = (function () {
        ...
        /**
         * EventDispatcher 对象是否为特定事件类型注册了任何侦听器。
         *@param type 事件的类型。
         *@return 如果指定类型的侦听器
         */
        __proto.getListener = function (type) {
            return this._events ? this._events[type] : null;
        }
            ...
            if (!list.lock) {
                ...
                while (list.cursor) {
                    ...
                    if (!node.recover) {
                                ...
                                if (!node.args) {
                                    if (data instanceof Array) {
                                        ...
                                        if (node.caller)
                                            ...
                                            Laya.recordError("事件派发错误", "node.caller:" + node.caller + "node.method:" + node.method + "data:" + data + "node.prev" + node.prev);
                                    }
                                } else {
                        ...
                        console.error("node is recover："+node.id);
                    }
                }
        ...
        EventDispatcher.createHandler = function (caller, method, args, once) {
            var result = EventDispatcher._freeHandler.length > 500 ? EventDispatcher._freeHandler.shift() : null;
            if (result) {
                if (result.recover) {
                    ...
                    var funcStack= "";
                    try { var func = arguments.propertyIsEnumerable.caller; }
                    catch (error) { funcStack = error.stack; }
                    var caller_name = (result.caller ? (result.caller.__proto__&&result.caller.__proto__.constructor&&result.caller.__proto__.constructor.name)+"|"+result.caller.__className+"|"+result.caller.name : "");
                    var method_name = (result.method ? result.method.name : "");
                    Laya.recordError("EventDispatcher重复创建", "caller="+caller_name+"  method="+method_name+"  stack="+funcStack);

                    EventDispatcher._nodeCount++;
                    result = { id: EventDispatcher._nodeCount, caller: caller, method: method, args: args, once: once, recover: false };
                }
            } else {
                EventDispatcher._nodeCount++;
                result = { id: EventDispatcher._nodeCount, caller: caller, method: method, args: args, once: once, recover: false };
        ...
        EventDispatcher.recoverHandler = function (handler) {
            if (!handler.recover) {
                handler.recover = true;
                handler.caller = handler.method = handler.args = handler.prev =  handler.next = null;               
                // Laya.timer.frameOnce(1, this, function () {     
                    if(EventDispatcher._freeHandler && EventDispatcher._freeHandler.length < 3000){
                        EventDispatcher._freeHandler.push(handler);
                    }                
                // });
                // var funcStack= "";
                // try { var func = arguments.propertyIsEnumerable.caller; }
                // catch (error) { funcStack = error.stack; }
                // var caller_name = (handler.caller ? (handler.caller.__proto__&&handler.caller.__proto__.constructor&&handler.caller.__proto__.constructor.name)+"|"+handler.caller.__className+"|"+handler.caller.name : "");
                // var method_name = (handler.method ? handler.method.name : "");
                // handler.msg += "\n|\nmsg="+msg+"|\ntime: "+Date.now()+", once: "+handler.once+", recover: "+handler.recover+", GID："+this.$_GID+", name："+this.name+", node.id="+handler.id+"  node.caller="+caller_name+"  node.method="+method_name+"  stack="+funcStack;

            } else {
                if (EventDispatcher._errorCount < 5) {
                    EventDispatcher._errorCount++;
                    ...
                    var method_name = (handler.method ? handler.method.name : "");
                    // handler.msg += "\n|\nmsg="+msg+"|\ntime: "+Date.now()+", once: "+handler.once+", recover: "+handler.recover+", GID："+this.$_GID+", name："+this.name+", node.id="+handler.id+"  node.caller="+caller_name+"  node.method="+method_name+"  stack="+funcStack;
                    Laya.recordError("EventDispatcher重复回收", "id="+handler.id+"  caller="+caller_name+"  method="+method_name+"  stack="+funcStack);
                }
                handler.caller = handler.method = handler.args = handler.prev = null;
            }
        }
        EventDispatcher._freeHandler = [];
        EventDispatcher._nodeCount = 0;
        EventDispatcher._errorCount = 0;
...
//class laya.events.MouseManager
    var MouseManager = (function () {
        function MouseManager() {
            ...
            /**鼠标事件的时间。单位为毫秒。*/
            this.mouseEventTime = Browser.now();
            /**鼠标移动精度。*/
        ...
        __proto.runEvent = function () {
            ...
            var _this = this;
            _this.mouseEventTime = Browser.now();
    ...
    var SoundManager = (function () {
        ...
        SoundManager._stageOnFocus = function () {
            ...
            if (SoundManager._blurPaused) {
                var _musicChannel = SoundManager._musicChannel;
                if (_musicChannel && SoundManager._music && _musicChannel.isStopped) {
                    SoundManager._blurPaused = false;
                    if(SoundManager._music._disposed){ //已销毁
                        SoundManager.playMusic(_musicChannel.url, _musicChannel.loops, _musicChannel.startHandler, _musicChannel.completeHandler, _musicChannel.startTime);
                    } else if(!_musicChannel.audioBuffer){ //未加载
                        SoundManager._music.play(_musicChannel.startTime, _musicChannel.loops, _musicChannel.startHandler, _musicChannel);
                    } else {
        ...
        SoundManager.playSound = function (url, loops, start, complete, soundClass, startTime) {
            ...
            ;
            var channel;
            channel = tSound.play(startTime, loops, start);
            if (!channel) return null;
            if (url == SoundManager._tMusic) {
                SoundManager._music = tSound;
        ...
        SoundManager.playMusic = function (url, loops, start, complete, startTime) {
            (loops === void 0) && (loops = 0);
            (startTime === void 0) && (startTime = 0);
            SoundManager._tMusic = url;
            if (SoundManager._musicChannel) SoundManager._musicChannel.stop();
            return SoundManager._musicChannel = SoundManager.playSound(url, loops, start, complete, /*SoundManager._musicClass*/null, startTime);
        }
    ...
    var ResourceManager = (function () {
        ...
        __proto.addResource = function (resource) {
            ...
            if (index === -1) {
                ...
                this.addRecord(resource.uri, resource.__className, resource.memorySize);
         ...
         *@param size 资源内存大小。
         */
        __proto.addRecord = function (uri, type, size) {
            if(DEBUG && size && !uri)
                console.error(uri, type, size);
            if(uri) {
                if(!this._records.hasOwnProperty(type+"|"+uri)) this._records[type+"|"+uri] = 0;
                this._records[type+"|"+uri] += size;
                if(DEBUG && this._records[type+"|"+uri] < 0)
                    console.error(uri, type, this._records[type+"|"+uri]);
            }
        }
...
//class laya.utils.Timer
    var Timer = (function () {
        ...
        __proto._update = function () {
            ...
            if(Laya.isDebug && Math.abs(now - this._lastTimer) > 60*1000){ //大于1分钟
                console.error("系统时间错误！"+(now - this._lastTimer));
            }
            this._lastTimer = now;
            ...
            for (var i = 0, n = laters.length - 1; i <= n; i++) {
                ...
                if (!this._isIgnoreRecover(handler)) {
                    if (handler.method !== null) {
        ...
        /**@private */
        __proto._create = function (useFrame, repeat, delay, caller, method, args, coverBefore, ignoreRecover) {
            if (!delay) {
            ...
            if (coverBefore) {
                ...
                if (handler) {
                    ...
                    handler.delay = delay;
                    handler.ignoreRecover = ignoreRecover;
        ...
        __proto.once = function (delay, caller, method, args, coverBefore) {
            ...
            this._create(false, false, delay, caller, method, args, coverBefore, false);
        }
         ...
         *@param ignoreRecover 是否忽略销毁，针对核心定时器，默认为 false 。
         */
        __proto.loop = function (delay, caller, method, args, coverBefore, jumpFrame, ignoreRecover) {
            (coverBefore === void 0) && (coverBefore = true);
            (jumpFrame === void 0) && (jumpFrame = false);
            (ignoreRecover === void 0) && (ignoreRecover = false);
            var handler = this._create(false, true, delay, caller, method, args, coverBefore, ignoreRecover);
        ...
        __proto.frameOnce = function (delay, caller, method, args, coverBefore) {
            (coverBefore === void 0) && (coverBefore = true);
            this._create(true, false, delay, caller, method, args, coverBefore, false);
        }
         ...
         *@param ignoreRecover 是否忽略销毁，针对核心定时器，默认为 false 。
         */
        __proto.frameLoop = function (delay, caller, method, args, coverBefore, ignoreRecover) {
            (coverBefore === void 0) && (coverBefore = true);
            (ignoreRecover === void 0) && (ignoreRecover = false);
            this._create(true, true, delay, caller, method, args, coverBefore, ignoreRecover);
        }
        ...
        /**@private */
        __proto._isIgnoreRecover = function (handler) {
            if (handler.ignoreRecover || handler.caller && handler.caller.callername == "IgnoreRecover") {
                if(!this._errorRecord) {
                    this._errorRecord = true;
                    var recoverStack= "";
                    try { var func = arguments.propertyIsEnumerable.caller; }
                    catch (error) { recoverStack = error.stack; }
                    Laya.recordError("报错：核心定时器销毁忽略  "+ recoverStack);
                } else {
                    console.warn("核心定时器销毁忽略");
                }
                return true;
            } else {
                return false;
         ...
         *@param method 定时器回调函数。
         */
        __proto.hasHandler = function (caller, method) {
            return this._map[caller.$_GID +"_"+ method.$_TID];
        }
...
//class laya.utils.Utils
    var Utils = (function () {
        ...
        Utils.contextMeasureText = function (txt) {
            var textMetrics = Browser.context.measureText(txt);
            if (!textMetrics) textMetrics = {width:16, height:16};
            return textMetrics;
        }
        ...
        __proto._displayChild = function (node, display) {
            ...
            if (childs) {
                for (var i = 0; i < childs.length; i++) {
                    var child = childs[i];
                    if (!child || !child._getBit(0x1)) continue;
...
//class laya.media.h5audio.AudioSound extends laya.events.EventDispatcher
    var AudioSound = (function (_super) {
         ...
         *
         */
        __proto.play = function (startTime, loops, startHandler) {
            (startTime === void 0) && (startTime = 0);
            (loops === void 0) && (loops = 0);
            ...
            channel.startHandler = startHandler;
            channel.play(false);
...
//class laya.media.webaudio.WebAudioSound extends laya.events.EventDispatcher
    var WebAudioSound = (function (_super) {
        ...
        __proto._err = function () {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (tParams[3] && !(tParams[3]).isStopped) {
                    (tParams[3]).event(/*laya.events.Event.ERROR*/"error");
                }
            }
         ...
         *
         */
        __proto.play = function (startTime, loops, startHandler, channel) {
            (startTime === void 0) && (startTime = 0);
            (loops === void 0) && (loops = 0);
            ...
            if (!this.audioBuffer) {
                if (this.url) {
                    ...
                    this.__toPlays.push([startTime, loops, startHandler, channel]);
        ...
        WebAudioSound.decode = function () {
            ...
            if (WebAudioSound.tInfo && WebAudioSound.tInfo["buffer"] && WebAudioSound.tInfo["buffer"].byteLength) {
                try {
                    WebAudioSound.ctx.decodeAudioData(WebAudioSound.tInfo["buffer"], WebAudioSound._done, WebAudioSound._fail);
                } catch (error) {
                    console.error("decodeAudioData error  byteLength:"+WebAudioSound.tInfo["buffer"].byteLength+"  url:"+WebAudioSound.tInfo.url);
                }
            } else {
                WebAudioSound._fail();
            }
        ...
        WebAudioSound._done = function (audioBuffer) {
            ...
            WebAudioSound._dataCache[WebAudioSound.tInfo.url] = audioBuffer;
        ...
        WebAudioSound._unlock = function () {
            ...
            } else if (WebAudioSound.ctx.state == "suspended") {
                SoundManager._recoverWebAudio();
            }
        }
...
//class laya.net.Loader extends laya.events.EventDispatcher
    var Loader = (function (_super) {
        ...
        __proto.load = function (url, type, cache, group, ignoreCache) {
            if(!url || url=="") console.error("Loader 加载的url不合法 "+url);
            ...
            if (Loader.preLoadedMap[url]) {
                ...
                window.resMgrLoad(url, this._onLoadedCallback, (this.priority>=0 ? this.priority : 2), this.realTime);
            }
        }
        ...
        __proto._loadImage = function (url) {
            ...
            window.imgMgrLoad(url, this._onImageCallback, (this.priority>=0 ? this.priority : 2), this.realTime);
        }
        ...
        __proto._loadImageComplete = function (url, handle, res) {
            ...
            if (res instanceof ArrayBuffer) {
                ...
                this.onLoaded(image);
            } else if (res instanceof Texture) { //外部加载N_CustomWebGLImage
                Loader.imgMgrSet[url] = handle;
                window.imgMgrLock(url, Loader.imgMgrSet[url]);
                this._url = url;
                this.complete(res);
            } else { //img标签或者微信Image
                var format = (url.indexOf(".png", url.length-4) != -1 ? 0x1908 : 0x1907);
                var image = new laya.webgl.resource.WebGLImage(res, url, format, false, res.width, res.height);
                this.onLoaded(image);
            }
        }

        __proto.imageCancel = function (url, handle, res) {
            window.imgMgrCancel(url, this._onImageCallback);
        }
        ...
        Loader.clearCache = function (obj, url) {
            ...
            if (last == 0) {
                ...
                if (Loader.imgMgrSet[url] > 0) { //外部加载的资源，由外部发起销毁
                    window.imgMgrFree(url, Loader.imgMgrSet[url]);
                } else {
...
//class laya.net.LoaderManager extends laya.events.EventDispatcher
    var LoaderManager = (function (_super) {
        ...
        __proto.create = function (url, complete, progress, clas, params, priority, cache, group) {
            ...
            if ((url instanceof Array)) {
                ...
                for (i = 0; i < itemCount; i++) {
                    ...
                    this._create(item.url, completeHandler, progressHandler, item.clas || clas, item.params || params, (item.priority!=null&&item.priority>=0 ? item.priority : priority), cache, item.group || group);
                }
        ...
        __proto._create = function (url, complete, progress, clas, params, priority, cache, group) {
            if(!url || url=="") console.error("加载的url不合法 "+url);
        ...
        __proto.load = function (url, complete, progress, type, priority, cache, group, ignoreCache) {
            ...
            if (content != null) {
                ...
                if (!info) {
                    ...
                    priority = priority < this._maxPriority ? (priority >= 0 ? priority : 0) : this._maxPriority - 1;
        ...
        /**
         *<p>取消加载资源。资源加载错误时，本对象会派发 Event.ERROR 事件，事件回调参数值为加载出错的资源地址。</p>
         *@param url 要加载的单个资源地址或资源信息数组。比如：简单数组：["a.png","b.png"]；复杂数组[{url:"a.png",type:Loader.IMAGE,size:100,priority:1},{url:"b.json",type:Loader.JSON,size:50,priority:1}]。
         *@param complete 加载结束回调。根据url类型不同分为2种情况：1. url为String类型，也就是单个资源地址，如果加载成功，则回调参数值为加载完成的资源，否则为null；2. url为数组类型，指定了一组要加载的资源，如果全部加载成功，则回调参数值为true，否则为false。
         *@param progress 加载进度回调。回调参数值为当前资源的加载进度信息(0-1)。
         */
        __proto.cancel = function (url, complete, progress) {
            var info = LoaderManager._resMap[url];
            if (info) {
                complete && info.off(/*laya.events.Event.COMPLETE*/"complete", complete.caller, complete.method, false);
                progress && info.off(/*laya.events.Event.PROGRESS*/"progress", progress.caller, progress.method, false);
            //    console.info("cancel  url="+url+"  loader="+info.loader+"  complete="+info.hasListener("complete")+"  progress="+info.hasListener("progress"));
                var infocomplete = info.getListener("complete"), infoprogress = info.getListener("progress");
                if ((!infocomplete || !infocomplete.head) && (!infoprogress || !infoprogress.head)) {
                    //正在加载
                    if (info.loader) { 
                        var loader = info.loader;
                        loader.imageCancel(url);
                        loader.offAll();
                        loader._data = null;
                        ...
                        this._loaderCount--;
                        delete this._loaderMap[info.url];
                    }
                    //在加载列表
        ...
        __proto._doLoad = function (resInfo) {
            ...
            this._loaderCount++;
            this._loaderMap[resInfo.url] = loader;
            ...
            resInfo.loader = loader;
            // console.info("准备加载  url="+resInfo.url+"  resInfo.loader="+resInfo.loader+"  loader="+loader);
            ...
            function onLoaded(data) {
                ...
                _this._loaderCount--;
                delete _this._loaderMap[resInfo.url];
            ...
            if (resInfo.priority >= 1) {
                loader.priority = resInfo.priority;
        ...
        __proto._loadAssets = function (arr, complete, progress, type, priority, cache, group) {
            ...
            for (var i = 0; i < itemCount; i++) {
                var item = arr[i];
                if (!item) throw new Error("LoaderManager._loadAssets 加载列表错误 arr="+arr);
                ...
                this.load(item.url, completeHandler, progressHandler, item.type, (item.priority!=null&&item.priority>=0 ? item.priority : 2), cache, item.group || group);
            }
...
//class laya.resource.Resource extends laya.events.EventDispatcher
    var Resource = (function (_super) {
        ...
        __getset(0, __proto, 'memorySize', function () {
            ...
            this.resourceManager && this.resourceManager.addRecord(this.uri, this.__className, offsetValue);
    ...
    var Texture = (function (_super) {
        ...
        __proto.destroy = function (forceDispose) {
            ...
            if (this.alphaTexture) {
                this.alphaTexture.destroy(forceDispose);
        ...
        __proto.onAsynLoaded = function (url, bitmap) {
            if (bitmap) {
                if (typeof bitmap._addReference !== "function") {
                    throw new Error("bitmap._addReference is not a function. " + (bitmap.className) + " " + (typeof bitmap._addReference) + " " + bitmap.constructor);
                }
                bitmap._addReference();
            }
        ...
        Texture.createFromTexture = function (texture, x, y, width, height) {
            ...
            if (result)
                ...
                return null;
            if(!tex.bitmap || !tex.bitmap._removeReference){
                var bitmap_name = (tex.bitmap ? (tex.bitmap.constructor ? tex.bitmap.constructor.name : tex.bitmap.__className) : "");
                var url_name = texture.url +" | "+ (tex.bitmap ? tex.bitmap.url : texture.url);
                var reason = "bitmap:"+bitmap_name+", url:"+url_name+", x="+x+",y="+y+",width="+width+",height="+height+", result="+(result ? result.x+","+result.y+","+result.width+","+result.height : "null")+", rect="+rect.x+","+rect.y+","+rect.width+","+rect.height;
                // console.error("报错：bitmap._removeReference is not a function  "+ reason);
                throw new Error("报错：bitmap._removeReference is not a function  "+ reason);
            }
...
//class laya.media.h5audio.AudioSoundChannel extends laya.media.SoundChannel
    var AudioSoundChannel = (function (_super) {
        ...
        __proto.play = function (loop) {
            ...
            Browser.container.appendChild(this._audio);
            if (!loop && this.startHandler) {
                this.startHandler.runWith(this);
            }
    ...
    var Text = (function (_super) {
        ...
        __proto.parseLines = function (text) {
            ...
            if (this._currBitmapFont) {
                ...
                var measureResult = Utils.contextMeasureText(Text._testWord);
                if (Render.isConchApp && measureResult.width === 0 && measureResult.height === 0) {
                    measureResult = Utils.contextMeasureText('W');
                }
        ...
        __proto.getTextWidth = function (text) {
            if (this._currBitmapFont)
                ...
                return Utils.contextMeasureText(text).width;
        }
        ...
        Text.supportFont = function (font) {
            ...
            var defaultFontWidth = Utils.contextMeasureText("abcji").width;
            Browser.context.font = "10px " + font;
            var customFontWidth = Utils.contextMeasureText("abcji").width;
    ...
    var Stage = (function (_super) {
        ...
        __proto.render = function (context, x, y) {
            ...
            if (this.renderingEnabled && (isFastMode || !isDoubleLoop || Render.isConchWebGL)) {
                if (Render.isWebGL) {
                    if (Render.WebGL.mainContext && !Render.WebGL.mainContext.isContextLost()) {
                        context.clear();
...
//class laya.media.SoundNode extends laya.display.Sprite
    var SoundNode = (function (_super) {
        ...
        __proto.play = function (loops, complete) {
            ...
            if (!this.url) return;
            this.stop();
            this._channel = SoundManager.playSound(this.url, loops, null, complete);
        }
...
//class laya.display.Input extends laya.display.Text
    var Input = (function (_super) {
        ...
        __proto.select = function () {
            if(this.nativeInput && this.nativeInput.select) this.nativeInput.select();
        }
