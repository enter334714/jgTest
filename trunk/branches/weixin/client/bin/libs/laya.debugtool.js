...
var DebugPanel=(function(){
	...
	__proto.adptPos=function(){
		...
		var x=0, y=Browser.clientHeight-this.height, wh=Browser.clientWidth, th=this.height;
		if(this.layout){
			if(this.layout.top >= 0){
				y = this.layout.top;
				if(this.layout.bottom >= 0){
					th = Browser.clientHeight - this.layout.top - this.layout.bottom;
				}
			}
			else if(this.layout.bottom >= 0){
				y = Browser.clientHeight - this.height - this.layout.bottom;
			}

			if(this.layout.left >= 0){
				x = this.layout.left;
				if(this.layout.right >= 0){
					wh = Browser.clientWidth - this.layout.left - this.layout.right;
				}
			}
			else if(this.layout.right >= 0){
				wh = Browser.clientWidth - this.layout.right;
			}
		}
		JSTools.setPos(this.div, x, y);
		this.debug_view.resize(wh, th);
	...
	DebugPanel.init=function(underGame,bgColor,layout){
	...
	*/
	...
	DebugTool.showDisBound=function(sprite,clearPre,color){
		...
		if (clearPre)
			DebugTool.clearDebugLayer();
		var pointList;
		pointList=sprite._getBoundPointsM&&sprite._getBoundPointsM(true);
...
var NodeUtils=(function(){
	...
	NodeUtils.getGRec=function(node){
		var pointList;
		pointList=node._getBoundPointsM&&node._getBoundPointsM(true);
		if (!pointList || pointList.length < 1)
