(function (window, document, Laya) {
    ...
    var Handler = laya.utils.Handler, Loader = laya.net.Loader, MathUtil = laya.maths.MathUtil,
        Matrix = laya.maths.Matrix;
    var Node = laya.display.Node, Point = laya.maths.Point, Rectangle = laya.maths.Rectangle,
    ...
    var AnimationParser01 = (function () {
        ...
        AnimationParser01.parse = function (templet, reader) {
            ...
            for (i = 0; i < aniCount; i++) {
                ...
                if(ani.playTime <= 0){
                    console.error("播放时间不能为0  url="+templet.url+" name="+ani.name);
                }
...
//class laya.ani.bone.SkinSlotDisplayData
    var SkinSlotDisplayData = (function () {
        ...
        __proto.createTexture = function (currTexture) {
            ...
            if (currTexture.bitmap.alphaTexture) {
                this.texture.alphaTexture = new Texture(currTexture.bitmap.alphaTexture, this.uvs);
            }
...
//class laya.ani.bone.TfConstraint
    var TfConstraint = (function () {
        ...
        __proto.apply = function () {
            var tTfBone;
            var ta = this.target.resultMatrix.a, tb = this.target.resultMatrix.b, tc = this.target.resultMatrix.c,
                td = this.target.resultMatrix.d;
            for (var j = 0, n = this._bones.length; j < n; j++) {
                tTfBone = this._bones[j];
                if (this.rotateMix > 0) {
                    var a = tTfBone.resultMatrix.a, b = tTfBone.resultMatrix.b, c = tTfBone.resultMatrix.c,
...
//class laya.ani.bone.Skeleton extends laya.display.Sprite
    var Skeleton = (function (_super) {
        ...
        __proto._parseFail = function () {
            console.error("[Error]:" + this._aniPath + "解析失败");
        }
    ...
    var Templet = (function (_super) {
        ...
        __proto.parse = function (data) {
            ...
            if (this._aniVersion != Templet.LAYA_ANIMATION_VISION) {
                console.error("[Error] 版本不一致，请使用IDE版本配套的重新导出" + this._aniVersion + "->" + Templet.LAYA_ANIMATION_VISION);
                this._loaded = false;
            }
            if (this.loaded) {
                if (this._mainTexture) {
        ...
        __proto._parseTexturePath = function () {
            ...
            this.event(/*laya.events.Event.INITED*/"inited", this);
        ...
        __proto._parsePublicExtData = function () {
            ...
            for (i = 0; i < tTextureLen; i++) {
                ...
                if (tTexture.alphaTexture) {
                    this.subTextureDic[tTextureName].alphaTexture = Texture.create(tTexture.alphaTexture, tX, tY, tWidth, tHeight, -tFrameX, -tFrameY, tFrameWidth, tFrameHeight);
                }
            }
