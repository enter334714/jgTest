...
//场景信息
tt.onShow((res) => {
    // window.reqRecordInfo("onShow:"+JSON.stringify(res));
    console.log("onshowres:",res)
    var scene = res.scene
    if (scene === '021036' && res.launch_from === 'homepage' && res.location === 'sidebar_card') {
        // 可判断小游戏用户通过侧边栏入口场景启动
        console.log("侧边栏打开");
        sceneInfo['isSidebarCard'] = 1;
        sceneInfo['SidebarCardNum'] = (sceneInfo['SidebarCardNum'] || 0) + 1;
    } else {
        console.log("非侧边栏打开");
    ...
    if (scene === '023040' || scene === '103040') {
        // 可判断小游戏用户通过侧边栏入口场景启动
        console.log("推荐卡场景打开");
        sceneInfo['recommendCard']['isCome'] = 1;
        let LaunchParams = tt.getLaunchOptionsSync();
        // console.log(LaunchParams.query)
        if(LaunchParams.query.start_page){
            sceneInfo['recommendCard']['start_page'] = LaunchParams.query.start_page;
        }
...
// console.log(sysInfo)
//检测侧边栏是否可用
if (sysInfo.SDKVersion >= '2.92.0.0') {
    tt.checkScene({
        scene: "sidebar",
        success: (res) => {
            console.log("check scene success: ", res.isExist);
            sceneInfo['isExist'] = res.isExist ? 1 : 0;
        },
        fail: (res) => {
    ...
    console.log("版本不支持checkScene");
}

//检查是否已添加到手机桌面上
if ('android' == platform && sysInfo.SDKVersion >= '2.46.0') {

    tt.checkShortcut({
        success(res) {
            console.log("检查快捷方式", res.status);
            sceneInfo.hasShortcut = res.status.exist ? 1 : 0;
        },
        fail(res) {
...
exports.navigateToScene = function (callback) {
    ...
    if (sysInfo.SDKVersion >= '2.92.0.0') {
        ...
        console.log("版本不支持跳转侧边栏");
        callback && callback(0)
    }

};

exports.addShortcut = function (callback) {

    if (sysInfo.SDKVersion >= '2.46.0.0') {
        tt.addShortcut({
            success() {
                console.log("添加桌面成功");
                callback && callback(1)
