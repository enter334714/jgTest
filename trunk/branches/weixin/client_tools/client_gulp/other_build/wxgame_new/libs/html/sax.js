function XMLReader() {}function parse(e, t, r, a, n) {
  function s(e) {
    ...
    }(parseInt(t.substr(1).replace("x", "0x"))) : (n.error("entity not found:" + e), e);
  }function c(t) {
    if (t > d) {
      var r = e.substring(d, t).replace(/&#?\w+;/g, s);_ && i(d), a.characters(r, 0, t - d), d = t;
    }
  }function i(t, r) {
    for (; t >= u && (r = l.exec(e));) o = r.index, u = o + r[0].length, _.lineNumber++;_.columnNumber = t - o + 1;
  }for (var o = 0, u = 0, l = /.*(?:\r\n?|\n)|.*$/g, _ = a.locator, m = [{ currentNSMap: t }], f = {}, d = 0;;) {
    try {
      var p = e.indexOf("<", d);if (0 > p) {
        if (!e.substr(d).match(/^\s*$/)) {
          var T = a.doc,
              S = T.createTextNode(e.substr(d));T.appendChild(S), a.currentElement = S;
        }return;
      }switch (p > d && c(p), e.charAt(p + 1)) {case "/":
          var h = e.indexOf(">", p + 3),
              A = e.substring(p + 2, h),
              g = m.pop();0 > h ? (A = e.substring(p + 2).replace(/[\s<].*/, ""), n.error("end tag name: " + A + " is not complete:" + g.tagName), h = p + 1 + A.length) : A.match(/\s</) && (A = A.replace(/[\s<].*/, ""), n.error("end tag name: " + A + " maybe not complete"), h = p + 1 + A.length);var w = g.localNSMap,
              E = g.tagName == A;if (E || g.tagName && g.tagName.toLowerCase() == A.toLowerCase()) {
            if (a.endElement(g.uri, g.localName, A), w) for (var N in w) a.endPrefixMapping(N);E || n.fatalError("end tag name: " + A + " is not match the current start tagName:" + g.tagName);
          } else m.push(g);h++;break;case "?":
          _ && i(p), h = parseInstruction(e, p, a);break;case "!":
          _ && i(p), h = parseDCC(e, p, a, n);break;default:
          _ && i(p);var b = new ElementAttributes(),
              x = m[m.length - 1].currentNSMap,
              h = parseElementStartPart(e, p, b, x, s, n),
              C = b.length;if (!b.closed && fixSelfClosed(e, h, b.tagName, f) && (b.closed = !0, r.nbsp || n.warning("unclosed xml attribute")), _ && C) {
            for (var v = copyLocator(_, {}), R = 0; C > R; R++) {
              var O = b[R];i(O.offset), O.locator = copyLocator(_, {});
            }a.locator = v, appendElement(b, a, x) && m.push(b), a.locator = _;
          } else appendElement(b, a, x) && m.push(b);"http://www.w3.org/1999/xhtml" !== b.uri || b.closed ? h++ : h = parseHtmlSpecialContent(e, h, b.tagName, s, a);}
    } catch (e) {
      n.error("element parse error: " + e), h = -1;
    }h > d ? d = h : c(Math.max(p, d) + 1);
  }
  ...
  for (var c, i = ++t, o = S_TAG;;) {
    var u = e.charAt(i);switch (u) {case "=":
        ...
        if (o === S_EQ || o === S_ATTR) {
          if (o === S_ATTR && (s.warning('attribute value must after "="'), c = e.slice(t, i)), t = i + 1, !((i = e.indexOf(u, t)) > 0)) throw new Error("attribute value no end '" + u + "' match");l = e.slice(t, i).replace(/&#?\w+;/g, n), r.add(c, l, t - 1), o = S_ATTR_END;
        } else {
        ...
        switch (o) {case S_TAG:
            ...
            throw new Error("attribute value missed!!");}return i;case "":
        u = " ";default:
  ...
  for (var a = e.tagName, n = null, s = e.length; s--;) {
    var c = e[s],
        i = c.qName,
        o = c.value;if ((m = i.indexOf(":")) > 0) var u = c.prefix = i.slice(0, m),
        l = i.slice(m + 1),
        _ = "xmlns" === u && l;else l = i, u = null, _ = "xmlns" === i && "";c.localName = l, !1 !== _ && (null == n && (n = {}, _copy(r, r = {})), r[_] = n[_] = o, c.uri = "http://www.w3.org/2000/xmlns/", t.startPrefixMapping(_, o));
  }for (s = e.length; s--;) {
    (u = (c = e[s]).prefix) && ("xml" === u && (c.uri = "http://www.w3.org/XML/1998/namespace"), "xmlns" !== u && (c.uri = r[u || ""]));
  }var m = a.indexOf(":");m > 0 ? (u = e.prefix = a.slice(0, m), l = e.localName = a.slice(m + 1)) : (u = null, l = e.localName = a);var f = e.uri = r[u || ""];if (t.startElement(f, l, a, e), !e.closed) return e.currentNSMap = r, e.localNSMap = n, !0;if (t.endElement(f, l, a), n) for (u in n) t.endPrefixMapping(u);
  ...
  switch (e.charAt(t + 2)) {case "-":
      if ("-" === e.charAt(t + 3)) {
        return (n = e.indexOf("--\x3e", t + 4)) > t ? (r.comment(e, t + 4, n - t - 4), n + 3) : (a.error("Unclosed comment"), -1);
      }return -1;default:
      if ("CDATA[" == e.substr(t + 3, 6)) {
        var n = e.indexOf("]]>", t + 9);return r.startCDATA(), r.characters(e, t + 9, n - t - 9), r.endCDATA(), n + 3;
      }var s = split(e, t),
          c = s.length;if (c > 1 && /!doctype/i.test(s[0][0])) {
        var i = s[1][0],
            o = c > 3 && /^public$/i.test(s[2][0]) && s[3][0],
            u = c > 4 && s[4][0],
            l = s[c - 1];return r.startDTD(i, o && o.replace(/^(['"])(.*?)\1$/, "$2"), u && u.replace(/^(['"])(.*?)\1$/, "$2")), r.endDTD(), l.index + l[0].length;
      }}return -1;
}function parseInstruction(e, t, r) {
  var a = e.indexOf("?>", t);if (a) {
    var n = e.substring(t, a).match(/^<\?(\S*)\s*([\s\S]*?)\s*$/);return n ? (n[0].length, r.processingInstruction(n[1], n[2]), a + 2) : -1;
  }return -1;
