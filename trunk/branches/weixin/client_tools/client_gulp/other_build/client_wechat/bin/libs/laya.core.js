    ...
    var Loader = (function (_super) {
        ...
        __proto.load = function (url, type, cache, group, ignoreCache) {
            ...
            if (Loader.preLoadedMap[url]) {
				...
				if(MainResourcePool){
					MainResourcePool.load(url, this._onLoadedCallback, this.priority, this.realTime);
				}else{
					if (!this._http){
						this._http=new HttpRequest();
               ...
               // ResourcePool.instance.load(url, this._onLoadedCallback, this.priority, this.realTime);
            }
        }
        ...
        __proto._loadImage = function (url) {
			if(MainResourcePool){
				MainResourcePool.load(url, this._onImageCallback, 0, true);
			}else{
                url=URL.formatURL(url);
		        var _this=this;
		        var image;
           ...
           //ResourcePool.instance.load(url, this._onImageCallback, 0, true);
        }
...
//class laya.net.LoaderManager extends laya.events.EventDispatcher
    var LoaderManager = (function (_super) {
     ...
     /**
         ...
         *@param url 资源地址
         */
         __proto.cancelLoadByUrl = function (url, complete) {
            for (var i = 0; i < this._maxPriority; i++) {
                var infos = this._resInfos[i];
                for (var j = infos.length - 1; j > -1; j--) {
                    var info = infos[j];
                    if (info && info.url === url) {
                        if (complete) {
                            info.off("complete", complete.caller, complete.method);
                        }
                        if (!complete || !info.hasListener("complete")) {
                            infos[j] = null;
                            info.offAll();
                            this._infoPool.push(info);
                            delete LoaderManager._resMap[url];
