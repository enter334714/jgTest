...
var DEST = '../../client/wx_dist/packageD'; //路径自行设置
var DEST_JS = '../../client/wx_dist/'; //路径
var BUILD = '../../client/wx_build/'; //路径
var PACK = 'jg_gameD'; //项目名
var INIT_PATH = ''; //init.min.js的目录
var SCOPE = 'abcdefijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$_gh';
var PREFIX = '';
...
gulp.task('build-all-D', function (cb) {
    sequence('set-param-d', 'scope-check', 'main-modify', 'main-map', 'main-backup', 'init-modify', 'init-backup', 'libs-min', 'libs-map', 'libs-backup', 'build-protobuf', 'copy', cb)
});
...
var filesMap = {
    ...
    "subPackage/main.min.js":  {url:"ddddddddd/dddmain.js",extractStr:true,count:1,strLen:3},
...
gulp.task('MT1_build_minify', function () {
    ...
    var stream = gulp.src([sourceUrl + '/**/*.js', "!" + sourceUrl + '/**/dddmain.js'])
        .pipe(js_minify())
...
gulp.task('MT1_COPY', function () {
    ...
    return gulp.src(sourceUrl + "/" + '/**/*')
        ...
        //报名需要修改
        .pipe(replace(/( name: 'main')/g, "name: 'ddddddddd'"))
        .pipe(replace(/( name: 'probuf')/g, "name: 'ddddddf'"))
