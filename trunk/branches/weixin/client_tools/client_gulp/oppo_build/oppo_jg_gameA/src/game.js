...
import "./libs/laya.quickgamemini";
...
// if (window.SDKVersion >= 1052) {   //分包qg.loadSubpackage：1052,  qg.share: 1056,  qg.pay: 1056,  qg.login: 1063,  qg.getMenuButtonBoundingClientRect: 1083
  // console.log("oppo基础库版本符合分包版本要求："+window.SDKVersion+">=1060");
  window.sdkInit();
  window.loadProbuf(); 
  window.loadMain();
// } else {
...
qg.getSystemInfo({
  success (res) {
    window.systemInfo = "品牌："+res.brand+"，型号："+res.model+"，系统及版本："+res.system+"，运行平台版本名："+res.platformVersionName+"，运行平台版本号："+res.platformVersionCode;
