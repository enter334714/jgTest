...
var modify_scope = function () {
    function onFile(file, enc, cb) {
        ...
        if (mch && mch.length == 1) {
            var repl = 'var leading = init("' + SCOPE + '");';
...
var minify_main = function () {
    function onFile(file, enc, cb) {
        ...
        var global_defsParam = {DEBUG: false,isWorkers:true}
        if(define_ENV != -1){
...
//其他公司的主文件压缩
var minify_main_other = function () {
    function onFile(file, enc, cb) {
        if (file.isStream()) {
            this.emit('error', new PluginError(PLUGIN_NAME, 'Streams are not supported!'));
            return cb();
    ...
    function onFile(file, enc, cb) {
        ...
        var options = {
            ...
            mangle: {
                ...
                eval: false,
                keep_fnames: false, //true表示不混淆函数名
                // reserved: ["Main", "MainWX", "LayaGPU"],
                // properties: {
                //     regex: /(^((p|P|n|N)_.{3,100})|(.{2,100}Cfg)$)|(^[A-Z].*(Panel|Alert|Model|Mod|Ctrl|Ctl|Item|Itm|View|Mgr)$)|(^(get|set|do)_[A-Z].{3,100})$/,
                // }
