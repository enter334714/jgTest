...
var config = {
    game_id: 112,
    game_pkg: 'tjqy_tjqytwwxxcxlxzc_VA',
    partner_label: 'twxyx1',
    partner_id: '592',
    game_ver: '70.0.1',
    is_auth: false, //授权登录
};
...
function mainSDK() {
    ...
    return {
        ...
        login: function (data,callback) {
            ...
            gameSDK.init(()=>{
                console.log('渠道sdk 初始化成功');
                gameSDK.entryGame(()=>{

                  console.log('渠道sdk 进入游戏成功');
                  gameSDK.loginUserInfo((res)=>{
                        console.log('渠道sdk 登录成功',JSON.stringify(res));
                        partner_user_info = res;
                        self.do_login(res);
                  },(err)=>{

                    console.log('渠道sdk 登录失败',err);
                  })

                },(err)=>{

                  console.log('渠道sdk 进入游戏失败',err);
                })
            },(err)=>{
                console.log('渠道sdk 初始化失败',err);
            });
        },
                ...
                success: function (res) {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        self.getShareInfo('menu', function (data) {
                            ...
                            gameSDK.setMenuShare({title:data.title,imageUrl:data.img,ext:data.query+'&share_ext='+ JSON.stringify(info.share_ext)})
                        });
                    } else {
                        callbacks['login'] && callbacks['login'](1, {
                            errMsg: '请求平台服务器失败！'
        ...
        share: function (data) {
            ...
            this.getShareInfo(type, function (data) {
                ...
                gameSDK.shareAppMessage({
                    title: data.title,
                    imageUrl: data.img,
                    ext: data.query + '&share_ext=' + JSON.stringify(partner_user_info.share_ext),
                })
            });
        },
        ...
        msgCheck: function (content, callback) {
            ...
            gameSDK.msgCheck(content).then((success) => {
                console.log("msgCheck success", success)
                ret.statusCode = 200;
                ret.data.state = 1;
                callback && callback(ret);
            }).catch((err) => {
                console.log("msgCheck error", err)
                ret.statusCode = 200;
                ret.data.state = 0;
                callback && callback(ret);
            });
                      ...
                      if(data.state){
                          ...
                          if(data.data.ext == ''){
                              ...
                              gameSDK.makePayment(data.data.pay_data).then(res => {
                                    ...
                                    callbacks['pay'] && callbacks['pay'](1, {errMsg: '发起支付失败'});
                                });
                          }else{
