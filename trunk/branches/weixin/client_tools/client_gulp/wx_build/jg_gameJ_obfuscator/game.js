var S = wx.$J;
import 'jjasdfads/jjjWEASAF.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.$jI39AS = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Jres/Jfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$J = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'jjjLIBS', success: function (r) {
...
wx.onShow(function (res) {
  window.$jS3 = res;
  if (window.$jA3S && window.$jS3) {
    console.info("小游戏切前台事件，场景值：" + window.$jS3.scene);
    window.$jA3S(window.$jS3);
    window.$jS3 = null;
  }
});