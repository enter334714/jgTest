...
var config = {
    game_id: 256,
    game_pkg: 'tjqy_tjqymsdmxkphdqxwxxcx_YX',
    partner_label: 'qianxixyx',
    partner_id: '258',
    game_ver: '93.0.1',
    is_auth: false, //授权登录
    partner_game_id:350,        // GameId
    partner_game_name:'美食大冒险酷跑HD',  // GameName
    partner_channel_id:195497,    // ChannelId
    wxmini_appid:'wx8923a82ea75b7d81'    // 微信appid
};
    ...
    return {
        ...
        login: function (data,callback) {
            ...
            sdk.init(init_params,function(init_res){
                console.log('初始化回调结果'+JSON.stringify(init_res));
                sdk.login({},function (login_res) {
                    if(login_res.statusCode == 0){
                        ...
                        console.log('渠道登录返回成功'+ JSON.stringify(login_res));
                    }
                });
            })
        },
        ...
        do_login: function (info) {
            ...
            public_data['statusCode'] = info.statusCode;
            public_data['platformChanleId'] = info.statusCode;
            public_data['userName'] = info.userName;
            public_data['timestamp'] = info.timestamp;
            public_data['sign'] = info.sign;
            public_data['userId'] = info.userId;
            public_data['guid'] = info.guid;
            public_data['cp_ext'] = info.cp_ext;
            public_data['ext'] = info.ext;
        ...
        msgCheck: function (content, callback) {
            ...
            sdk.msgSecCheck({scene: 1,content:content},(res)=>{
                console.log('回调函数参数',res);

                let ret = {
                    data:{}
                };

                if(res.statusCode==0 && res.checkResult.result.suggest=='pass')
                {
                        ret.statusCode = 200;
                        ret.data.state = 1;
                }else{
