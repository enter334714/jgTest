...
function getOtherGameData(name) {
  if (!_globalData.otherData.game_id) {
    var extraData = wx.getEnterOptionsSync()
    if (extraData.referrerInfo.extraData && extraData.referrerInfo.extraData.game_id) {
      _globalData.otherData = extraData.referrerInfo.extraData
    }
  }
  if (name) {
    return _globalData.otherData[name] || ''
  }
  return _globalData.otherData;
}
...
var qxMiniSDK = {
  ...
  _dologin(xcxUserInfo, type, callback) {
    ...
    wx.login({
      success: ret => {
        ...
        let ext_header = {}
        var otherData = getOtherGameData()
        if (otherData.game_id) {
          ext_header = {
            game_id: otherData.game_id,
            game_name: otherData.game_name,
            from_id: otherData.from_id,
            user_id: otherData.user_id,
            guid: otherData.guid,
            wxmini_appid: otherData.wxmini_appid
          }
        }
        console.log('[ext_header]', ext_header);
        let dt_header = {
          code,
          userinfo
        }
        ...
        request('minigame', 'login', loginParams, 1,headers).then((result) => {
          ...
          if (result.code == 0) {
            ...
            if (result.data.relationship) {
              that.initData.old_gameid = that.initData.game_id
    ...
    request('minigame', 'make_order', payParams, 1).then((resulte) => {
      if (resulte.code == 0) {
        ...
        var paycbData = {
		  ...
		  old_gameid: that.initData.old_gameid || '',
  ...
  _getActionSheet() {
    return new Promise((resolve) => {
      var loginTipsRes = _globalData.loginTipsRes
      if (!loginTipsRes) {
        let qxyxUserInfo = getStorageSync(USER_INFO)
        let user_id = qxyxUserInfo.user_id
        let openid = qxyxUserInfo.ext.openid
        ...
        request('wxa', 'login_tips', dt, 1).then(res => {
          console.log('[res]', res);
          _globalData.loginTipsRes = res.data
          if (res.code == 0) {
            resolve(res.data)
          }
        })
      } else if (loginTipsRes.title) {
        resolve(_globalData.loginTipsRes)
      }
    })
  }
}
