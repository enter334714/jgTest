...
export default class RYSDK {
    ...
    ryCheckTurnTerminalMsg(){
        ...
        return new Promise((resolve, reject) => {
            that.ryRequest('https://weixin.iskywan.com/data/api/ApiSetXcxBehavior.php',{
                ...
                sdkver:common_param.sdkver,
                roleName:common_param.role,
                roleId:common_param.roleid,
                roleLevel:common_param.rolelevel,
                serverId:common_param.server,
                uid:common_param.uid,
            }).then(res => {
                if(res.info.status){
                    //客服
        ...
        return new Promise((resolve, reject) => {
            if(!common_param.uid){
                ...
                that.ryPopUp(1, common_param).then(ryPopUp_res => {
                    resolve({msg:'成功'})
                }).catch(ryPopUp_err => {reject({mgs:ryPopUp_err})})
    ...
    ryMidasPayment(res,data){
        ...
        wx.requestMidasPayment({
            ...
            success: function(resp) {
                ...
                that.ryPopUp(4, ryPopUp_param).then(ryPopUp_res => {
                }).catch(ryPopUp_err=>{console.log(ryPopUp_err.msg)})
            },
            fail: function(err) {
                console.log(err)
            }
        ...
        return new Promise((resolve, reject) => {
            if(!common_param.uid){
                ...
                that.ryPopUp(3, ryPopUp_param).then(ryPopUp_res => {
                    that.ryCheckTurnPackage({}).then(res => {
    ...
    ryPopUp(situation, popup_param) {
        ...
        if (situation == 2 || situation == 3 || situation == 4) { //不同情况key也不同
        ...
        data_key.forEach(function (value) {
            if (popup_param.hasOwnProperty(value) == false) { //判别参数key格式
                no_keys.push(value)
            } else { //符合条件的key放入key和value都放入data中进行post，无用则不使用
                data[value] = popup_param[value]
            }
        })
        let url = "https://weixin.iskywan.com/data/api/ApiCheckTurnPackage2.php"
        return new Promise((resolve, reject) => {
            if (no_keys.length > 0) { //当有不存在的key 错误提醒(容错)
                let err_msg = ""
                no_keys.forEach(function (key) {
                    err_msg = err_msg + key + "  "
                })
                reject({msg: "缺少参数：" + err_msg})
            } else if (localStorage.getItem("is_pop_up_" + situation + "_" + date.getFullYear() + date.getMonth() + date.getDate()) == true) { //查看有当天弹窗缓存 有就跳过请求
                resolve({msg: '今日已弹出过'})
            } else { //没问题的情况
                that.ryRequest(url, data).then(res => {
                    ...
                    if (res.info.status) { //需要弹窗
                        //除进入游戏后的所有类型-跳转方法内部同步
                        //强制模式
                        console.log("强制模式")
                        if (situation == 2 || situation == 4) { //当进入模式，充值后的情况 使用异步弹窗
                        ...
                        if(situation==2){
                            ...
                            if(ryPopUpMode_is_synchronization==false){resolve({msg: "非强制模式已经执行"})}
                        }
    ...
    /**
     * @description 复制功能
     * @param { String } copyText - 放到用户粘贴板的东西
     * @example
     */
    ryCopy(copyText){
    ...
    getGiftInfo(){
        ...
        //请求
        that.ryRequest(get_gift_url,data).then(res=>{
            if(res.info.status){
                //显示弹窗参数
                let title=res.info.info.title
                let context=res.info.info.context
    ...
    setGiftInfo(set_gift_param){
        ...
        that.ryRequest(set_gift_url,data).then(res=>{
            console.log(res)
        }).catch(err=>{
            console.log(err)
        })
