...
var image = new Image();
image.src = "wxlogin_atlas/l22b.png";
...
render();
window.$LMRUX7 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Z20es/Z20iles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.$L7R = res;
  if (window.$LXR7 && window.$L7R) {
    console.info("小游戏切前台事件，场景值：" + window.$L7R.scene);
    window.$LXR7(window.$L7R);
    window.$L7R = null;
  }
});