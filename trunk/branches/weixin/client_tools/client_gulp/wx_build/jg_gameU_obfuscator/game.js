...
render();
window.u$708G$ = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/uues/uuiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.u$80G = res;
  if (window.u$8G0 && window.u$80G) {
    console.info("小游戏切前台事件，场景值：" + window.u$80G.scene);
    window.u$8G0(window.u$80G);
    window.u$80G = null;
  }
});