...
var image = new Image();
image.src = "wxlogin_atlas/t22b.png";
...
render();
window.pT14HS = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/z17ttes/z17ttiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.u$ = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'tIBSt', success: function (r) {
...
wx.onShow(function (res) {
  window.pS1 = res;
  if (window.pH1S && window.pS1) {
    console.info("小游戏切前台事件，场景值：" + window.pS1.scene);
    window.pH1S(window.pS1);
    window.pS1 = null;
  }
});