...
class YishiSdk {
  ...
  /** 游戏内绑定手机号 **/
  bindPhoneInGame(object) {
    if (!this.systemInfo) {
      console.error("初始化错误");
      return false;
    }
    let paramsKeys = ['phone', 'sms_code'];
    if (!this.checkParams(object, paramsKeys)) {
      this.alert('参数错误');
      return false;
    }
  ...
  customerService(object) {
    ...
    let gamePackageId = this.systemInfo.gamePackageId;
    let cardUrl = loginInfo.customer_service_card_url; // 卡片图片地址
    ...
    wx.openCustomerServiceConversation({
      'sessionFrom': JSON.stringify(sessionFrom),
      'showMessageCard': true,
      'sendMessageTitle': "发送任意消息接入人工客服",
      'sendMessageImg': cardUrl,
      'success': res => {
        console.log("openCustomerServiceConversation complete :", sessionFrom);
        if (object && "function" == typeof object.success) {
          object.success(res);
        }
      },
      'fail': err => {
        if (object && "function" == typeof object.fail) {
          object.fail(err);
        }
      }
    });
  ...
  svipCare(object) {
    ...
    let appId = loginInfo.miniapp_mch_app_id;
    let url = loginInfo.svip_customer_service_url;
    let corpId = loginInfo.svip_customer_service_corp_id;

    if (!(appId && url && corpId)) {
      console.error('svip customer service params miss error');
      return false;
    }
    wx.navigateToMiniProgram({
      appId: appId,
      path: '/pages/svipService/index?url=' + url + '&corpId=' + corpId,
      envVersion: 'release', // 正式版: release , 体验版: trial , 开发版: develop
      success: res => {
        if (object && "function" == typeof object.success) {
          object.success(res);
        }
  ...
  miniAppPay(object) {
    ...
    wx.navigateToMiniProgram({
      ...
      envVersion: 'release', // 正式版: release , 体验版: trial , 开发版: develop , 测试开发版支付还要手动设置 this.version
      success: res => {
