console.info("0 \u8fdb\u5165\u6e38\u620f\u5305"), window.lastError, wx.onError(function (e) {
  if (e) {
    if (e.message) {
      var o = window.config.game_ver.replace(new RegExp(/\./, "g"), "_");var n = e.message;var i = n.match(/(eeeeeeee\/eeegame.js:)[0-9]{1,60}(:)/g);if (i) for (var w = 0; w < i.length; w++) {
        var a;i[w] && 0 < i[w].length && (a = parseInt(i[w].replace("eeeeeeee/eeegame.js:", "").replace(":", "")), n = n.replace(i[w], i[w].replace(":" + a + ":", ":" + (a - 2) + ":")));
      }n = (n = n.replace(new RegExp("eeeeeeee/eeegame.js", "g"), "eeeeeeee/main__" + o + ".min.js")).replace(new RegExp("eeeeeeee/eeemain.js", "g"), "eeeeeeee/main__" + o + ".min.js"), e.message = n;
...
});import "eeemd5min.js";import "eeezlibs.js";window.Parser = require("eeedomparser.js");import "eeeindex.js";import "eeelibsmin.js";import "eeewxmini.js";import "eeeinitmin.js";console.info("1 \u521d\u59cb\u5316"), console.info("2 \u52a0\u8f7d\u6e38\u620f"), wxShowLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });var wxData = { showLoadingBtn: !0 };new window.ServerLoading(wxData), window.ServerLoading.instance.openAuthor(), window.loadingInterval && clearInterval(window.loadingInterval), window.loadingInterval = null, window.compareVersion = function (e, o) {
  if (!e || !o) return 0;e = e.split("."), o = o.split(".");var n = Math.max(e.length, o.length);for (; e.length < n;) e.push("0");for (; o.length < n;) o.push("0");for (var i = 0; i < n; i++) {
    var w = parseInt(e[i]),
        a = parseInt(o[i]);if (a < w) return 1;if (w < a) return -1;
  }return 0;
}, window.SDKVersion = wx.getSystemInfoSync().SDKVersion, console.log("\u5fae\u4fe1\u57fa\u7840\u5e93\u7248\u672c\uff1a" + window.SDKVersion);var updateManager = wx.getUpdateManager();updateManager.onCheckForUpdate(function (e) {
  ...
  console.log("protobuf \u5206\u5305\u52a0\u8f7d");var e = wx.loadSubpackage({ name: "eeeeeepf", success: function (e) {
  ...
  console.log("Main \u5206\u5305\u52a0\u8f7d");var e = wx.loadSubpackage({ name: "eeeeeeee", success: function (e) {
