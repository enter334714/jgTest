...
wx.onError(function (error) {
  if (error) {
    if (error.message) {
      ...
      var arr = message.match(/(cccccccc\/cccgame.js:)[0-9]{1,60}(:)/g);
      if (arr) {
        for (var i = 0; i < arr.length; i++) { //行数减2
          if (arr[i] && arr[i].length > 0) {
            var line = parseInt(arr[i].replace("cccccccc/cccgame.js:", "").replace(":", ""));
            message = message.replace(arr[i], arr[i].replace(":"+line+":", ":"+(line-2)+":"))
          }
        }
      }
      message = message.replace(new RegExp("cccccccc/cccgame.js", "g"), "cccccccc/main__" + gamever + ".min.js");
      message = message.replace(new RegExp("cccccccc/ccccccccmain.js", "g"), "cccccccc/main__" + gamever + ".min.js");
...
import "cccmd5min.js";
import "ccczlibs.js";
window["Parser"] = require("cccdomparser.js");
import "cccindex.js";
import "ccclibsmin.js";
import "cccwxmini.js";
import "cccinitmin.js";
