require('../utils/qjsdk-1.0.4');
//TODO 替换对应参数
var config = {
    game_id: 256,
    game_pkg: 'tjqy_tjqyqjwxxcxhxfx_WS',
    partner_label: 'qjxyx1',
    partner_id: '605',
    game_ver: '77.0.1',
    is_auth: false, //授权登录
};
...
function mainSDK() {
    ...
    return {
        ...
        login: function (data,callback) {
            ...
            var self = this;
             //登录
             window['qjsdk'].SDKLogin().then(loginRes => {
                    partner_user_info = loginRes;
                    self.do_login(loginRes);
                    console.log('渠道登录返回成功'+ JSON.stringify(loginRes));
            }).catch(failRes => {
                console.warn('sdk初始化失败，重试。',failRes);
            })
        },
        ...
        msgCheck: function (content, callback) {
            ...
            window['qjsdk'].msgSecCheck(partner_user_info.openId,'1',content).then(res => {
                    ...
                    if(res.statusCode == 200){
                        ...
                        if(data.state){
                            ...
                            if(data.data.ext == ''){
                              ...
                              window['qjsdk'].SDKPay(data.data.pay_data);
                            }else{
        ...
        logCreateRole: function (data) {
            ...
            window['qjsdk'].UploadRoleInfo(roleInfo).then(res => {
