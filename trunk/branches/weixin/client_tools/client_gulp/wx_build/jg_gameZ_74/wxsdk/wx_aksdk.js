import sdk from '../utils/wxasdk';

//TODO 替换对应参数
var config = {
    game_id: '256',
    game_pkg: 'tjqy_tjqysjwxxcxjljs_AAT',
    partner_label: 'sijinxcx2', 
    partner_id: '480',
    game_ver: '107.0.1',
    is_auth: false, //授权登录
    partner_game_id:'113',
    partner_pid:'409',
    partner_cid:'42',
    partner_app_key: '3d904af0-b877-4782-838e-5daf03e781a1',
    partner_ogameID:'5153',
    partner_app_id:'13952',
    partner_share_adid:'2042241',
    partner_wx_appid:'wx7381579c07a7d1f9',
    partner_chid:'2039221',
    partner_key:'b253408b-94fe-4f9f-8481-60cfedc14380',
    partner_isCode:true,
};
    ...
    return {
        ...
        init: function (ops, callback) {
            ...
            sdk.init({
                ...
                cid:config.partner_cid,
                pid:config.partner_pid,
                appKey:config.partner_app_key,
                appID:config.partner_app_id,
                shareAdID:config.partner_share_adid,
                wxAppID:config.partner_app_id,
                appVersion:game_ver,
                chid:config.partner_chid,
                ogameID:config.partner_ogameID,
            ...
            sdk.login(
                function (result) {
                    ...
                    if(result.state == 1){
                        ...
                        console.log('渠道登录返回成功'+ JSON.stringify(result));
                    }else{
                        //登录失败
                        callback(1, {
                            errMsg: '渠道登录返回失败！'
                        });
                        console.log('渠道登录返回失败'+ JSON.stringify(result));
                    }
                }
            );
        },
        ...
        msgCheck: function (content, callback) {
            ...
            sdk.checkContent({content:content},function(res){
                let ret_ata = {
                    data :{}
                };
                if(res.state == 1){
                ...
                success: function (res) {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            if(data.data.ext == ""){
                                sdk.pay(payInfo, function (res) {
                                    // console.log('渠道支付结果：'+JSON.stringify(res)); // res.state 1成功，0失败
                                });
                            }else{
                                self.extDo({ext1:data.data.ext,ext2:payInfo});
                            }
        ...
        logCreateRole: function (data) {
            ...
            sdk.dataReport('createRole', {
                serverID: parseInt(data.serverid),
                serverName: data.servername,
                serverZone:'1',
                roleID: parseInt(data.roleid),
                roleName: data.rolename,
                roleLevel: parseInt(data.rolelevel),
                createTime:parseInt(data.rolecreatetime),
                levelTime:0,
                roleVip:0,
                balance:0,
                profession:'',
        ...
        logEnterGame: function (data) {
            ...
            sdk.dataReport('enterGame', roleInfo);
        },
...
exports.requestSubscribe = function (callback) {
    run('requestSubscribe',callback);
}