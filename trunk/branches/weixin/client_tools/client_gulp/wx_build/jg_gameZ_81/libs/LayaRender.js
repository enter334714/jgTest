	...
	LayaRender.body = function(render, body)
	{
		...
		if (bodyRender.sprite && bodyRender.sprite.texture)
		{
			...
			if (!sprite) {
				...
				if (body.effPendant && !body.effPendant.parent) sprite.addChild(body.effPendant);
			}
			...
			sprite.width = bodyRender.sprite.width;
			sprite.height = bodyRender.sprite.height;
			if (bodyRender.sprite.sizeGrid) sprite.sizeGrid = bodyRender.sprite.sizeGrid;
		}
		else // 没有纹理的body
		{
		...
		if(t){
			var lbl = sprite.lbl;
			if(!lbl){
				lbl = sprite.lbl = new Laya.Label();
				lbl.fontSize = 30;
				lbl.anchorX = 0.5;
				lbl.anchorY = 0.5;
				lbl.color = body.textClr ? body.textClr : "#ffffff";
				lbl.strokeColor = "#000000";
				lbl.stroke = 2;
				lbl.centerX = 0, lbl.centerY = 0;
				sprite.addChild(lbl);
		...
		if(body.pendant && !sprite.pendantSpr){
			var spr = new Laya.Image();
			spr.skin = body.pendant;
			sprite.addChild(spr);
			sprite.pendantSpr = spr;
		}
	};
