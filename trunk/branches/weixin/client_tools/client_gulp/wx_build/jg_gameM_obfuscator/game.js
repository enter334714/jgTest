var Y = wx.$M;
import 'afa1/mmmWEASAF.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.$m54201 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/mres/mfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$M = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'MMMMIBS', success: function (r) {
...
wx.onShow(function (res) {
  window.$m240 = res;
  if (window.$m204 && window.$m240) {
    console.info("小游戏切前台事件，场景值：" + window.$m240.scene);
    window.$m204(window.$m240);
    window.$m240 = null;
  }
});