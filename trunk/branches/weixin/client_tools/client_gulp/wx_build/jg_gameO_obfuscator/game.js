var c = wx.$o;
import 'aa3891/oooWEs1.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window._oQUE8A = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/ores/ofiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$o = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'ooooIBS', success: function (r) {
...
wx.onShow(function (res) {
  window._oEU8 = res;
  if (window._oE8U && window._oEU8) {
    console.info("小游戏切前台事件，场景值：" + window._oEU8.scene);
    window._oE8U(window._oEU8);
    window._oEU8 = null;
  }
});