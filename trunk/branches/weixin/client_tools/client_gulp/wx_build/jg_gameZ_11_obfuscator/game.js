var B = wx.$z;
import 'ddssss/Z11weasaf.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/z1122b.png";
...
render();
window.z04Y7LG = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Z11res/Z11iles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$z = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'Z11libs', success: function (r) {
...
wx.onShow(function (res) {
  window.z0GY = res;
  if (window.z0LYG && window.z0GY) {
    console.info("小游戏切前台事件，场景值：" + window.z0GY.scene);
    window.z0LYG(window.z0GY);
    window.z0GY = null;
  }
});