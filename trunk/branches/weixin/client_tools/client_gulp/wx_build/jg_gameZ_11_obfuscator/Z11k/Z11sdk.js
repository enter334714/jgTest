    ...
    return {
                        ...
                        if (data.state) {
                            ...
                            if (data.data.ext == '') {
                                ...
                                SDKyyw.onPayCallback = backRes => {
                                    ...
                                    console.log(B[0x3d], backRes);
                                    if (backRes.status === 200 && backRes.type === B[0x3e] && backRes.data.base64) {
                                ...
                                console.log(B[0x3f] + JSON.stringify(payData));
                                SDKyyw.pay(payData);
                            } else {
        ...
        extDo: function (data) {
            wx.navigateToMiniProgram({
                appId: data.ext1,
                path: B[0x41] + data.ext2.orderId + B[0x42] + data.ext2.amount,
                extraData: {},
                envVersion: B[0x43],
                success(res) {
                    // 打开成功
        ...
        logCreateRole: function (data) {
            ...
            var postData = {};
            postData[B[0x44]] = uid;
            postData[B[0x45]] = username;
            postData[B[0x46]] = data.roleid;
            postData[B[0x47]] = data.rolelevel;
            postData[B[0x48]] = data.rolename;
            postData[B[0x49]] = data.serverid;
        ...
        getPublicData: function () {
            ...
            return {
                ...
                dname: system.model,
                mac: B[0x4e],
                net_type: system.wifiSignal == 0 ? '4G' : B[0x4f],
                os_ver: system.system,
                sdk_ver: system.version, //存放的是微信版本号
                game_ver: config.game_ver, //存放的是SDK版本号
                device: system.platform == B[0x50] ? 1 : 2
            };
        },
        ...
        log: function (type, data) {
            ...
            console.log(B[0x51] + type);
            console.log(public_data);

            wx.request({
                url: B[0x1a] + HOST + B[0x52] + type + B[0x53] + encodeURIComponent(JSON.stringify(public_data))
            });
        },
        ...
        getLaunchOptionsSync: function (callback) {
            ...
            console.log(B[0x54] + JSON.stringify(launchOptions));
        ...
        msgCheck: function (msg, callback) {
            SDKyyw.msgSecCheck(msg, function (res) {
                console.log(B[0x55] + JSON.stringify(res));
                // 返回内容和微信返回内容一样
        ...
        bindPhone: function (data, callback) {
            ...
            console.log(B[0x57], data.phone, data.code);

            SDKyyw.bindTelephoneCallback = data => {
                console.log(B[0x58], data);
                // 成功data = {status:"1", data: res, msg: success}
                // 失败data = {status:"0", data: fail, msg: fail}
                if (data.status == 1) {
                    callback && callback(0, data);
        ...
        weiduanHelper: function (data, callback) {
            console.log(B[0x59], h5BindTelephoneOn);
            if (h5BindTelephoneOn == 1) {
                SDKyyw.openCustomerService(B[0x5a]);
            } else {
                callback && callback({ status: 0, msg: B[0x32] });
            }
        },
        subscribeMessage: function (tmplIds, callback) {
            console.log(B[0x5b] + tmplIds);
            //获取模板ID
            callbacks[B[0x5c]] = typeof callback == B[0x12] ? callback : null;
            wx.requestSubscribeMessage({
                tmplIds: tmplIds,
                success(res) {
                    console.log(B[0x5d]);
                    console.log(res);
                    callbacks[B[0x5c]] && callbacks[B[0x5c]](res);
                },
                fail(res) {
                    console.log(B[0x5e]);
                    console.log(res);
...
exports.sendCode = function (data, callback) {
    run(B[0x69], data, callback);
};

exports.bindPhone = function (data, callback) {
    run(B[0x6a], data, callback);
};
exports.subscribeMessage = function (data, callback) {
    run(B[0x5c], data, callback);
};