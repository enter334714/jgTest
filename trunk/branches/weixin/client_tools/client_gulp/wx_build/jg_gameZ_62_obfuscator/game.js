var f = wx.$B;
import 'ywemm/cz.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/ax9tc.png";
...
render();
window.t1$RJWS = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/a1hq/w8o.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.t1SR = res;
  if (window.t1WRS && window.t1SR) {
    console.info("小游戏切前台事件，场景值：" + window.t1SR.scene);
    window.t1WRS(window.t1SR);
    window.t1SR = null;
  }
});