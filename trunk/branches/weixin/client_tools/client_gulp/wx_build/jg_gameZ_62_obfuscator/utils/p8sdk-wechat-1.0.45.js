...
function goto_gift() {
    ...
    t.sign = hex_md5(a);
    print("弹窗开关参数 ", t);
    let i = `${sdklogData.platform_url}/switch/wxToMsg`;
    let n = function (e) {
        let t = e.data.data;
        if (!window.localStorage.getItem("show_num")) {
            window.localStorage.setItem("show_num", 1);
        }
        console.log("当前记录次数", window.localStorage.getItem("show_num"));
        let a = parseInt(window.localStorage.getItem("show_num"));
        print("弹窗开关返回的值:", t);
        ...
        if (t.s == true) {
            if (a <= t.show_num && t.show_num != 0) {
                a = a + 1;
                window.localStorage.setItem("show_num", a);
                show();
            } else if (t.show_num == 0) {
                show();
            } else {
                print("不显示弹窗");
            }
...
export function wxshowShareMenu(e = "", t = "", a = "") {
...
export function wxshareAppMessage(e = "", t = "", a = "") {
...
export function sceneADShow(e, t) {
...
export function gotoSystem(e, t, a) {
    ...
    wx.login({
        success(r) {
            if (r.code) {
                ...
                console.log("已经获取到登录jscode 登录获取openid请求参数：", a);
                print("已经获取到登录jscode 登录获取openid请求参数：", a);
                if (systemType === "TT") {
                ...
                let o = `${sdklogData.platform_url}/oauth/wxLoginToReport`;
                let e = "POST";
                let t = function (i) {
...
function getTempLateIDs() {
    ...
    a.sign = hex_md5(t);
    print("订阅推送消息请求参数 ", a);
    let i = `${sdklogData.platform_url}/oauth/getWxSubscriptionId`;
    let n = "GET";
    let o = t => {
...
export function pay(g) {
    ...
    let e = new Promise((l, e) => {
        ...
        ChangeUndefined(a);
        i = newSignGetType(a);
        var n = hex_md5(`${c}${sdklogData.site}${i}${sdklogData.key}${c}`);
        a.sign = n;
        print("支付请求参数", a);
        let o = `${sdklogData.rg_url}/h/p`;
        let r = "GET";
        let s = d => {
            if (d.data.result == 0) {
                if (false) {} else if (d.data.data.paytype == 5) {
                    ...
                    print(" 支付正常返回 开始请求检查余额 ", r);
                    var o = "";
                    for (let e in r) {
                        o += "&" + e + "=" + r[e];
                    }
                    o = o.substring(1);
                    var s = hex_md5(`${sdklogData.site}${o}${sdklogData.key}`);
                    r.sign = s;
                        ...
                        if (t.result == 1) {
                            ...
                            if (t.data && t.data.errcode == 90013) {
                                ...
                                var a = {
                                    result: 1,
                                    data: {
                                        errorcode: t.data && t.data.errcode,
                                        msg: t.data && t.data.errmsg,
                                        res: "服务端返回参数：" + t
                                    }
                                };
