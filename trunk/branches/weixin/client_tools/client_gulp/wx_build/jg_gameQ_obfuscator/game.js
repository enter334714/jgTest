var g = wx.$Q;
import 'pa1/qWE41q.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window._qAY7UO = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/qres/qfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$Q = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'qIBSq', success: function (r) {
...
wx.onShow(function (res) {
  window._q7YU = res;
  if (window._q7UY && window._q7YU) {
    console.info("小游戏切前台事件，场景值：" + window._q7YU.scene);
    window._q7UY(window._q7YU);
    window._q7YU = null;
  }
});