var K = wx.$S;
import 'sa11/sWE4ss.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.s1MT4PX = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/sses/ssiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$S = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'sIBSs', success: function (r) {
...
wx.onShow(function (res) {
  window.s14TP = res;
  if (window.s14PT && window.s14TP) {
    console.info("小游戏切前台事件，场景值：" + window.s14TP.scene);
    window.s14PT(window.s14TP);
    window.s14TP = null;
  }
});