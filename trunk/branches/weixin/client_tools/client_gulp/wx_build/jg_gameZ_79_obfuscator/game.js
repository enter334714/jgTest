var b = wx.e$;
import 'brd/ac.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.e1$F6EV = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/jap/tjo.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.e1VF = res;
  if (window.e1EFV && window.e1VF) {
    console.info("小游戏切前台事件，场景值：" + window.e1VF.scene);
    window.e1EFV(window.e1VF);
    window.e1VF = null;
  }
});