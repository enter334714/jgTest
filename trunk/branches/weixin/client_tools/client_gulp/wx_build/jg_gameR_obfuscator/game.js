...
render();
window._rUSEFY = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/rres/rfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window._rESF = res;
  if (window._rEFS && window._rESF) {
    console.info("小游戏切前台事件，场景值：" + window._rESF.scene);
    window._rEFS(window._rESF);
    window._rESF = null;
  }
});