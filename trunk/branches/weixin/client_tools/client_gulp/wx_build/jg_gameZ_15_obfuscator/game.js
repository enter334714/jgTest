...
render();
window.P__GU9V = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/z15ttes/z15ttiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.P_VG = res;
  if (window.P_9GV && window.P_VG) {
    console.info("小游戏切前台事件，场景值：" + window.P_VG.scene);
    window.P_9GV(window.P_VG);
    window.P_VG = null;
  }
});