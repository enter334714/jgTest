var b = wx.$e;
import 'd1p8a/gugxq.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.e1ANIZD = setInterval(function () {
  render();
}, 16);
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'z49slibs', success: function (r) {
...
wx.onShow(function (res) {
  window.e1DN = res;
  if (window.e1ZND && window.e1DN) {
    console.info("小游戏切前台事件，场景值：" + window.e1DN.scene);
    window.e1ZND(window.e1DN);
    window.e1DN = null;
  }
});