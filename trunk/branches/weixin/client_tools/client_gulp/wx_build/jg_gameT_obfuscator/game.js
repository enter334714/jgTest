...
render();
window.t_FMK6Z = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/ttes/ttiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.t_KM6 = res;
  if (window.t_K6M && window.t_KM6) {
    console.info("小游戏切前台事件，场景值：" + window.t_KM6.scene);
    window.t_K6M(window.t_KM6);
    window.t_KM6 = null;
  }
});