...
render();
window.$lTZNMO = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/z13res/z13files.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.$lOZ = res;
  if (window.$lMZO && window.$lOZ) {
    console.info("小游戏切前台事件，场景值：" + window.$lOZ.scene);
    window.$lMZO(window.$lOZ);
    window.$lOZ = null;
  }
});