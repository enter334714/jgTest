...
var config = {
    ...
    game_ver: '50.0.5',
    is_auth: false, //授权登录
    partner_app_id: b[70025],
    partner_app_key: b[70026],
    tmpId: { 1: b[70027], 2: b[70028], 3: b[70029] // 订阅的类型 和 模板id
    } };
...
function e1I0G1U() {
    ...
    return {
        ...
        init: function (ops, callback) {
            ...
            var uuid = wx.getStorageSync(b[70032]);
            var is_new;
            if (!uuid) {
                uuid = self.uuid(16, 32);
                wx.setStorageSync(b[70032], uuid);
                is_new = 1;
            } else {
                is_new = 0;
            }
            var idfv = wx.getStorageSync(b[70033]);
            if (!idfv) {
                idfv = self.uuid(16, 32);
                wx.setStorageSync(b[70033], idfv);
            }
            ...
            if (is_new && info.query && info.query.ad_code) {
                wx.setStorageSync(b[70034], info.query.ad_code);
            }
        ...
        login: function (data, callback) {
            console.log(b[70035]);
            var self = this;
            callbacks[b[70036]] = typeof callback == b[70037] ? callback : null;

            //初始化参数
            SDKyyw.initLoginCallback = res => {
                console.log(b[70038] + JSON.stringify(res));
                if (res.status == 0) {
                    console.log(b[70039]);
                    callback(1, {
                        errMsg: b[70040]
        ...
        do_login: function (info) {
            ...
            public_data[b[70041]] = info ? info.nick_name : '';
            public_data[b[70042]] = info ? info.head_img : '';

            if (e1I01GU && typeof e1I01GU == b[40297]) {
                for (var key in e1I01GU) {
                    public_data[key] = e1I01GU[key];
                }
            }

            public_data[b[70043]] = info.uid;

            wx.request({
                url: b[66400] + HOST + b[70044],
                method: b[70045],
                dataType: b[46192],
                header: {
                    'content-type': b[70046] // 默认值
                },
                data: public_data,
                success: function (res) {
                    console.log(b[70047] + JSON.stringify(res));
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            try {
                                wx.setStorageSync(b[70048], data.data.sdk_token);
                                wx.setStorageSync(b[70049], data.data.user_id);
                                wx.setStorageSync(b[70050], data.data.username);
                                if (data.data.ext) {
                                    wx.setStorageSync(b[70051], data.data.ext);
                                }
                            } catch (e) {}

                            callbacks[b[70036]] && callbacks[b[70036]](0, userData);
                        } else {
                            callbacks[b[70036]] && callbacks[b[70036]](1, { errMsg: data.msg });
                        }

                        //登录成功，加载右上角分享数据
                        self.getShareInfo(b[70052], function (data) {
                            console.log(b[70053]);
                            wx.onShareAppMessage(function () {
                                //记录开始分享
                                self.logStartShare(b[70052]);
                                return {
                                    title: data.title,
                                    imageUrl: data.img,
                                    query: data.query
                        ...
                        callbacks[b[70036]] && callbacks[b[70036]](1, { errMsg: b[70054] });
                    }
                }
            });
        },
        ...
        share: function (data) {
            callbacks[b[70055]] = typeof callback == b[70037] ? callback : null;
            var type = data.type || b[70055];
            console.log(b[70056] + type);
            var self = this;
        ...
        logStartShare: function (type) {
            var sdk_token = wx.getStorageSync(b[70048]);
            wx.request({
                url: b[66400] + HOST + b[70057],
                method: b[70045],
                dataType: b[46192],
                header: {
                    'content-type': b[70046] // 默认值
                ...
                success: function (res) {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            callbacks[b[70055]] && callbacks[b[70055]](1, { errMsg: b[70064] + data.msg });
                        }
                    } else {
                        callbacks[b[70055]] && callbacks[b[70055]](1, { errMsg: b[70065] });
                    }
                }
            });
        },
        ...
        switchGame: function (callbacks) {
            ...
            if (partner_swtich_info.showSwitchOn == 1) {
                ...
                callbacks({ status: 0, msg: b[70066] });
            }
        },
        ...
        startPay: function (data, callback) {
            ...
            var self = this;
            callbacks[b[70072]] = typeof callback == b[70037] ? callback : null;
            //先下单
            this_pay_order = 0;
            var sdk_token = wx.getStorageSync(b[70048]);
            var session_key = wx.getStorageSync(b[70051]);
            if (!sdk_token && !session_key) {
                callbacks[b[70072]] && callbacks[b[70072]](1, { errMsg: b[70073] });
                return;
            }
            ...
            var public_data = self.getPublicData();
            public_data[b[70074]] = JSON.stringify(order_data);

            //发起网络请求
            wx.request({
                url: b[66400] + HOST + b[70075],
                method: b[70045],
                dataType: b[46192],
                header: {
                    'content-type': b[70046] // 默认值
                ...
                data: public_data,
                success: function (res) {
                    console.log(b[70076] + JSON.stringify(res));
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            if (data.data.ext == '') {
                                ...
                                console.log(b[70077] + JSON.stringify(payData));
                                SDKyyw.pay(payData);
                            } else {
                                self.extDo({ ext1: data.data.ext, ext2: data.data.pay_data });
                            }
                        } else {
                            callbacks[b[70072]] && callbacks[b[70072]](1, { errMsg: data.msg });
                        }
                    } else {
                        callbacks[b[70036]] && callbacks[b[70036]](1, { errMsg: b[70054] });
                    }
        ...
        extDo: function (data) {
            wx.navigateToMiniProgram({
                appId: data.ext1,
                path: b[70078] + data.ext2.orderId + b[70079] + data.ext2.amount,
                extraData: {},
                envVersion: b[70080],
                success(res) {
                    // 打开成功
        ...
        logCreateRole: function (data) {
            var uid = wx.getStorageSync(b[70049]);
            var username = wx.getStorageSync(b[70050]);

            var postData = {};
            postData[b[70081]] = uid;
            postData[b[70082]] = username;
            postData[b[51263]] = data.roleid;
            postData[b[70083]] = data.rolelevel;
            postData[b[70084]] = data.rolename;
            postData[b[51940]] = data.serverid;
        ...
        getPublicData: function () {
            ...
            var uuid = wx.getStorageSync(b[70032]);
            var idfv = wx.getStorageSync(b[70033]);
            var ad_code = wx.getStorageSync(b[70034]);
            ...
            return {
                ...
                dname: system.model,
                mac: b[70087],
                net_type: system.wifiSignal == 0 ? '4G' : b[70088],
                os_ver: system.system,
                sdk_ver: system.version, //存放的是微信版本号
                game_ver: config.game_ver, //存放的是SDK版本号
                device: system.platform == b[70089] ? 1 : 2
            };
        },
        ...
        log: function (type, data) {
            ...
            console.log(b[70090] + type);
            console.log(public_data);

            wx.request({
                url: b[66400] + HOST + b[70091] + type + b[70092] + encodeURIComponent(JSON.stringify(public_data))
            });
        },
        ...
        getLaunchOptionsSync: function (callback) {
            ...
            console.log(b[70093] + JSON.stringify(launchOptions));
        ...
        msgCheck: function (msg, callback) {
            SDKyyw.msgSecCheck(msg, function (res) {
                console.log(b[70094] + JSON.stringify(res));
                // 返回内容和微信返回内容一样
        ...
        sendCode: function (data, callback) {
            ...
            console.log(b[70095], data.phone);
        ...
        bindPhone: function (data, callback) {
            ...
            console.log(b[70096], data.phone, data.code);

            SDKyyw.bindTelephoneCallback = data => {
                console.log(b[70097], data);
                // 成功data = {status:"1", data: res, msg: success}
                // 失败data = {status:"0", data: fail, msg: fail}
                if (data.status == 1) {
                    callback && callback(0, data);
        ...
        weiduanHelper: function (data, callback) {
            console.log(b[70098], h5BindTelephoneOn);
            if (h5BindTelephoneOn == 1) {
                SDKyyw.openCustomerService(b[70099]);
            } else {
                callback && callback({ status: 0, msg: b[70066] });
            }
        }
    };
}
...
exports.init = function (data, callback) {
    run(b[40392], data, callback);
};

exports.login = function (callback) {
    run(b[70036], '', callback);
};
