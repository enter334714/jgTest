...
var config = {
    ...
    game_ver: b[31034],
    is_auth: false, //授权登录
    partner_app_id: b[31035],
    partner_app_key: b[31036],
    tmpId: { 1: b[31037], 2: b[31038], 3: b[31039] // 订阅的类型 和 模板id
    } };
...
function e1I0G1U() {
    ...
    return {
        ...
        init: function (ops, callback) {
            ...
            var uuid = wx.getStorageSync(b[31042]);
            var is_new;
            if (!uuid) {
                uuid = self.uuid(16, 32);
                wx.setStorageSync(b[31042], uuid);
                is_new = 1;
            } else {
                is_new = 0;
            }
            var idfv = wx.getStorageSync(b[31043]);
            if (!idfv) {
                idfv = self.uuid(16, 32);
                wx.setStorageSync(b[31043], idfv);
            }
            ...
            if (is_new && info.query && info.query.ad_code) {
                wx.setStorageSync(b[31044], info.query.ad_code);
            }
        ...
        login: function (data, callback) {
            console.log(b[31045]);
            var self = this;
            callbacks[b[31046]] = typeof callback == b[30944] ? callback : null;

            //初始化参数
            SDKyyw.initLoginCallback = res => {
                console.log(b[31047] + JSON.stringify(res));
                if (res.status == 0) {
                    console.log(b[31048]);
                    callback(1, {
                        errMsg: b[31049]
        ...
        do_login: function (info) {
            ...
            public_data[b[31050]] = info ? info.nick_name : '';
            public_data[b[31051]] = info ? info.head_img : '';
            ...
            public_data[b[31052]] = info.uid;

            wx.request({
                url: b[26498] + HOST + b[31053],
                method: b[31054],
                dataType: b[5628],
                header: {
                    'content-type': b[31055] // 默认值
                },
                data: public_data,
                success: function (res) {
                    console.log(b[31056] + JSON.stringify(res));
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            try {
                                wx.setStorageSync(b[31057], data.data.sdk_token);
                                wx.setStorageSync(b[31058], data.data.user_id);
                                wx.setStorageSync(b[31059], data.data.username);
                                if (data.data.ext) {
                                    wx.setStorageSync(b[31060], data.data.ext);
                                }
                            } catch (e) {}

                            callbacks[b[31046]] && callbacks[b[31046]](0, userData);
                        } else {
                            callbacks[b[31046]] && callbacks[b[31046]](1, { errMsg: data.msg });
                        }

                        //登录成功，加载右上角分享数据
                        self.getShareInfo(b[31061], function (data) {
                            console.log(b[31062]);
                            wx.onShareAppMessage(function () {
                                //记录开始分享
                                self.logStartShare(b[31061]);
                                return {
                                    title: data.title,
                                    imageUrl: data.img,
                                    query: data.query
                        ...
                        callbacks[b[31046]] && callbacks[b[31046]](1, { errMsg: b[31063] });
                    }
                }
            });
        },
        ...
        share: function (data) {
            callbacks[b[31064]] = typeof callback == b[30944] ? callback : null;
            var type = data.type || b[31064];
            console.log(b[31065] + type);
            var self = this;
        ...
        logStartShare: function (type) {
            var sdk_token = wx.getStorageSync(b[31057]);
            wx.request({
                url: b[26498] + HOST + b[31066],
                method: b[31054],
                dataType: b[5628],
                header: {
                    'content-type': b[31055] // 默认值
                ...
                success: function (res) {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            ...
                            callbacks[b[31064]] && callbacks[b[31064]](1, { errMsg: b[31073] + data.msg });
                        }
                    } else {
                        callbacks[b[31064]] && callbacks[b[31064]](1, { errMsg: b[31074] });
                    }
                }
            });
        },
        ...
        switchGame: function (callbacks) {
            ...
            if (partner_swtich_info.showSwitchOn == 1) {
                ...
                callbacks({ status: 0, msg: b[31075] });
            }
        },
        ...
        startPay: function (data, callback) {
            ...
            var self = this;
            callbacks[b[31081]] = typeof callback == b[30944] ? callback : null;
            //先下单
            this_pay_order = 0;
            var sdk_token = wx.getStorageSync(b[31057]);
            var session_key = wx.getStorageSync(b[31060]);
            if (!sdk_token && !session_key) {
                callbacks[b[31081]] && callbacks[b[31081]](1, { errMsg: b[31082] });
                return;
            }
            ...
            var public_data = self.getPublicData();
            public_data[b[31083]] = JSON.stringify(order_data);

            //发起网络请求
            wx.request({
                url: b[26498] + HOST + b[31084],
                method: b[31054],
                dataType: b[5628],
                header: {
                    'content-type': b[31055] // 默认值
                ...
                data: public_data,
                success: function (res) {
                    console.log(b[31085] + JSON.stringify(res));
                        ...
                        if (data.state) {
                            ...
                            if (data.data.ext == '') {
                                ...
                                SDKyyw.onPayCallback = backRes => {
                                    console.log(b[31086], backRes);
                                    if (backRes.status === 200 && backRes.type === b[31087] && backRes.data.base64) {
                                ...
                                console.log(b[31088] + JSON.stringify(payData));
                                SDKyyw.pay(payData);
                            } else {
                                self.extDo({ ext1: data.data.ext, ext2: data.data.pay_data });
                            }
                        } else {
                            callbacks[b[31081]] && callbacks[b[31081]](1, { errMsg: data.msg });
                        }
                    } else {
                        callbacks[b[31046]] && callbacks[b[31046]](1, { errMsg: b[31063] });
                    }
        ...
        extDo: function (data) {
            wx.navigateToMiniProgram({
                appId: data.ext1,
                path: b[31090] + data.ext2.orderId + b[31091] + data.ext2.amount,
                extraData: {},
                envVersion: b[31092],
                success(res) {
                    // 打开成功
        ...
        logCreateRole: function (data) {
            var uid = wx.getStorageSync(b[31058]);
            var username = wx.getStorageSync(b[31059]);

            var postData = {};
            postData[b[31093]] = uid;
            postData[b[31094]] = username;
            postData[b[10814]] = data.roleid;
            postData[b[31095]] = data.rolelevel;
            postData[b[31096]] = data.rolename;
            postData[b[11492]] = data.serverid;
        ...
        getPublicData: function () {
            ...
            var uuid = wx.getStorageSync(b[31042]);
            var idfv = wx.getStorageSync(b[31043]);
            var ad_code = wx.getStorageSync(b[31044]);
            ...
            return {
                ...
                dname: system.model,
                mac: b[31099],
                net_type: system.wifiSignal == 0 ? '4G' : b[31100],
                os_ver: system.system,
                sdk_ver: system.version, //存放的是微信版本号
                game_ver: config.game_ver, //存放的是SDK版本号
                device: system.platform == b[31101] ? 1 : 2
            };
        },
        ...
        log: function (type, data) {
            ...
            console.log(b[31102] + type);
            console.log(public_data);

            wx.request({
                url: b[26498] + HOST + b[31103] + type + b[31104] + encodeURIComponent(JSON.stringify(public_data))
            });
        },
        ...
        getLaunchOptionsSync: function (callback) {
            ...
            console.log(b[31105] + JSON.stringify(launchOptions));
        ...
        msgCheck: function (msg, callback) {
            SDKyyw.msgSecCheck(msg, function (res) {
                console.log(b[31106] + JSON.stringify(res));
                // 返回内容和微信返回内容一样
        ...
        sendCode: function (data, callback) {
            ...
            console.log(b[31107], data.phone);
        ...
        bindPhone: function (data, callback) {
            ...
            console.log(b[31108], data.phone, data.code);

            SDKyyw.bindTelephoneCallback = data => {
                console.log(b[31109], data);
                // 成功data = {status:"1", data: res, msg: success}
                // 失败data = {status:"0", data: fail, msg: fail}
                if (data.status == 1) {
                    callback && callback(0, data);
        ...
        weiduanHelper: function (data, callback) {
            console.log(b[31110], h5BindTelephoneOn);
            if (h5BindTelephoneOn == 1) {
                SDKyyw.openCustomerService(b[31111]);
            } else {
                callback && callback({ status: 0, msg: b[31075] });
            }
        },
        subscribeMessage: function (tmplIds, callback) {
            console.log(b[31112] + tmplIds);
            //获取模板ID
            callbacks[b[31113]] = typeof callback == b[30944] ? callback : null;
            wx.requestSubscribeMessage({
                tmplIds: tmplIds,
                success(res) {
                    console.log(b[31114]);
                    console.log(res);
                    callbacks[b[31113]] && callbacks[b[31113]](res);
                },
                fail(res) {
                    console.log(b[31115]);
                    console.log(res);
