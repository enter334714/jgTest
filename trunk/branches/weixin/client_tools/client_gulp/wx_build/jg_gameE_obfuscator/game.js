var b = wx.$e;
import 'eessss/eeeweasaf.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.E$NOECV = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/eres/efiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'eeelibs', success: function (r) {
...
wx.onShow(function (res) {
  window.E$EOC = res;
  if (window.E$ECO && window.E$EOC) {
    console.info("小游戏切前台事件，场景值：" + window.E$EOC.scene);
    window.E$ECO(window.E$EOC);
    window.E$EOC = null;
  }
});