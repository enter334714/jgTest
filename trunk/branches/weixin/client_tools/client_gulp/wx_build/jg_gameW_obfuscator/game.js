var s = wx.$W;
import 'wv2/wwE4ww.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window._wBQ302 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/wwes/wwiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$W = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'wIBw', success: function (r) {
...
wx.onShow(function (res) {
  window._w3Q0 = res;
  if (window._w30Q && window._w3Q0) {
    console.info("小游戏切前台事件，场景值：" + window._w3Q0.scene);
    window._w30Q(window._w3Q0);
    window._w3Q0 = null;
  }
});