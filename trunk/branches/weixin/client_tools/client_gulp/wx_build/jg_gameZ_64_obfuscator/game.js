var f = wx.$B;
import 'a18e/jqv6n.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/p3kr.png";
...
render();
window.t1CZLOI = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/c9a/dw7vs.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.t1IZ = res;
  if (window.t1OZI && window.t1IZ) {
    console.info("小游戏切前台事件，场景值：" + window.t1IZ.scene);
    window.t1OZI(window.t1IZ);
    window.t1IZ = null;
  }
});