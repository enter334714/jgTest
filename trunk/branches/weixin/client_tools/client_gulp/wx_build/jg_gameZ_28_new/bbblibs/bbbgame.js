...
wx.onError(function (error) {
  if (error) {
    if (error.message) {
      ...
      var arr = message.match(/(bbbbbbbbb\/bbbgame.js:)[0-9]{1,60}(:)/g);
      if (arr) {
        for (var i = 0; i < arr.length; i++) {
          //行数减2
          if (arr[i] && arr[i].length > 0) {
            var line = parseInt(arr[i].replace("bbbbbbbbb/bbbgame.js:", "").replace(":", ""));
            message = message.replace(arr[i], arr[i].replace(":" + line + ":", ":" + (line - 2) + ":"));
          }
        }
      }
      message = message.replace(new RegExp("bbbbbbbbb/bbbgame.js", "g"), "bbbbbbbbb/main__" + gamever + ".min.js");
      message = message.replace(new RegExp("bbbbbbbbb/lsjdflaamain.js", "g"), "bbbbbbbbb/main__" + gamever + ".min.js");
...
import "bbbmd5min.js";
import "bbbzlibs.js";
window["Parser"] = require("bbbdomparser.js");
import "bbbindex.js";
