...
window.sdkOnInited = function(res) {
  ...
  if (!res.game_ver || window.compareVersion(window.versions.wxVersion, res.game_ver) < 0) {  //当前版本 < 后台版本   
    ...
    window["ShieldNoise"] = "1.0#0.1#0.0044#0.0034"; //随机数(0~100) # 透明度(0~1) # 马赛克宽 # 马赛克高
    window["ShieldColor"] = "#FFFFCCFF";
  } else {
    console.log("#开发版=============================");
...
window.updCurServer = function(response) {
  PF_INFO.newRegister = 1;//response.is_new != undefined ? response.is_new : 0;
  PF_INFO.selectedServer = {
