var f = wx.$B;
import 'b0p/a593fp.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/a64ih.png";
...
render();
window.t1OBKYW = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/yg4a/gqhi.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'qdei1a', success: function (r) {
...
wx.onShow(function (res) {
  window.t1WB = res;
  if (window.t1YBW && window.t1WB) {
    console.info("小游戏切前台事件，场景值：" + window.t1WB.scene);
    window.t1YBW(window.t1WB);
    window.t1WB = null;
  }
});