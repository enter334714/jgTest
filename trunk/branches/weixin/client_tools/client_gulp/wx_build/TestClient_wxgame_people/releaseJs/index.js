...
var __reflect = this && this.__reflect || function (t, r, h) {
    t.__class__ = r, h ? h.push(r) : h = [r], t.__types__ = t.__types__ ? h.concat(t.__types__) : h;
},
    md5 = function () {
    function t() {
        ...
        var h = this.rstr2binl(t);h.length > 16 && (h = this.binl_md5(h, 8 * t.length));for (var i = Array(16), s = Array(16), _ = 0; 16 > _; _++) i[_] = 909522486 ^ h[_], s[_] = 1549556828 ^ h[_];var d = this.binl_md5(i.concat(this.rstr2binl(r)), 512 + 8 * r.length);return this.binl2rstr(this.binl_md5(s.concat(d), 640));
    }, t.prototype.rstr2hex = function (t) {
        try {
            this.hexcase;
        } catch (r) {
            this.hexcase = 0;
        }for (var h, i = this.hexcase ? "0123456789ABCDEF" : "0123456789abcdef", s = "", _ = 0; _ < t.length; _++) h = t.charCodeAt(_), s += i.charAt(h >>> 4 & 15) + i.charAt(15 & h);return s;
    }, t.prototype.rstr2b64 = function (t) {
        try {
            this.b64pad;
        } catch (r) {
            this.b64pad = "";
        }for (var h = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", i = "", s = t.length, _ = 0; s > _; _ += 3) for (var d = t.charCodeAt(_) << 16 | (s > _ + 1 ? t.charCodeAt(_ + 1) << 8 : 0) | (s > _ + 2 ? t.charCodeAt(_ + 2) : 0), n = 0; 4 > n; n++) i += 8 * _ + 6 * n > 8 * t.length ? this.b64pad : h.charAt(d >>> 6 * (3 - n) & 63);return i;
        ...
        var h = (65535 & t) + (65535 & r),
            i = (t >> 16) + (r >> 16) + (h >> 16);return i << 16 | 65535 & h;
...
//资源版本
window["resVersion"] = "?v=20191119";
window["thmVersion"] = "?v=20191119";
...
if(window["checkVer"] == 1) //测试
{
	//拉取服务器列表
	window["serviceListdUrl"] = "https://api.k1.5tun.cn/server_list_zr_wxts/Serverlist.php";
	//保存最近登录
	window["setServiceListdUrl"] = "https://api.k1.5tun.cn/server_list_zr_wxts/Serverlist.php";
	window["Report"] = "2";
	window["myresroot"] = "https://static.k1.5tun.cn/zr_wxts";
	window["pf"] = "zr_wxts";
}
else
{
...
function showLoadProgress(progress, des) {
    ...
    if (pIframe) {
        ...
        if (selectServer) selectServer.showLoadProgress(progress, des);
    }
...
function start(param) {
    ...
    var mainJsUrl = '../releaseMainJs/main.min.js';
    if (window["wxdebug"] == 0) mainJsUrl = '../debugMainJs/main.js';
