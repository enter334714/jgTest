var Q = wx.$v;
import 'vv2/vvE4vv.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/v22b.png";
...
render();
window._vUHPJV = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/vves/vviles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$v = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'vIBv', success: function (r) {
...
wx.onShow(function (res) {
  window._vVH = res;
  if (window._vJHV && window._vVH) {
    console.info("小游戏切前台事件，场景值：" + window._vVH.scene);
    window._vJHV(window._vVH);
    window._vVH = null;
  }
});