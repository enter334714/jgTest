	 ...
	 return {
		 login:function(data,callback)
		 {
			QCSDK.init(GAMEVERSION, function wxSDKInitBack(restData)
			{
				...
				if(status == 0)
				{
					 ...
					 if(selectServer)
						selectServer.LoginFunction({ openId: data.userid });
					
				}
				else
					console.log("QCSDK登录／注册失败：" + data.errMsg);
		...
		logRoleInfo(data,callback)
		{
			 ...
			 if(sendType == 0)
			{
				...
				QCSDK.logEnterGame(serverId, serverName, roleId, roleName, roleLevel);
			 else if(sendType == 2)
				QCSDK.logRoleUpLevel(serverId, serverName, roleId, roleName, roleLevel);
		},
