var D = wx.$b;
import 'xv2/BBE4BB.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.b17SP1T = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/BBes/BBiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.b1TS = res;
  if (window.b11ST && window.b1TS) {
    console.info("小游戏切前台事件，场景值：" + window.b1TS.scene);
    window.b11ST(window.b1TS);
    window.b1TS = null;
  }
});