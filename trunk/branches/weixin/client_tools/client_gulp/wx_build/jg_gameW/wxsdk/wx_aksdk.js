    ...
    return {
        ...
        init: function (ops, callback) {
            ...
            qingjs.instance.onInit(function (initResult) {
                console.log("[SDK]初始化回调",initResult);
            });
        ...
        login: function (data,callback) {
            ...
            qingjs.instance.login({}, function (loginResult) {
                console.log("[SDK]登录回调:",loginResult);
        ...
        logEnterGame: function (data, callback) {
            ...
            qingjs.instance.reportRoleInfo(roleInfo ,function(result){
                console.log("[SDK]角色进入游戏回调：", result);
                if(qingjs.instance.canPay()){
                    callback && callback({'show_pay':1});
                }else{
                    callback && callback({'show_pay':0});
        ...
        logRoleUpLevel: function (data, callback) {
            ...
            var roleInfo = {
                ...
                roleVip: 0,
                rolePower: data.rolepower ? data.rolepower : 0,
                reportType: "roleupgrade"
            };

            qingjs.instance.reportRoleInfo(roleInfo, function(result){
                console.log("[SDK]角色升级回调：", result);
                if(qingjs.instance.canPay()){
                    callback && callback({'show_pay':1});
                }else{
                    callback && callback({'show_pay':0});
        ...
        beQuit: function (data) {
            wx.request({
                url: 'https://' + HOST + '/partner/data/report/'+config.partner_id+'/'+config.game_pkg,
                method: 'POST',
                dataType: 'json',
                header: {
                    'content-type': 'application/x-www-form-urlencoded' // 默认值
                ...
                success: function (res) {
                    console.log('下线上报结果'+JSON.stringify(res));
                }
            });
