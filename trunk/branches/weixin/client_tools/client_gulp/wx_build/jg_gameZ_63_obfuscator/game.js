var f = wx.$B;
import 'yxppi/ufi.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/a2t.png";
...
render();
window.t18XW$H = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/a3quf/t1v96a.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.t1HX = res;
  if (window.t1$XH && window.t1HX) {
    console.info("小游戏切前台事件，场景值：" + window.t1HX.scene);
    window.t1$XH(window.t1HX);
    window.t1HX = null;
  }
