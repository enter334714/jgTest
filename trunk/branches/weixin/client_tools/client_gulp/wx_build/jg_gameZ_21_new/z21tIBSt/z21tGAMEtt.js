...
wx.onError(function (error) {
  if (error) {
    if (error.message) {
      ...
      var arr = message.match(/(z21ttttttt\/z21tGAMEtt.js:)[0-9]{1,60}(:)/g);
      if (arr) {
        for (var i = 0; i < arr.length; i++) {
          //行数减2
          if (arr[i] && arr[i].length > 0) {
            var line = parseInt(arr[i].replace("z21ttttttt/z21tGAMEtt.js:", "").replace(":", ""));
            message = message.replace(arr[i], arr[i].replace(":" + line + ":", ":" + (line - 2) + ":"));
          }
        }
      }
      message = message.replace(new RegExp("z21ttttttt/z21tGAMEtt.js", "g"), "z21ttttttt/main__" + gamever + ".min.js");
      message = message.replace(new RegExp("z21ttttttt/z21MAItttt.js", "g"), "z21ttttttt/main__" + gamever + ".min.js");
