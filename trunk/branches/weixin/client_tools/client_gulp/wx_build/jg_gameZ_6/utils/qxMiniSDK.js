...
var qxMiniSDK = {
  ...
  login(xcxUserInfo = {}, callback) {
    ...
    that.checkSession(function (res) {
      console.log('[checkSessionRes]', res)
      if (res.statusCode === 0) {
      ...
      success: ret => {
        ...
        request('minigame', 'login', loginParams, 1).then((result) => {
          ...
          if (result.code == 0) {
            ...
            //配置被动分享信息
            var shareData = that.onShareToArk({})
  ...
  getUnionid: function (data) {
    ...
    wx.getSetting({
      success(res) {
        if (res.authSetting["scope.userInfo"] && data.ext.is_unionid != '1') {
          console.log("用户已授权");
        } else {
          console.log("用户未授权");
  ...
  _requestMidasPayment(data = {}, paycbData = {}, callback) {
    ...
    wx.requestMidasPayment({
      ...
      success(res) {
        // 发起pay回调请求
        console.log(res, '米大师成功')
        that._payCallback(paycbData, callback)
      },
      fail(res) {
        console.log(res, '米大师失败')
        if (callback) {
          callback({
            statusCode: 1,
            status: '米大师fail'
  ...
  //登录成功显示提示
  _loginTips: function (roleParams) {
  ...
  shareToArk(params = {}, callback) {
    var options = this.onShareToArk()
    options = __assign(options, params)
  ...
  onShareToArk(params = {}) {
    var options = {
      ...
      query: 'share_user=' + localUserInfo.user_id
    }
  ...
  showEgretFloatBall(egretobj, roleParams = {}) {
    ...
    btn.init({
      iconpath: ["https://yxfile.gowan8.com/upload/icon/wxmini/bg-left.png", "https://yxfile.gowan8.com/upload/icon/wxmini/bg-right.png", "https://yxfile.gowan8.com/upload/icon/wxmini/main.png", "https://yxfile.gowan8.com/upload/icon/wxmini/weixin.png", "https://yxfile.gowan8.com/upload/icon/wxmini/youxi.png"],
      texts: ["公众号", "升 级"],
      icon: {
        posX: 200,
        posY: 340
