import gameSDK from "../utils/fhhj.bundle"
var config = {
    game_id: '256',
    game_pkg: 'tjqy_tjqygfwxxyxfhhj_ACM',
    partner_id: '664',
    game_ver: '125.0.3',
    is_auth: false, //授权登录
    partner_label: 'gfxyxx',
};
...
function mainSDK() {
    ...
    return {
        ...
        login: function (data, callback) {
            ...
            gameSDK.init(() => {
              console.log("【渠道】初始化成功");
              gameSDK.entryGame(() => {
                gameSDK.loginUserInfo((res) => {
                  console.log("【渠道】登录成功");
                  parter_user_info = res;
                  self.do_login(res);
                })
              });
            ...
            wx.request({
                ...
                success: function (res) {
                    ...
                    if(res.statusCode == 200){
                        ...
                        if(pay_data.state){
                          if(pay_data.data.ext == ''){
                            gameSDK.makePayment(pay_data.data.pay_data)
                          }else{
                            var pay_data1 = pay_data.data.pay_data;
                            pay_data1['money'] = order_data.price
                            self.extDo(pay_data.data.ext,pay_data1);
                          }
                        }else{
        ...
        extDo: function(ext1,ext2){
          wx.navigateToMiniProgram({
              appId: ext1,
              path: 'pages/pay/pay?order_id='+ext2.ext+'&money='+ext2.money,
              extraData: {
