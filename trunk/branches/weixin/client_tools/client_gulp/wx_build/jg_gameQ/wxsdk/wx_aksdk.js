...
var config = {
    game_id: 256,
    game_pkg: 'tjqy_tjqyqmldg_LS', //趣侬vs果子--全民来打怪
    partner_label: 'gzxcx',
    partner_id: '533',
    game_ver: '38.0.1',
    is_auth: false, //授权登录
    partner_game_id:"1177028",
    partner_channel_id:"1039701",
    partner_app_id:"wx8caa1c8ff4d77277",
    partner_secret:"0bfbfa8611ec255ed20fce50e2536286",
};
    ...
    return {
                ...
                this.checkGameVersion(game_ver, function (data) {                   
                    gzsdk.init({game_id:config.partner_game_id, channel_id:config.partner_channel_id, appid: config.partner_app_id, 
                        version: game_ver}, function(_res){
                            console.log("渠道初始化:"+JSON.stringify(_res));
        ...
        login: function (data,callback) {
            ...
            gzsdk.login(function(_res){
                if(_res && 200 == _res.res_code){
                    // 登录成功
                    console.log("渠道登录成功"+JSON.stringify(_res));
                    self.do_login(_res.res_msg);
                }else{
                    // 登录失败
                    console.log("渠道登录失败"+JSON.stringify(_res));
                }
              })
                        ...
                        if (data.state) {
                            if(data.data.ext == ''){
                                gzsdk.pay(data.data.pay_data,function(res){
                                    console.log("渠道下单"+JSON.stringify(res))
                                })
                            }else{
        ...
        logCreateRole: function (data) {
            ...
            var s2 = s1+"secret="+config.partner_secret;
            params['sign'] = md5(s2).toUpperCase();
            gzsdk.report(params)
        },
