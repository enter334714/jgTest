var s1 = wx.l$;
import 'u8vh/lw9a.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/dj8us.png";
...
render();
window._sTNYIW = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/mvbi/k6a.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window._sWN = res;
  if (window._sINW && window._sWN) {
    console.info("小游戏切前台事件，场景值：" + window._sWN.scene);
    window._sINW(window._sWN);
    window._sWN = null;
  }
});