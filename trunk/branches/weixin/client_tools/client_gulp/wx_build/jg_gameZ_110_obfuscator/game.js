var b = wx.$e;
import 'bo/yxxb8a.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/tx.png";
...
render();
window.e1PKVJ$ = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/a8pvm0a/aph0a.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'x85v4a', success: function (r) {
...
wx.onShow(function (res) {
  window.e1$K = res;
  if (window.e1JK$ && window.e1$K) {
    console.info("小游戏切前台事件，场景值：" + window.e1$K.scene);
    window.e1JK$(window.e1$K);
    window.e1$K = null;
  }
});