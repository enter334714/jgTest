...
let host = `rpgmoba.com`;

let api = `https://sdk-js.${host}`;
let wanBaApi = `https://wanba.${host}`;
let logApi = `https://sdk-data.${host}`;
let idkApi = `https://idk.${host}`;
...
const Tools = {
  ...
  idkBuildParams: params => {
    params.sign_version = "v4";
    if (params.sign_version === "v4") {
      if (!params.timestamp) {
        params.timestamp = Tools.getTimeStamp();
      }
      if (!params.os) {
        params.os = Tools.getDeviceType();
      }
      let _params = {
        app_id: encodeURIComponent(commonParams.gameId),
        os: encodeURIComponent(Tools.getDeviceType()),
        // pf: encodeURIComponent(commonParams.pf),
        timestamp: params.timestamp,
        key: encodeURIComponent(commonParams.gameKey)
      };
      params["sign"] = md5(Tools.setQueryConfig(_params));
    }
    return params;
  },
  dataReportBuildParams: params => {
    let valueList = Object.values(params).sort();
    let stringParams = valueList.join("");
...
const init = function (gameId, gameKey) {
  const SDKyyw = this;
  if (String(gameId).indexOf("2") === 0) {
    host = `hainanxingyue.cn`;
    api = `https://sdk-js.${host}`;
    wanBaApi = `https://wanba.${host}`;
    logApi = `https://sdk-data.${host}`;
    idkApi = `https://idk.${host}`;
  }
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        // 强制弹窗打开客服
        commonParams.showH5On.on === 1 && commonParams.showH5On.forcibly_on === 1 && quantityToH5();

        // 游戏圈
        const showGameCircle = ret.data.showGameCircle || {};
        if (showGameCircle && showGameCircle.on === 1 && showGameCircle.switch_ctrl === "sdk") {
          const config = showGameCircle && showGameCircle.config || {};
          const clubButton = gameClub.createGameClubButton(config);
          clubButton.show();
        }
 ...
 * @scens {string} 场景值
 */
const toShareAppMessage = function (data = {}) {
  wx.showLoading({
    title: "正在请求分享数据...",
    icon: "none"
  });
  ...
  data.scens && (requestData.scens = data.scens);
  wx.request({
    url: idkApi + "/v2/wx/share/index",
    data: Tools.idkBuildParams(requestData),
    success: function (res) {
      wx.hideLoading();
      if (res.data.status === 200) {
        const shareData = Object.assign(res.data.data, data);
        delete shareData.scens;
            ...
            if (payRes.data.status == 1) {
              ...
              switch (commonParams.payType) {
                ...
                case 2:
                  ...
                  wx.showModal({
                    title: commonParams.customPayOn && commonParams.customPayOn.title || "温馨提示",
                    content: commonParams.customPayOn && commonParams.customPayOn.content || "请按确定后进入客服窗口，如果未收到本订单支付入口，请发送窗口右下角卡片获取.祝您游戏愉快!",
                    showCancel: false,
                    confirmText: "确定",
                    success() {
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    fail: function (err) {
                      ...
                      wx.request({
                        url: api + "/pay/order/wxMiniPayBan?appid=" + commonParams.gameId + "&errMsg=" + err.errMsg,
                        success(res) {
...
const gameClub = {
  // 创建游戏圈按钮
  createGameClubButton(config = {}) {
    if (commonParams.clubButton) {
      commonParams.clubButton.destroy();
      commonParams.clubButton = null;
    }

    let button = wx.createGameClubButton(config);
    button.onTap(() => {
      this.pushGameClubData();
    });
    commonParams.clubButton = button;
    return button;
  },
  // 获取用户当天在游戏圈的操作
  getGameClubData(dataTypeList = []) {
    return new Promise(resolve => {
      if (!wx.getGameClubData) {
        console.error("wx.getGameClubData找不到，请升级基础库");
        return;
      }
      wx.getGameClubData({
        dataTypeList,
        success: res => {
          console.log("getGameClubData", res);
          resolve(res);
        },
  ...
  pushGameClubData() {
    ...
    this.getGameClubData(dataTypeList).then(res => {
      const params = {
        app_id: commonParams.gameId,
        encrypted_data: res.encryptedData,
        iv: res.iv,
      ...
      getWxCode().then(code => {
        params.code = code;
        wx.request({
          url: idkApi + "/v2/wx/game_circle/get_game_club_data",
          method: "post",
          data: Tools.idkBuildParams(params),
          success(res) {
            console.log("get_game_club_data: ", res);
          },
          fail(err) {
            console.log("get_game_club_data", err);
          },
          timeout: 10 * 1000
        });
      });
  ...
  return new Promise((resolve, reject) => {
    wx.login({
      success(res) {
        if (res.code == null || res.code == undefined || res.code == "") {
          console.log("微信登录失败code is null:");
          reject("微信登录失败code is null:");
        } else {
          resolve(res.code);
        }
      },
      fail(err) {
        reject("微信登录失败code is null:", err);
      }
    });
  });
};
...
const pushData = function (data) {
  ...
  // 角色上报
  if (["1", "2"].includes(String(data.type))) {
  ...
  delete data["type"];
  data.logTime = parseInt(new Date().getTime() / 1000);
  const nonce = Tools.randomString(16); // 随机字符串
  const buildData = Tools.dataReportBuildParams({
    appId: commonParams.gameId,
    appKey: commonParams.gameKey,
    nonce: nonce,
    logTime: data.logTime
  });
  const api_ver = "1.0"; // 版本号
  const requestData = {
    data: data,
    api_ver: api_ver, // 版本
    nonce: nonce, // 随机字符串
...
module.exports = {
  ...
  gameClub: gameClub,
  getWxCode: getWxCode,
  toShareAppMessage: toShareAppMessage
};