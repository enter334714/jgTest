...
render();
window.gGA086 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Z8es/Z8iles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.g6A = res;
  if (window.g8A6 && window.g6A) {
    console.info("小游戏切前台事件，场景值：" + window.g6A.scene);
    window.g8A6(window.g6A);
    window.g6A = null;
  }
});