...
let qy_sdk = {
  ...
  gameRoleUploadTypeList: [
    'JOIN_SERVER', 'CREATE_ROLE', 'UPGRADE', 'FACTION', 'EXIT', 'OTHER'
  ],
  ...
  role: function (callback, gameRoleInfo) {
    ...
    // 限制角色上报频率
    let gameRoleInfoStorageName = gameRoleInfo['game_id'] + gameRoleInfo['user_id'] +
      gameRoleInfo['cp_server_id'] + gameRoleInfo['cp_role_id'] + gameRoleInfo['upload_type'],
    ...
    if (this.gameRoleSubmitIntervalObj.hasOwnProperty(gameRoleInfo['upload_type']) !== true) {
      ...
      this.gameRoleSubmitIntervalObj[gameRoleInfo['upload_type']] = setInterval(this.roleInterval,
        10000, this, gameRoleInfoStorageName, gameRoleSubmitTimeStorageName);
    } else {
  ...
  roleInfoValidate: function (callback, validateRoleInfo) {
    // 角色上报必传参数
    let validateRoleList = ['game_id', 'user_id', 'cp_server_id', 'cp_server_name', 'cp_role_id',
      'cp_role_name', 'cp_role_level', 'submit_time', 'upload_type'
    ];
  ...
  bindPhoneInGame: function(callback,params){
    if (!this.checkLoginStatus(callback)) {
      this.errorReturn(callback, '调用绑定手机号失败');
      return false;
    }
    this.platformSdk.bindPhoneInGame({
      phone : params.phone,
      sms_code : params.phone,
      success: res => {
        this.successReturn(callback, res);
      },
      fail: res=>{
  ...
  sendSms: function(callback,params){
    if (!this.checkLoginStatus(callback)) {
      this.errorReturn(callback, '调用发送验证码失败');
      return false;
    }
    this.platformSdk.sendSms({
      type: params.type,
      phone: params.phone,
      success: res => {
        this.successReturn(callback, res);
  ...
  customerService: function(callback,params){
    if (!this.checkLoginStatus(callback)) {
      this.errorReturn(callback, '调用客服消息失败');
      return false;
    }
    this.platformSdk.customerService({
      success: res => {
        this.successReturn(callback, res);
      },
      fail: res=>{
  ...
  svipCare: function(callback,params){
    if (!this.checkLoginStatus(callback)) {
      this.errorReturn(callback, '调用svip客服失败');
      return false;
    }
    this.platformSdk.svipCare({
      success: res => {
        this.successReturn(callback, res);
      },
      fail: res=>{
