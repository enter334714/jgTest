...
var config = {
    game_id: 256,
    game_pkg: 'tjqy_tjqyqywxxyxjyzt_TP',
    partner_label: 'qyxyx',
    partner_id: '567',
    game_ver: '66.0.3',
    is_auth: false, //授权登录
    partner_game_id : 1290,
    partner_app_key : '52b8afd081c255a921983dd40d911c8a'
};
    ...
    return {
        ...
        login: function (data,callback) {
            ...
            sdk.initSdk(function(e){
                ...
                if (true === e.status) {
                    ...
                    console.log('sdkInit:success');
                    //登录
                    sdk.login(function(e){
                        if (true === e.status) {    
                            // TODO 成功逻辑    
                            console.log('sdkLogin:success', e.data);  
                            wx.setStorageSync('partner_uid', e.data.user_id);
                            self.do_login(e.data);
                            console.log('渠道登录返回成功'+ JSON.stringify(e.data));
                        } else {    
                            // TODO 失败逻辑    
                            console.log('sdkLogin:error', e.msg);  
                    ...
                    console.log('sdkInit:error', e.msg);
                    callbacks['login'] && callbacks['login'](1, {
                        errMsg: 'sdk初始化失败，重试。'
                    });
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (data.state) {
                            try {
                                    ...
                                    wx.setStorageSync('partner_openid', data.data.openid);
                                    if(data.data.ext){
                        ...
                        self.getShareInfo('menu', function (data) {
                            ...
                            sdk.onShareAppMessage(function(e){
                                if (true === e.status) {    
                                    // TODO 成功逻辑    
                                    console.log('sdkOnShareAppMessage:success');  
                                } else {    
                                    // TODO 失败逻辑    
                                    console.log('sdkOnShareAppMessage:error', e.msg);  
                                }
        ...
        share: function (data) {
            ...
            this.getShareInfo(type, function (data) {
                ...
                sdk.shareAppMessage(function(e){
                  ...
                  if (true === e.status) {
                    console.log('sdkShareAppMessage:success');
                  }else{
                    console.log('sdkShareAppMessage:error', e.msg);
                  }
                },{title:data.title,imageUrl:data.img})
        ...
        openService: function () {
            ...
            sdk.customerService(function (e) {
              if (true === e.status) {
                console.log('customerService:success', e.data);
              }else{
                console.log('customerService:error', e.msg);
              }
            })
        },
            ...
            sdk.msgSecCheck(function(e){
              ...
              if (true === e.status) {
                if(e.data.errno==1){
                  ...
                  console.log('msgSecCheck:success', e.data);
                  ret.statusCode = 200;
                  ret.data.state = 1;

                }else{

                  console.log('msgSecCheck:error', e.msg);
                  ret.statusCode = 200;
                  ret.data.state = 0;
        ...
        startPay: function (data, callback) {
            ...
            var order_data = {
                ...
                platform: platform,
                partner_openid:wx.getStorageSync('partner_openid')
            };
                       ...
                       if (data.state) {
                          ...
                          if(data.data.ext == ""){
                            ...
                            sdk.pay(function (e) {  
                              if (true === e.status) {   
                                // TODO 成功逻辑    
                                console.log('sdkPay:success', e.data);  
                                } else {    
                                  // TODO 失败逻辑    
                                  console.log('sdkPay:error', e.msg);  
                                }
                              }, data.data.pay_data);
                          }else{
        ...
        extDo: function(data){
          wx.navigateToMiniProgram({
              appId: data.ext1,
              path: 'pages/pay/pay?order_id='+data.ext2.extra+'&money='+data.ext2.totalCharge/100,
              extraData: {
        ...
        partner_role_log: function(data,upload_type){
            let roleInfo = {    
                ...
                user_id: wx.getStorageSync('partner_uid'),
                cp_user_id: wx.getStorageSync('plat_uid'),
            ...
            wx.request({
                url: 'https://' + HOST + '/partner/data/getPartnerSign/' + config.partner_id + '/' + config.game_pkg,
                method: 'POST',
                dataType: 'json',
                header: {
                    'content-type': 'application/x-www-form-urlencoded' // 默认值
                ...
                success: function (res) {
                    if(res.data.state==1){
                        roleInfo['sign'] = res.data.data;
                        console.log('sdk.role',roleInfo);
                        sdk.role(function(e){
                            if (true === e.status) {
                                console.log('sdkRole:success ',upload_type);
                            }else{
                                onsole.log('sdkRole:error', upload_type,e.msg);
                            }
                        },roleInfo);
                    }
                }
        ...
        logCreateRole: function (data) {
            ...
            // 渠道上报
            this.partner_role_log(data,'CREATE_ROLE');
        },
        ...
        logEnterGame: function (data) {
            ...
            this.partner_role_log(data,'JOIN_SERVER');
        },
        ...
        logRoleUpLevel: function (data) {
            ...
            this.partner_role_log(data,'UPGRADE');
        },
        ...
        weiduanHelper: function() {
            sdk.bindPhone(function(e){
              if (true === e.status) {
                console.log('sdkOpenQQGroup:success', e.data);
              }else{
                console.log('sdkOpenQQGroup:error', e.msg);
              }
            });
        },
        // 微端小助手(兼容)
        ...
        subscribeMessage : function (tmplIds, callback){
            ...
            sdk.subcribeMessage(function(e){
              ...
              if (true === e.status) {
                console.log('subcribeMessage:success', e.data);
                callbacks['subscribeMessage'] && callbacks['subscribeMessage'](e.data);
              }else{
                console.log('subcribeMessage:error', e.msg);
                callbacks['subscribeMessage'] && callbacks['subscribeMessage'](e.msg);
              }
