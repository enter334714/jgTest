window.pjsdk = function () {
	return {
			...
			window.pjsdk.api(payParam, "/api/getOrderNo", function (data) {
				...
				if (data.data.orderInfo.ptype == 2) {
					...
					wx.requestMidasPayment({
						...
						platform: 'android',
						zoneId: '1',
						buyQuantity: data.data.orderInfo.money, //购买数量。mode=game 时必填。购买数量。详见 buyQuantity 限制说明。   * 
						success: function () {
							...
							console.info("请求payCallBack参数：" + JSON.stringify(payCallBackParm));

							var payCallBackUrl = "/api/payCallBack/" + data.data.orderInfo.channelGameId;
							window.pjsdk.api(payCallBackParm, payCallBackUrl, function (data) {
		...
		initLayaFloat: function (stage) {
			...
			//加载logo
			ape.loadImage(window.pjsdk.logoUrl, Laya.Handler.create(this, function () {
				ape.zOder = 999999;
		...
		openUc: function () {
			wx.navigateToMiniProgram({
				...
				path: 'pages/login/login?fromChannel=' + window.pjsdk.channel + "&fromGameId=" + window.pjsdk.channelGameId + "&fromUid=" + window.pjsdk.uid + "&fromOpenid=" + window.pjsdk.suid + "&fromToken=" + window.pjsdk.token,
				success(res) {},
				fail(res) {},
				complete(res) {}
			});
		...
		openZd: function () {
			wx.navigateToMiniProgram({
				appId: window.pjsdk.ucAppid,
				path: 'pages/login/zd?fromChannel=' + window.pjsdk.channel + "&fromGameId=" + window.pjsdk.channelGameId + "&fromUid=" + window.pjsdk.uid + "&fromOpenid=" + window.pjsdk.suid + "&fromToken=" + window.pjsdk.token,
				success(res) {},
				fail(res) {},
				complete(res) {}
			});
