var A = wx.$N;
import 'afa891/nnnWEs1.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window._nK5Z8W = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/nres/nfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$N = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'nnnnIBS', success: function (r) {
...
wx.onShow(function (res) {
  window._nZ58 = res;
  if (window._nZ85 && window._nZ58) {
    console.info("小游戏切前台事件，场景值：" + window._nZ58.scene);
    window._nZ85(window._nZ58);
    window._nZ58 = null;
  }
});