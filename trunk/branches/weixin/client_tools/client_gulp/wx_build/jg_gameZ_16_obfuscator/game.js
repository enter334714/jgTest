...
render();
window.$LTNDFP = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/z16ttes/z16ttiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.$LPN = res;
  if (window.$LFNP && window.$LPN) {
    console.info("小游戏切前台事件，场景值：" + window.$LPN.scene);
    window.$LFNP(window.$LPN);
    window.$LPN = null;
  }
});