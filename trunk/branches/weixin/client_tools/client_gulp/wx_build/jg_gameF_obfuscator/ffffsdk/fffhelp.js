var k = wx.$f;
let confArr = ['', k[86256], k[86257], k[86258], k[86259], k[86260], k[86261], k[86262], k[86263], k[86264], k[86265], k[86266], k[86267], k[86268], k[86269], k[86270], k[86271], k[86272]];
...
const Sygame = {
  ...
  init: data => {
    ...
    Sygame.offerId = SY_CONF[k[86278]];
    Sygame.scene = data.scene;
    console.log(k[86279], Sygame);
    let queryData = {
      query: data.query
    };
    //获取该游戏是否开启获取剪切板功能
    ...
    wx.onShareAppMessage(() => {
      ...
      console.log(k[86284], shareData);
      var data = {
        title: shareData.title,
        imageUrlId: shareData.imageUrlId,
        imageUrl: shareData.imageUrl
      };
      console.log(k[86285], shareData);
      return data;
    });
  },
  // 游戏登录
            ...
            success(ret) {
              ...
              if (ret.data.code == 1001) {
                ...
                if (ret.data.jump_mandatory == 1) {
                  ...
                  // 判断用户点击取消次数
                  var loginKey = k[86288] + ret.data.openid;
                ...
                wx.showModal({
                  title: ret.data.jump_title_tip ? ret.data.jump_title_tip : k[86289],
                    ...
                    if (res.cancel) {
                      ...
                      if (ret.data.jump_to) {
                        ...
                        wx.onTouchStart(() => {
                          wx.showModal({
                            title: k[86289],
                            content: ret.data.jump_tip ? ret.data.jump_tip : k[86290],
                            confirmText: k[68553],
                            showCancel: false,
                            success: () => {
                              wx.previewImage({
                        ...
                        wx.setClipboardData({
                          ...
                          success(res) {
                            console.log(k[86291], ret.data); // data
                          }
                        });
                        wx.onTouchStart(() => {
                          wx.showModal({
                        ...
                        wx.onTouchStart(() => {
                          wx.openCustomerServiceConversation({
                            sessionFrom: k[86292] + Sygame.appid,
                            showMessageCard: true,
                            sendMessageImg: k[86293],
                            success: () => {
                              console.log(k[68940]);
                            }
                          });
                ...
                wx.showModal({
                  title: k[86294],
                  content: ret.data.game_tip ? ret.data.game_tip : k[86295],
                  confirmText: k[68553],
                  showCancel: false,
                  success: () => {
                    console.log(k[86296], ret);
                  }
                });
                wx.onTouchStart(() => {
                  wx.showModal({
  ...
  syReportRoleInfo: data => new Promise(function (resolve, reject) {
    if (typeof data === k[60272]) {
      let url = confArr[3];
      if (data.role_id && typeof data.role_id !== k[86125]) Sygame.role_id = data.role_id;
      if (data.role_name && typeof data.role_name !== k[86125]) Sygame.role_name = data.role_name;
      if (data.server_id && typeof data.server_id !== k[86125]) Sygame.server_id = data.server_id;
      if (data.server_name && typeof data.server_name !== k[86125]) Sygame.server_name = data.server_name;
            ...
            case "2":
              wx.showModal({
                ...
                success: ret => {
                  if (ret.confirm) {
                    wx.openCustomerServiceConversation({
                      sessionFrom: k[86305] + res.data.payId,
                      showMessageCard: true,
                      sendMessageImg: k[86306],
                      success: () => {
                        console.log(k[68940]);
                      }
                    });
  ...
  syMidasPay: data => {
    wx.requestMidasPayment({
      ...
      currencyType: k[86311],
      buyQuantity: data.product_price * 100,
      platform: k[86312],
      success(res) {
        Sygame.syDescMidasCoin(data);
        console.log(k[86313], res);
      },
      fail(res) {
        console.log(res);
      },
  ...
  syGetSubscribe: data => new Promise(function (reslove, reject) {
    wx.requestSubscribeMessage({
      ...
      success: res => {
        console.log(k[86316], res);
        let type = '';
        if (res[data.template] === k[86317]) {
          type = k[86318];
        } else {
          type = k[60526];
        }
  ...
  syClickOpenBox: () => new Promise(function (reslove, reject) {
    // 判断用户是否点击
    var clickOpenBox = k[86320] + Sygame.openid;
  ...
  goShareData: params => {
    ...
    var ranIndex = Math.floor(Math.random() * len);
    var shareData = arr[ranIndex];
    console.log(k[86326], shareData);
    var data = {
      title: shareData.title,
      imageUrlId: shareData.imageUrlId,
      imageUrl: shareData.imageUrl,
      query: params.shareQuery + k[86327] + shareData.id
    };
    console.log(k[86324], shareData);
   ...
   */
  upShareData: params => {
    var key = k[86328] + params.material_id;
   ...
   */
  upClickData: params => {
    var key = k[86329] + params.material_id;
    var log = Sygame.cookieData({ type: k[60437], 'key': key }) ? 1 : 0; //1-已记录，0-未记录
    if (log == 0) Sygame.cookieData({ type: k[60019], key: key, data: new Date().getTime() });
    params.log = log;
    wx.request({
