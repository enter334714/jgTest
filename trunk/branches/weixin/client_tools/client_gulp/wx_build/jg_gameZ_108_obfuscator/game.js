var _j = wx.n$;
import 'lz6s/a46oub.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/ftbbv.png";
...
render();
window.y$HIX5T = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/kluma/a6v.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.y$TI = res;
  if (window.y$5IT && window.y$TI) {
    console.info("小游戏切前台事件，场景值：" + window.y$TI.scene);
    window.y$5IT(window.y$TI);
    window.y$TI = null;
  }
});