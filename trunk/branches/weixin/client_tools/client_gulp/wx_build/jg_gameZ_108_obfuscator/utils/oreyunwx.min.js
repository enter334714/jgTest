var _j = wx.n$;
var TKIO_CONFIG = { api_base: "https://log.reyun.com/receive/tkio/", prefix: "rywx_", version: "v1.0.4" };function getID(e) {
  try {
  ...
  setInterval(function () {
        ...
        var t = e.networkType;if (t != "none") {
          var n = getID("requests");if (n) {
            for (var i in n) {
              ...
              }n[i].params.appid = getID("appid");setID("requests", n);TKIO.send(n[i].what, n[i].params, n[i]);
            }
          }
        }
      } });
    ...
    setID("apptype", "wg");wx.getLaunchOptionsSync(function (e) {
      ...
      var t = e.query;if (JSON.stringify(t) !== "{}") {
        var n = {};for (var i in t) {
          var a = i.replace("?", "");n[a] = t[i];
        }setID("querystring", n);
      }if (!getID("campaignid")) {
        setID("campaignid", e.query.surl ? e.query.surl : "_default_");
      }if (!getID("cid")) {
        setID("cid", e.query.cid ? parseInt(e.query.cid) : -1);
      }setID("starttime", Date.now());setID("sceneid", e.scene);setTimeout(function () {
        TKIO.launch();
      }, 2e3);
    });return;
  }var e = App,
...
}();var TKIO = { init: function (e, t, n) {
    setID("appid", e);setID("openid", t);setID("params", JSON.stringify(n));if (!t) {
      this.getOpenId(e);
    }
    ...
    var e = {};var t = getID("params");if (t) {
      e = JSON.parse(t);
    }var n = getID("querystring");var i = Object.assign(n, e);if (i) {
      var a = this.getCommonParams(i);
    } else {
      var a = this.getCommonParams();
    }a.what = "launch";this.send("event", a);
  }, hide: function (e) {
    ...
    if (!getID("appid")) return;var r = this;wx.request({ url: TKIO_CONFIG.api_base + i, method: "POST", data: JSON.stringify(a), header: { "content-type": "application/x-www-form-urlencoded; charset=UTF-8" }, success: function (e) {
        if (e.data.status != 0) {
          if (!s && e.data.result && !e.data.result.appid) r.cacheRequsets(i, a);
        } else {
