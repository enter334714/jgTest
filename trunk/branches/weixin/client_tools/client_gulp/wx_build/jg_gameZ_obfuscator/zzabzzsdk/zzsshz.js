...
// 地址
let api = B[441725];
let wanBaApi = B[441726];
let logApi = B[441727];
let wxRedApi = B[441728];

const Tools = {
  getDeviceType: () => {
    const systemInfo = wx.getSystemInfoSync();
    let osType;
    if (systemInfo.system.toLowerCase().indexOf(B[441284]) > -1) {
      osType = B[441284];
    } else if (systemInfo.system.toLowerCase().indexOf(B[441283]) > -1) {
      osType = B[441283];
  ...
  buildActiveParams: (params, gameKey) => {
    const keys = [B[441731], B[441732], B[441733], B[440910], B[441734], B[441049]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
    signString = signString + gameKey;
    params[B[441047]] = md5(signString);
    return params;
  },
  buildChangeShell: (params, gameKey) => {
    const keys = [B[441731], B[441735], B[441049]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
  ...
  buildLoginParams: params => {
    const keys = [B[441736], B[441731], B[441733], B[441732], B[441049]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildBindParams: params => {
    const keys = [B[441731], B[441737], B[441738], B[441049]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildPayParams: params => {
    const keys = [B[441731], B[441736], B[441733], B[441739], B[441740], B[441049]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildRepublish: params => {
    const keys = [B[441739], B[441731], B[441736], B[441733], B[441049]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildCheckMsg(params) {
    const keys = [B[441731], B[441733], B[441736], B[441049]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
    ...
    signString = signString + commonParams.gameKey;
    params[B[441047]] = md5(signString).toLowerCase();
    return params;
  },
  ...
  buildRedParams: params => {
    ...
    stringParams += B[441532] + "=" + commonParams.gameKey;
    params[B[441047]] = md5(stringParams);
    return params;
  },
  initLock: locks => {
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.data.code == 1) {
        ...
        let orderList = [];
        wx.setStorageSync(B[441749], orderList);
        wxLogin(SDKyyw);
      } else {
        state.initStatus = B[441750];
        console.log(B[441751], B[441752] + gameId, B[441753] + gameKey);
    ...
    fail: err => {
      console.log(B[441754]);
      if (commonParams.initTimer) {
        clearTimeout(commonParams.initTimer);
      }
      wx.request({
        url: B[441755] + B[441756],
        timeout: 5000,
        success: res => {
          res = res.data;
          if (res.status === 1) {
            console.log(B[441757], res.data.host);
  ...
  wx.login({
    success: res => {
      if (res.code == null || res.code == undefined || res.code == "") {
        ...
        wx.getSetting({
          complete(com) {
            if (com.authSetting[B[441759]]) {
              wx.getUserInfo({
                success(res) {
                  console.log(B[441760], res);
        ...
        let params = {
          ...
          code: jscode,
          iv: commonParams.iv,
          osType: Tools.getDeviceType() == B[441283] ? "2" : Tools.getDeviceType() == B[441284] ? "1" : Tools.getDeviceType() == B[441729] ? "3" : B[441730],
        ...
        console.log(B[441761], params);
        sdkLogin(SDKyyw, params);
      }
    },
    fail: err => {
      wxLogin(SDKyyw);
      console.log(B[441762], err);
    }
  });
};
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        console.log(B[441764], commonParams.referrerInfo);
        ...
        console.log(B[441766], ret);
        // SDK登录回调
        SDKyyw.initLoginCallback(ret);
      } else {
        console.log(B[441767], res);
...
const cutGame = function () {
  ...
  if (typeof SDKyyw.cutGameCallback != B[440130]) {
    ...
    wx.navigateToMiniProgram({
      ...
      envVersion: commonParams.switchEnv || B[441770], // develop trial release
      success(res) {
        console.log(B[441771], res);
...
const getCaptcha = function (data, sucCallback, errorCallback) {
  ...
  if (typeof sucCallback == B[440130]) {
    ...
    wx.request({
      url: wanBaApi + B[441775],
      data: params,
      timeout: 3000,
      success: res => {
        if (res.data.code === 18000) {
          console.log(B[441776], res);
          sucCallback(res);
        } else {
          console.log(B[441777], res);
          errorCallback(res);
        }
      },
      error: err => {
        console.log(B[441778], err);
      }
    });
  } else {
...
const replenish = function () {
  let orderList = wx.getStorageSync(B[441749]);
...
const openData = function (data) {
  ...
  if (typeof SDKyyw.openDataCallback != B[440130]) {
    console.error(B[441811]);
  } else {
    if (data.type == B[441812]) {
      //好友感兴趣
...
const pay = function (payData) {
  ...
  if (typeof this.onPayCallback != B[440130]) {
    console.error(B[441818]);
  } else {
    console.log(B[441819], commonParams);
    const params = {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success: payRes => {
            ...
            console.log(B[441822], payRes.data);
            wx.hideLoading();
            let ret = payRes.data;
              ...
              switch (commonParams.payType) {
                  ...
                  wx.setClipboardData({
                    ...
                    success(res) {
                      wx.showModal({
                        ...
                        content: B[441825],
                        showCancel: false,
                        confirmText: B[441826],
                        confirmColor: B[441827]
                      });
                    }
                  });
                  break;
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    extraData: Minidata,
                    success: function (res) {
                      console.log(B[441834], res);
                    },
                    fail: function (err) {
                      console.log(B[441835], err);
                      wx.request({
                        url: B[441836] + commonParams.gameId + B[441837] + err.errMsg,
                        success(res) {
                          console.log(B[441838], res);
                        }
                      });
                    },
                    complete: function (com) {
                      console.log(B[441839], com);
                    }
                  });
                  break;
              }
      ...
      success(res) {
        if (res.code) {
          ...
          if (!httpLock.httpRedLoginFlag) {
            ...
            wx.request({
              url: wxRedApi + B[441847],
              method: B[441848],
              data: Tools.buildRedParams(params),
              header: {
                "Content-Type": B[441142]
              },
              success(res) {
                let ret = res.data;
                if (ret.status == 200) {
          ...
          console.log(B[441850] + res.errMsg);
        }
      }
    });
  }
...
const toRedShare = function (data) {
  ...
  wx.request({
    url: wxRedApi + B[441864],
    data: Tools.buildRedParams(params),
    success(res) {
      let ret = res.data;
      console.log(B[441865], ret);
      if (ret.status == 200) {
        wx.shareAppMessage({
    ...
    success: res => {
      if (res.code) {
        ...
        wx.request({
          ...
          success(res) {
            let ret = res.data;
            console.log(B[441869], ret.data, ret.msg);
          },
          fail(err) {
            console.log(B[441870], err);
          },
          timeout: 4000
        });
      } else {
        console.log(B[441758], res);
      }
    }
  });
};
...
const getSquareList = function (data) {
  ...
  if (typeof SDKyyw.getSquareListCallback != B[440130]) {
    ...
    if (!httpLock.httpSquareListFlag) {
      ...
      wx.request({
        url: wxRedApi + B[441876],
        data: Tools.buildRedParams(params),
        success(res) {
          let ret = res.data;
          console.log(B[441877], ret);
...
const squareBind = function (data) {
  wx.login({
    success(ret) {
      ...
      console.log(B[441880], params);
      wx.request({
        url: wxRedApi + B[441881],
        data: params,
        timeout: 5000,
        success: res => {
...
const showAppModal = function () {
  if (commonParams.showAppOn) {
    wx.showModal({
      ...
      content: commonParams.appContent || B[441889],
      cancelText: B[441890],
      confirmText: B[441891],
      success(res) {
        if (res.confirm) {
          wx.openCustomerServiceConversation({
            showMessageCard: true,
...
const msgSecCheck = function (msg, callback) {
  const msgIsString = Object.prototype.toString.call(msg).indexOf(B[441893]) > -1;
  const msgIsObject = Object.prototype.toString.call(msg).indexOf(B[441894]) > -1;
  var params = {};
  if (msgIsString) {
    params = {
      gameid: commonParams.gameId,
  ...
  startGameLive() {
    ...
    wx.startGameLive({
      success(res) {
        ...
        console.log(B[441901], res);
      },
      fail(err) {
        console.error(B[441901], err);
        SDKyyw.startGameLiveCallback({
          status: "0",
          msg: "",
          data: err
                      ...
                      success: res => {
                        if (res.code == null || res.code == undefined || res.code == "") {
                          ...
                          wx.request({
                            ...
                            data: Tools.buildRedParams(params),
                            success(res) {
                              console.log(B[441908], res);
                            },
                            fail(err) {
                              console.log(B[441909], err);
                            },
                            timeout: 10 * 1000
                          });
                        }
...
const pushData = function (data) {
  ...
  if (data.type == "1") {
    //登录
    data.logType = B[441024];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[B[441933]];
    delete data[B[441934]];
    //accountId serverId
  } else if (data.type == "2") {
    //创建角色或者账号
    if (!data.roleId) {
    ...
    if (!data.roleName) {
      console.log(B[441936]);
    }

    data.logType = B[441937];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[B[441933]];
    delete data[B[441934]];
    ...
    data.logType = B[441938]; //升级
    data.region = "0";
    data.levelType = "1";
    data.duration = "0";
    data.channelAccid = commonParams.uid;
    delete data[B[441389]];
    //accountId00  roleID00   roleName00 level00 vipLevel00   startLevel00 endLevel00
  } else if (data.type == "5") {
    data.logType = B[441939]; //在线
    delete data[B[441940]];
    delete data[B[441941]];
    delete data[B[440910]];
    delete data[B[441942]];
    delete data[B[441943]];
    delete data[B[441944]];
    delete data[B[441945]];
    //accountId serverId  roleID  roleName
  } else if (data.type == 6) {
    // 分享
    data.logType = B[441098];
    delete data[B[441940]];
    delete data[B[441941]];
    delete data[B[440910]];
    delete data[B[441942]];
    ...
    // 自定义事件
    data.logType = B[441456];
    delete data[B[441940]];
    delete data[B[441941]];
    delete data[B[441946]];
    delete data[B[440910]];
    delete data[B[441942]];
    delete data[B[441943]];
    delete data[B[441944]];
    ...
    data.logType = B[441947]; // 游戏关卡
    data.channelAccid = commonParams.uid;
  } else if (data.type == 11) {
    data.logType = B[441948]; // 广告
    data.channelAccid = commonParams.uid;
  }
...
function setLog(str, level) {
  if (state.debug == true) {
    if (level == state.level.error) {
      console.log(B[441950] + str, B[441951]);
    } else if (level == state.level.warn) {
      console.log(B[441952] + str, B[441953]);
    } else if (level == state.level.debug) {
      console.log(B[441954] + str, B[441953]);
    } else {
      console.log(B[441955] + str, B[441953]);
    }
  }
}
