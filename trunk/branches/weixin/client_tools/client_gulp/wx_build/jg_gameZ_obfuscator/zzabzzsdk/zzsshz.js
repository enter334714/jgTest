...
// 地址
let api = B[441780];
let wanBaApi = B[441781];
let logApi = B[441782];
let wxRedApi = B[441783];

const Tools = {
  getDeviceType: () => {
    const systemInfo = wx.getSystemInfoSync();
    let osType;
    if (systemInfo.system.toLowerCase().indexOf(B[440412]) > -1) {
      osType = B[440412];
    } else if (systemInfo.system.toLowerCase().indexOf(B[440411]) > -1) {
      osType = B[440411];
    } else if (systemInfo.system.toLowerCase().indexOf(B[441784]) > -1) {
      osType = B[441784];
    } else {
      osType = B[441785];
    }
  ...
  buildActiveParams: (params, gameKey) => {
    const keys = [B[441786], B[441787], B[441788], B[440012], B[441789], B[440162]];
    let signString = "";
    for (let k in params) {
  ...
  buildChangeShell: (params, gameKey) => {
    const keys = [B[441786], B[441790], B[440162]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
  ...
  buildLoginParams: params => {
    const keys = [B[441791], B[441786], B[441788], B[441787], B[440162]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildBindParams: params => {
    const keys = [B[441786], B[441792], B[441793], B[440162]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildPayParams: params => {
    const keys = [B[441786], B[441791], B[441788], B[441794], B[441795], B[440162]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildRepublish: params => {
    const keys = [B[441794], B[441786], B[441791], B[441788], B[440162]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildCheckMsg(params) {
    const keys = [B[441786], B[441788], B[441791], B[440162]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  getDeviceTypeId: () => {
    ...
    let deviceType = B[441785];
    if (osType == B[440411]) {
      deviceType = 1;
    } else if (osType == B[440412]) {
      deviceType = 2;
    } else if (osType == B[441784]) {
      deviceType = 3;
    }
    return deviceType;
  },
  ...
  buildRedParams: params => {
    ...
    stringParams += B[440718] + "=" + commonParams.gameKey;
...
const init = function (gameId, gameKey) {
  ...
  if (typeof SDKyyw.initLoginCallback != B[440935]) {
    ...
    console.log(B[441797], accountInfo);
    console.log(B[441798], accountInfo.miniProgram.envVersion);
    ...
    console.log(B[441799], launchOption);
...
const active = function (SDKyyw, params, gameId, gameKey) {
  wx.request({
    ...
    success: res => {
      ...
      if (ret.data.code == 1) {
        ...
        if (commonParams.gameId) {
          let data = {};
          data.logType = B[441801];
          data.appId = commonParams.gameId;
          data.appVersion = B[441802];
          data.platform = B[441803];
        ...
        let orderList = [];
        wx.setStorageSync(B[441804], orderList);
        wxLogin(SDKyyw);
      } else {
        state.initStatus = B[441805];
        console.log(B[441806], B[441807] + gameId, B[441808] + gameKey);
    ...
    fail: err => {
      console.log(B[441809]);
      if (commonParams.initTimer) {
        clearTimeout(commonParams.initTimer);
      }
      wx.request({
        url: B[441810] + B[441811],
        timeout: 5000,
        success: res => {
          res = res.data;
          if (res.status === 1) {
            console.log(B[441812], res.data.host);
    ...
    success: res => {
      if (res.code == null || res.code == undefined || res.code == "") {
        ...
        let params = {
          ...
          code: jscode,
          iv: commonParams.iv,
          osType: Tools.getDeviceType() == B[440411] ? "2" : Tools.getDeviceType() == B[440412] ? "1" : Tools.getDeviceType() == B[441784] ? "3" : B[441785],
        ...
        console.log(B[441816], params);
        sdkLogin(SDKyyw, params);
      }
    },
    fail: err => {
      wxLogin(SDKyyw);
      console.log(B[441817], err);
    }
  });
};
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        console.log(B[441819], commonParams.referrerInfo);
        ...
        console.log(B[441794], ret.data.uid);
        console.log(B[441821], ret);
        // SDK登录回调
        SDKyyw.initLoginCallback(ret);
      } else {
        console.log(B[441822], res);
...
const cutGame = function () {
  ...
  if (typeof SDKyyw.cutGameCallback != B[440935]) {
    ...
    wx.navigateToMiniProgram({
      ...
      envVersion: commonParams.switchEnv || B[441825], // develop trial release
      success(res) {
        console.log(B[441826], res);
...
const replenish = function () {
  let orderList = wx.getStorageSync(B[441804]);
  ...
  if (data.type == B[441839]) {
    ...
    if (typeof SDKyyw.createGridAdCallback != B[440935]) {
      ...
      state.gridAd.onError(err => {
        ...
        console.log(B[441861], err);
      });
    }
  } else if (data.type == B[441862]) {
    if (typeof SDKyyw.createCustomAdCallback != B[440935]) {
      console.log(B[441863]);
    } else {
      // 原生模板广告
      adData.adType = 5;
...
const openData = function (data) {
  ...
  if (typeof SDKyyw.openDataCallback != B[440935]) {
    console.error(B[441866]);
  } else {
    if (data.type == B[441867]) {
      //好友感兴趣
...
const pay = function (payData) {
  ...
  if (typeof this.onPayCallback != B[440935]) {
    console.error(B[441873]);
  } else {
    console.log(B[441874], commonParams);
    const params = {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success: payRes => {
            ...
            console.log(B[441877], payRes.data);
            wx.hideLoading();
            let ret = payRes.data;
              ...
              switch (commonParams.payType) {
                  ...
                  wx.setClipboardData({
                    ...
                    success(res) {
                      wx.showModal({
                        ...
                        content: B[441880],
                        showCancel: false,
                        confirmText: B[441881],
                        confirmColor: B[441882]
                      });
                    }
                  });
                  break;
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    extraData: Minidata,
                    success: function (res) {
                      console.log(B[441889], res);
                    },
                    fail: function (err) {
                      console.log(B[441890], err);
                      wx.request({
                        url: B[441891] + commonParams.gameId + B[441892] + err.errMsg,
                        success(res) {
                          console.log(B[441893], res);
                        }
                      });
                    },
                    complete: function (com) {
                      console.log(B[441894], com);
                    }
                  });
                  break;
              }
...
const getRedPagState = function (data) {
  ...
  if (typeof SDKyyw.getRedPagStateCallback != B[440935]) {
    console.log(B[441898]);
  } else {
    if (commonParams.showRedPagOn) {
      wx.request({
        url: wxRedApi + B[441899],
        data: {
          app_id: commonParams.gameId,
          open_id: commonParams.uid,
    ...
    wx.login({
      success(res) {
        if (res.code) {
          ...
          console.log(B[441905] + res.errMsg);
        }
      }
    });
  }
...
const toRedShare = function (data) {
  ...
  wx.request({
    url: wxRedApi + B[441919],
    data: Tools.buildRedParams(params),
    success(res) {
      let ret = res.data;
      console.log(B[441920], ret);
      if (ret.status == 200) {
        wx.shareAppMessage({
  ...
  wx.login({
    success: res => {
      if (res.code) {
        ...
        wx.request({
          url: wxRedApi + B[441923],
          data: Tools.buildRedParams(params),
          success(res) {
            let ret = res.data;
            console.log(B[441924], ret.data, ret.msg);
          },
          fail(err) {
            console.log(B[441925], err);
          },
          timeout: 4000
        });
      } else {
        console.log(B[441813], res);
      }
    }
  });
};
...
const getSquareList = function (data) {
  ...
  if (typeof SDKyyw.getSquareListCallback != B[440935]) {
    ...
    if (!httpLock.httpSquareListFlag) {
      ...
      wx.request({
        url: wxRedApi + B[441931],
        data: Tools.buildRedParams(params),
        success(res) {
          let ret = res.data;
          console.log(B[441932], ret);
...
const squareClick = function (data) {
  if (!httpLock.httpSquareClickFlag) {
    ...
    wx.request({
      url: wxRedApi + B[441933],
      data: Tools.buildRedParams(params),
      success(res) {
        console.log(B[441934], res);
        wx.navigateToMiniProgram({
          appId: data.wxapp_id,
          extraData: {
            square: {
...
const squareBind = function (data) {
  wx.login({
    success(ret) {
      ...
      console.log(B[441935], params);
      wx.request({
        url: wxRedApi + B[441936],
        data: params,
        timeout: 5000,
        success: res => {
          console.log(B[441937], res);
        }
      });
    }
  });
...
const showAppModal = function () {
  if (commonParams.showAppOn) {
    wx.showModal({
      ...
      content: commonParams.appContent || B[441944],
      cancelText: B[441945],
      confirmText: B[441946],
      success(res) {
        if (res.confirm) {
          wx.openCustomerServiceConversation({
            showMessageCard: true,
...
const msgSecCheck = function (msg, callback) {
  const msgIsString = Object.prototype.toString.call(msg).indexOf(B[441948]) > -1;
  const msgIsObject = Object.prototype.toString.call(msg).indexOf(B[441949]) > -1;
  var params = {};
  if (msgIsString) {
    params = {
      gameid: commonParams.gameId,
...
const gameLive = {
  ...
  startGameLive() {
    ...
    wx.startGameLive({
      success(res) {
        ...
        console.log(B[441956], res);
      },
      fail(err) {
        console.error(B[441956], err);
        SDKyyw.startGameLiveCallback({
          status: "0",
          msg: "",
          data: err
                    ...
                    wx.login({
                      success: res => {
                        if (res.code == null || res.code == undefined || res.code == "") {
                          ...
                          params.code = res.code;
                          wx.request({
                            url: wxRedApi + B[441962],
                            method: B[441903],
                            data: Tools.buildRedParams(params),
                            success(res) {
                              console.log(B[441963], res);
                            },
                            fail(err) {
                              console.log(B[441964], err);
                            },
                            timeout: 10 * 1000
                          });
                        }
  ...
  getUserCurrentGameliveInfo() {
    ...
    wx.getUserCurrentGameliveInfo({
      success(res) {
        ...
        console.log(B[441968], res);
      },
      fail(err) {
        console.log(B[441968], err);
  ...
  getUserGameLiveDetails(data = {}) {
    ...
    if (!data.feedIdList) {
      console.log(B[441972]);
      return;
    }
    wx.getUserGameLiveDetails({
      feedIdList: data.feedIdList,
      success(res) {
        console.log(B[441973], res);
...
const pushData = function (data) {
  ...
  if (data.type == "1") {
    ...
    if (!data.roleName) {
      console.log(B[441991]);
    }

    data.logType = B[441992];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[B[441988]];
    delete data[B[441989]];
    ...
    // 自定义事件
    data.logType = B[440607];
    delete data[B[441995]];
    delete data[B[441996]];
    delete data[B[442001]];
    delete data[B[440012]];
    delete data[B[441997]];
    delete data[B[441998]];
    delete data[B[441999]];
    delete data[B[442000]];
    delete data[B[441789]];
  } else if (data.type == 10) {
    data.logType = B[442002]; // 游戏关卡
    data.channelAccid = commonParams.uid;
  } else if (data.type == 11) {
    data.logType = B[442003]; // 广告
    data.channelAccid = commonParams.uid;
  }
...
function setLog(str, level) {
  if (state.debug == true) {
    if (level == state.level.error) {
      console.log(B[442005] + str, B[442006]);
    } else if (level == state.level.warn) {
      console.log(B[442007] + str, B[442008]);
    } else if (level == state.level.debug) {
      console.log(B[442009] + str, B[442008]);
    } else {
      console.log(B[442010] + str, B[442008]);
    }
  }
}
