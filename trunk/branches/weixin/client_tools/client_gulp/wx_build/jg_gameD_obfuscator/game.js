var v = wx.$d;
import 'ddssss/dddweasaf.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window.D$YSLZA = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/dres/dfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'dddlibs', success: function (r) {
...
wx.onShow(function (res) {
  window.D$AS = res;
  if (window.D$ZSA && window.D$AS) {
    console.info("小游戏切前台事件，场景值：" + window.D$AS.scene);
    window.D$ZSA(window.D$AS);
    window.D$AS = null;
  }
});