...
// 地址
let api = v[0x7514];
let wanBaApi = v[0x7515];
let logApi = v[0x7516];
let wxRedApi = v[0x7517];

const Tools = {
  getDeviceType: () => {
    const systemInfo = wx.getSystemInfoSync();
    let osType;
    if (systemInfo.system.toLowerCase().indexOf(v[0x7518]) > -1) {
      osType = v[0x7518];
    } else if (systemInfo.system.toLowerCase().indexOf(v[0x7519]) > -1) {
      osType = v[0x7519];
    } else if (systemInfo.system.toLowerCase().indexOf(v[0x1ee5]) > -1) {
      osType = v[0x1ee5];
    } else {
      osType = v[0x751a];
    }
  ...
  buildActiveParams: (params, gameKey) => {
    const keys = [v[0x751b], v[0x751c], v[0x751d], v[0x751e], v[0x751f], v[0x35a]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
    signString = signString + gameKey;
    params[v[0x2dbe]] = md5(signString);
    return params;
  },
  buildChangeShell: (params, gameKey) => {
    const keys = [v[0x751b], v[0x7520], v[0x35a]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
  ...
  buildLoginParams: params => {
    const keys = [v[0x7521], v[0x751b], v[0x751d], v[0x751c], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildBindParams: params => {
    const keys = [v[0x751b], v[0x7522], v[0x7523], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildPayParams: params => {
    const keys = [v[0x751b], v[0x7521], v[0x751d], v[0x1ea3], v[0x4ab1], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildRepublish: params => {
    const keys = [v[0x1ea3], v[0x751b], v[0x7521], v[0x751d], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildCheckMsg(params) {
    const keys = [v[0x751b], v[0x751d], v[0x7521], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
    ...
    signString = signString + commonParams.gameKey;
    params[v[0x2dbe]] = md5(signString).toLowerCase();
    return params;
  },

  getDeviceTypeId: () => {
    const osType = Tools.getDeviceType();
    let deviceType = v[0x751a];
    if (osType == v[0x7519]) {
      deviceType = 1;
    } else if (osType == v[0x7518]) {
      deviceType = 2;
    } else if (osType == v[0x1ee5]) {
      deviceType = 3;
    }
    return deviceType;
  },
  ...
  buildRedParams: params => {
    ...
    stringParams += v[0x109] + "=" + commonParams.gameKey;
    params[v[0x2dbe]] = md5(stringParams);
    return params;
  },
  initLock: locks => {
...
const init = function (gameId, gameKey) {
  ...
  if (typeof SDKyyw.initLoginCallback != v[0x74ba]) {
    console.error(v[0x7524]);
  } else {
    ...
    console.log(v[0x7525], accountInfo);
    console.log(v[0x7526], accountInfo.miniProgram.envVersion);
    ...
    console.log(v[0x7527], launchOption);
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.data.code == 1) {
        ...
        commonParams.gameId = gameId;
        commonParams.gameKey = gameKey;
        state.initStatus = v[0x2785];

        if (commonParams.gameId) {
          let data = {};
          data.logType = v[0x234];
          data.appId = commonParams.gameId;
          data.appVersion = v[0x7529];
          data.platform = v[0x752a];
        ...
        let orderList = [];
        wx.setStorageSync(v[0x752b], orderList);
        wxLogin(SDKyyw);
      } else {
        state.initStatus = v[0x663a];
        console.log(v[0x752c], v[0x752d] + gameId, v[0x752e] + gameKey);
    ...
    fail: err => {
      console.log(v[0x752f]);
      if (commonParams.initTimer) {
        clearTimeout(commonParams.initTimer);
      }
      wx.request({
        url: v[0x7530] + v[0x7531],
        timeout: 5000,
        success: res => {
          res = res.data;
          if (res.status === 1) {
            console.log(v[0x7532], res.data.host);
  ...
  wx.login({
    success: res => {
      if (res.code == null || res.code == undefined || res.code == "") {
        ...
        wx.getSetting({
          complete(com) {
            if (com.authSetting[v[0x7534]]) {
              wx.getUserInfo({
                success(res) {
                  console.log(v[0x137], res);
        ...
        let params = {
          ...
          code: jscode,
          iv: commonParams.iv,
          osType: Tools.getDeviceType() == v[0x7519] ? "2" : Tools.getDeviceType() == v[0x7518] ? "1" : Tools.getDeviceType() == v[0x1ee5] ? "3" : v[0x751a],
        ...
        console.log(v[0x7535], params);
        sdkLogin(SDKyyw, params);
      }
    },
    fail: err => {
      wxLogin(SDKyyw);
      console.log(v[0x7536], err);
    }
  });
};
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        console.log(v[0x7538], commonParams.referrerInfo);
        ...
        console.log(v[0x1ea3], ret.data.uid);
        console.log(v[0x753a], ret);
        // SDK登录回调
        SDKyyw.initLoginCallback(ret);
      } else {
        console.log(v[0x753b], res);
...
const cutGame = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.cutGameCallback != v[0x74ba]) {
    console.log(v[0x753d]);
  } else {
    wx.navigateToMiniProgram({
      ...
      envVersion: commonParams.switchEnv || v[0x753e], // develop trial release
      success(res) {
        console.log(v[0x753f], res);
...
const getCaptcha = function (data, sucCallback, errorCallback) {
  const SDKyyw = this;
  if (typeof sucCallback == v[0x74ba]) {
    ...
    wx.request({
      url: wanBaApi + v[0x7543],
      data: params,
      timeout: 3000,
      success: res => {
        if (res.data.code === 18000) {
          console.log(v[0x7544], res);
          sucCallback(res);
        } else {
          console.log(v[0x7545], res);
          errorCallback(res);
        }
      },
      error: err => {
        console.log(v[0x7546], err);
      }
    });
  } else {
    if (typeof SDKyyw.getCaptchaCallback != v[0x74ba]) {
      console.log(v[0x7547]);
    } else {
      let params = Tools.buildParams({
        yy_pf: commonParams.yy_pf,
        app_id: commonParams.gameId,
...
const bindTelephone = function (data, smsCode, sucCallback, errorCallback) {
  ...
  if (typeof sucCallback == v[0x74ba]) {
    ...
    if (typeof SDKyyw.bindTelephoneCallback != v[0x74ba]) {
      console.log(v[0x7549]);
    } else {
      let params = Tools.buildParams({
        yy_pf: commonParams.yy_pf,
        app_id: commonParams.gameId,
...
const replenish = function () {
  let orderList = wx.getStorageSync(v[0x752b]);
...
const subscribeMessage = function (tmplIds) {
  const SDKyyw = this;
  if (typeof SDKyyw.subscribeMsgCallback != v[0x74ba]) {
    console.log(v[0x754b]);
  } else {
...
const advertisement = function (data) {
  ...
  if (data.type == v[0x754c]) {
    if (typeof SDKyyw.createBannerAdCallback != v[0x74ba]) {
      console.log(v[0x754d]);
    } else {
  ...
  } else if (data.type == v[0x7551]) {
    if (typeof SDKyyw.rewardedVideoAdCallback != v[0x74ba]) {
      console.log(v[0x7552]);
    } else {
      adData.adType = 2;
      // 创建激励视频广告实例，提前初始化
  ...
  } else if (data.type == v[0x7557]) {
    if (typeof SDKyyw.createInterstitialAdCallback != v[0x74ba]) {
      console.log(v[0x7558]);
    } else {
      adData.adType = 3;
      // 创建插屏广告实例，提前初始化
  ...
  } else if (data.type == v[0x755c]) {
    if (typeof SDKyyw.createGridAdCallback != v[0x74ba]) {
      console.log(v[0x755d]);
    } else {
      // 格子广告
      adData.adType = 4;
      ...
      state.gridAd.onError(err => {
        ...
        console.log(v[0x7561], err);
      });
    }
  } else if (data.type == v[0x7562]) {
    if (typeof SDKyyw.createCustomAdCallback != v[0x74ba]) {
      console.log(v[0x7563]);
    } else {
      // 原生模板广告
      adData.adType = 5;
...
const openData = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.openDataCallback != v[0x74ba]) {
    console.error(v[0x7566]);
  } else {
    if (data.type == v[0x7567]) {
      //好友感兴趣
...
const pay = function (payData) {
  ...
  if (typeof this.onPayCallback != v[0x74ba]) {
    console.error(v[0x756d]);
  } else {
    console.log(v[0x756e], commonParams);
    const params = {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success: payRes => {
            ...
            console.log(v[0x7572], payRes.data);
            wx.hideLoading();
            let ret = payRes.data;
              ...
              switch (commonParams.payType) {
                  ...
                  wx.setClipboardData({
                    ...
                    success(res) {
                      wx.showModal({
                        ...
                        content: v[0x7574],
                        showCancel: false,
                        confirmText: v[0x7575],
                        confirmColor: v[0x7576]
                      });
                    }
                  });
                  break;
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    extraData: Minidata,
                    success: function (res) {
                      console.log(v[0x757c], res);
                    },
                    fail: function (err) {
                      console.log(v[0x757d], err);
                      wx.request({
                        url: api + v[0x757e] + commonParams.gameId + v[0x757f] + err.errMsg,
                        success(res) {
                          console.log(v[0x7580], res);
                        }
                      });
                    },
                    complete: function (com) {
                      console.log(v[0x7581], com);
                    }
                  });
                  break;
              }
...
const getWhatsNewSubscriptionsSetting = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getWhatsNewSubscriptionsSettingCallback != v[0x74ba]) {
    console.log(v[0x7583]);
  } else {
...
const requestSubscribeWhatsNew = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.requestSubscribeWhatsNewCallback != v[0x74ba]) {
    console.log(v[0x7584]);
  } else {
...
const getRedPagState = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPagStateCallback != v[0x74ba]) {
    console.log(v[0x7585]);
  } else {
    if (commonParams.showRedPagOn) {
      wx.request({
        url: wxRedApi + v[0x7586],
        data: {
          app_id: commonParams.gameId,
          open_id: commonParams.uid,
...
const redLogin = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.redLoginCallback != v[0x74ba]) {
    console.log(v[0x7588]);
  } else {
    // 初始化锁
      ...
      success(res) {
        if (res.code) {
          ...
          if (!httpLock.httpRedLoginFlag) {
            ...
            wx.request({
              url: wxRedApi + v[0x7589],
              method: v[0x125a],
              data: Tools.buildRedParams(params),
              header: {
                "Content-Type": v[0x758a]
              },
              success(res) {
                let ret = res.data;
                if (ret.status == 200) {
          ...
          console.log(v[0x758b] + res.errMsg);
        }
      }
    });
  }
...
const getWxUserInfo = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getWxUserInfoCallback != v[0x74ba]) {
    console.log(v[0x758c]);
  } else {
    const params = {
      app_id: commonParams.gameId,
...
const getRedPackageList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPackageListCallback != v[0x74ba]) {
    console.log(v[0x758e]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const getRedConfig = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedConfigCallback != v[0x74ba]) {
    console.log(v[0x7590]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const toReceiveRedPackage = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toReceiveRedPackageCallback != v[0x74ba]) {
    console.log(v[0x7592]);
  } else {
...
const getRedDetailList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedDetailListCallback != v[0x74ba]) {
    console.log(v[0x7595]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const toWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toWithdrawCallback != v[0x74ba]) {
    console.log(v[0x7597]);
  } else {
    const params = {
...
const toRedShare = function (data) {
  ...
  wx.request({
    url: wxRedApi + v[0x7599],
    data: Tools.buildRedParams(params),
    success(res) {
      let ret = res.data;
      console.log(v[0x759a], ret);
      if (ret.status == 200) {
        wx.shareAppMessage({
...
const getShareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getShareListCallback != v[0x74ba]) {
    console.log(v[0x759b]);
  } else {
    const params = {
      activity_id: data.share_activity_id,
      app_id: commonParams.gameId,
  ...
  wx.login({
    success: res => {
      if (res.code) {
        ...
        wx.request({
          url: wxRedApi + v[0x759d],
          data: Tools.buildRedParams(params),
          success(res) {
            let ret = res.data;
            console.log(v[0x759e], ret.data, ret.msg);
          },
          fail(err) {
            console.log(v[0x759f], err);
          },
          timeout: 4000
        });
      } else {
        console.log(v[0x7533], res);
      }
    }
  });
};
...
const getLuckDrawIndex = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getLuckDrawIndexCallback != v[0x74ba]) {
    console.log(v[0x75a0]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token,
...
const luckyDraw = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.luckyDrawCallback != v[0x74ba]) {
    console.log(v[0x75a2]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token,
...
const getSquareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getSquareListCallback != v[0x74ba]) {
    console.log(v[0x75a4]);
  } else {
    if (!httpLock.httpSquareListFlag) {
      ...
      wx.request({
        url: wxRedApi + v[0x75a5],
        data: Tools.buildRedParams(params),
        success(res) {
          let ret = res.data;
          console.log(v[0x75a6], ret);
...
const squareClick = function (data) {
  if (!httpLock.httpSquareClickFlag) {
    ...
    wx.request({
      url: wxRedApi + v[0x75a7],
      data: Tools.buildRedParams(params),
      success(res) {
        console.log(v[0x75a8], res);
        wx.navigateToMiniProgram({
          appId: data.wxapp_id,
          extraData: {
            square: {
...
const squareBind = function (data) {
  wx.login({
    success(ret) {
      ...
      console.log(v[0x75a9], params);
      wx.request({
        url: wxRedApi + v[0x75aa],
        data: params,
        timeout: 5000,
        success: res => {
          console.log(v[0x75ab], res);
        }
      });
    }
  });
...
const gameWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.gameWithdrawCallback != v[0x74ba]) {
    console.log(v[0x75ac]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      withdraw_amount: data.withdraw_amount,
...
const toCouponReceive = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toCouponReceiveCallback != v[0x74ba]) {
    console.log(v[0x75ae]);
  } else {
...
const getChangeShellOn = function (appid, appkey) {
  const SDKyyw = this;
  if (typeof SDKyyw.getChangeShellOnCallback != v[0x74ba]) {
    console.log(v[0x75b0]);
  } else {
...
const showAppModal = function () {
  if (commonParams.showAppOn) {
    wx.showModal({
      ...
      content: commonParams.appContent || v[0x75b2],
      cancelText: v[0x75b3],
      confirmText: v[0x75b4],
      success(res) {
        if (res.confirm) {
          wx.openCustomerServiceConversation({
            showMessageCard: true,
            ...
            complete(res) {
              console.log(v[0x75b5], res);
            }
          });
        } else if (res.cancel) {
          console.log(v[0x75b6]);
        }
      }
    });
  }
...
const msgSecCheck = function (msg, callback) {
  const msgIsString = Object.prototype.toString.call(msg).indexOf(v[0x75b7]) > -1;
  const msgIsObject = Object.prototype.toString.call(msg).indexOf(v[0x75b8]) > -1;
  var params = {};
  if (msgIsString) {
    params = {
      gameid: commonParams.gameId,
...
const gameLive = {
  ...
  startGameLive() {
    const SDKyyw = this;
    if (typeof SDKyyw.startGameLiveCallback != v[0x74ba]) {
      console.log(v[0x75be]);
      return;
    }
    wx.startGameLive({
      success(res) {
        ...
        console.log(v[0x75bf], res);
      },
      fail(err) {
        console.error(v[0x75bf], err);
        SDKyyw.startGameLiveCallback({
          status: "0",
          msg: "",
          data: err
  ...
  checkGameLiveEnabled() {
    const SDKyyw = this;
    if (typeof SDKyyw.checkGameLiveEnabledCallback != v[0x74ba]) {
      console.log(v[0x75c0]);
      return;
    }
  ...
  onGameLiveStateChange() {
    const SDKyyw = this;
    if (typeof SDKyyw.onGameLiveStateChangeCallback != v[0x74ba]) {
      console.log(v[0x75c2]);
      return;
    }
                    ...
                    wx.login({
                      success: res => {
                        if (res.code == null || res.code == undefined || res.code == "") {
                          ...
                          params.code = res.code;
                          wx.request({
                            url: wxRedApi + v[0x75c5],
                            method: v[0x125a],
                            data: Tools.buildRedParams(params),
                            success(res) {
                              console.log(v[0x75c6], res);
                            },
                            fail(err) {
                              console.log(v[0x75c7], err);
                            },
                            timeout: 10 * 1000
                          });
                        }
  ...
  getGameLiveState() {
    const SDKyyw = this;
    if (typeof SDKyyw.getGameLiveStateCallback != v[0x74ba]) {
      console.log(v[0x75c8]);
      return;
    }
    const state = wx.getGameLiveState();
    // isLive	boolean	是否正在直播
    SDKyyw.getGameLiveStateCallback(state);
    console.log(v[0x75c9], state);
  },
  // 获取小游戏用户当前正在直播的信息（可查询当前直播的 feedId）
  getUserCurrentGameliveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserCurrentGameliveInfoCallback != v[0x74ba]) {
      console.log(v[0x75ca]);
      return;
    }
    wx.getUserCurrentGameliveInfo({
      success(res) {
        ...
        console.log(v[0x75cb], res);
      },
      fail(err) {
        console.log(v[0x75cb], err);
  ...
  getUserRecentGameLiveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserRecentGameLiveInfoCallback != v[0x74ba]) {
      console.log(v[0x75cc]);
      return;
    }
    wx.getUserRecentGameLiveInfo({
      success(res) {
  ...
  getUserGameLiveDetails(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserGameLiveDetailsCallback != v[0x74ba]) {
      console.log(v[0x75ce]);
      return;
    }
    if (!data.feedIdList) {
      console.log(v[0x75cf]);
      return;
    }
    wx.getUserGameLiveDetails({
      feedIdList: data.feedIdList,
      success(res) {
        console.log(v[0x75d0], res);
  ...
  getChannelsLiveNoticeInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveNoticeInfoCallback != v[0x74ba]) {
      console.log(v[0x75d1]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x75d2]);
      return;
    }
  ...
  getChannelsLiveInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveInfoCallback != v[0x74ba]) {
      console.log(v[0x75d4]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x75d2]);
  ...
  openChannelsLive(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.openChannelsLiveCallback != v[0x74ba]) {
      console.log(v[0x75d6]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x75d2]);
  ...
  reserveChannelsLive(data = {}) {
    ...
    if (typeof SDKyyw.reserveChannelsLiveCallback != v[0x74ba]) {
      console.log(v[0x75d8]);
      return;
    }
    if (!data.noticeId) {
      console.log(v[0x75d9]);
      return;
    }
    wx.reserveChannelsLive({
      noticeId: data.noticeId || "",
      success(res) {
        console.log(v[0x75da], res);
  ...
  getLiveTask() {
    const SDKyyw = this;
    if (typeof SDKyyw.getLiveTaskCallback != v[0x74ba]) {
      console.log(v[0x75db]);
    } else {
      const params = {
        app_id: commonParams.gameId,
        open_id: commonParams.uid,
  ...
  toLiveTaskReceive(data) {
    const SDKyyw = this;
    if (typeof SDKyyw.toLiveTaskReceiveCallback != v[0x74ba]) {
      console.log(v[0x75dd]);
    } else {
  ...
  getUserLiveInfo(data) {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserLiveInfoCallback != v[0x74ba]) {
      console.log(v[0x75df]);
    } else {
...
const pushData = function (data) {
  ...
  if (data.type == "1") {
    //登录
    data.logType = v[0x7570];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x75e1]];
    delete data[v[0x75e2]];
    //accountId serverId
  } else if (data.type == "2") {
    //创建角色或者账号
    if (!data.roleId) {
      console.log(v[0x75e3]);
    }
    if (!data.roleName) {
      console.log(v[0x75e4]);
    }

    data.logType = v[0x75e5];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x75e1]];
    delete data[v[0x75e2]];
    ...
    data.logType = v[0x75e6]; //升级
    data.region = "0";
    data.levelType = "1";
    data.duration = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x75e7]];
    //accountId00  roleID00   roleName00 level00 vipLevel00   startLevel00 endLevel00
  } else if (data.type == "5") {
    data.logType = v[0x75e8]; //在线
    delete data[v[0x75e9]];
    delete data[v[0x75ea]];
    delete data[v[0x751e]];
    delete data[v[0x75eb]];
    delete data[v[0x663b]];
    delete data[v[0x75ec]];
    delete data[v[0x75ed]];
    //accountId serverId  roleID  roleName
  } else if (data.type == 6) {
    // 分享
    data.logType = v[0x75ee];
    delete data[v[0x75e9]];
    delete data[v[0x75ea]];
    delete data[v[0x751e]];
    delete data[v[0x75eb]];
    ...
    // 自定义事件
    data.logType = v[0x1fb];
    delete data[v[0x75e9]];
    delete data[v[0x75ea]];
    delete data[v[0x75ef]];
    delete data[v[0x751e]];
    delete data[v[0x75eb]];
    delete data[v[0x663b]];
    delete data[v[0x75ec]];
    delete data[v[0x75ed]];
    delete data[v[0x751f]];
  } else if (data.type == 10) {
    data.logType = v[0x75f0]; // 游戏关卡
    data.channelAccid = commonParams.uid;
  } else if (data.type == 11) {
    data.logType = v[0x75f1]; // 广告
    data.channelAccid = commonParams.uid;
  }
  delete data[v[0x66]];
  wx.request({
...
function setLog(str, level) {
  if (state.debug == true) {
    if (level == state.level.error) {
      console.log(v[0x75f3] + str, v[0x75f4]);
    } else if (level == state.level.warn) {
      console.log(v[0x75f5] + str, v[0x75f6]);
    } else if (level == state.level.debug) {
      console.log(v[0x75f7] + str, v[0x75f6]);
    } else {
      console.log(v[0x75f8] + str, v[0x75f6]);
    }
  }
}
