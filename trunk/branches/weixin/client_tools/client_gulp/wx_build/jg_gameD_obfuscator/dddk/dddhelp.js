...
// 地址
let api = v[0x7516];
let wanBaApi = v[0x7517];
let logApi = v[0x7518];
let wxRedApi = v[0x7519];

const Tools = {
  getDeviceType: () => {
    const systemInfo = wx.getSystemInfoSync();
    let osType;
    if (systemInfo.system.toLowerCase().indexOf(v[0x751a]) > -1) {
      osType = v[0x751a];
    } else if (systemInfo.system.toLowerCase().indexOf(v[0x751b]) > -1) {
      osType = v[0x751b];
  ...
  buildActiveParams: (params, gameKey) => {
    const keys = [v[0x751d], v[0x751e], v[0x751f], v[0x7520], v[0x7521], v[0x35a]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
    signString = signString + gameKey;
    params[v[0x2dc0]] = md5(signString);
    return params;
  },
  buildChangeShell: (params, gameKey) => {
    const keys = [v[0x751d], v[0x7522], v[0x35a]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
  ...
  buildLoginParams: params => {
    const keys = [v[0x7523], v[0x751d], v[0x751f], v[0x751e], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildBindParams: params => {
    const keys = [v[0x751d], v[0x7524], v[0x7525], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildPayParams: params => {
    const keys = [v[0x751d], v[0x7523], v[0x751f], v[0x1ea3], v[0x4ab3], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildRepublish: params => {
    const keys = [v[0x1ea3], v[0x751d], v[0x7523], v[0x751f], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildCheckMsg(params) {
    const keys = [v[0x751d], v[0x751f], v[0x7523], v[0x35a]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
    ...
    signString = signString + commonParams.gameKey;
    params[v[0x2dc0]] = md5(signString).toLowerCase();
    return params;
  },

  getDeviceTypeId: () => {
    const osType = Tools.getDeviceType();
    let deviceType = v[0x751c];
    if (osType == v[0x751b]) {
      deviceType = 1;
    } else if (osType == v[0x751a]) {
      deviceType = 2;
  ...
  buildRedParams: params => {
    ...
    params[v[0x2dc0]] = md5(stringParams);
    return params;
  },
  initLock: locks => {
...
const init = function (gameId, gameKey) {
  ...
  if (typeof SDKyyw.initLoginCallback != v[0x74bc]) {
    console.error(v[0x7526]);
  } else {
    ...
    console.log(v[0x7527], accountInfo);
    console.log(v[0x7528], accountInfo.miniProgram.envVersion);
    ...
    console.log(v[0x7529], launchOption);
...
const active = function (SDKyyw, params, gameId, gameKey) {
  wx.request({
    ...
    success: res => {
      ...
      if (ret.data.code == 1) {
        ...
        if (commonParams.gameId) {
          ...
          data.appVersion = v[0x752b];
          data.platform = v[0x752c];
        ...
        let orderList = [];
        wx.setStorageSync(v[0x752d], orderList);
        wxLogin(SDKyyw);
      } else {
        state.initStatus = v[0x663c];
        console.log(v[0x752e], v[0x752f] + gameId, v[0x7530] + gameKey);
    ...
    fail: err => {
      console.log(v[0x7531]);
      if (commonParams.initTimer) {
        clearTimeout(commonParams.initTimer);
      }
      wx.request({
        url: v[0x7532] + v[0x7533],
        timeout: 5000,
        success: res => {
          res = res.data;
          if (res.status === 1) {
            console.log(v[0x7534], res.data.host);
    ...
    success: res => {
      if (res.code == null || res.code == undefined || res.code == "") {
        ...
        let params = {
          ...
          code: jscode,
          iv: commonParams.iv,
          osType: Tools.getDeviceType() == v[0x751b] ? "2" : Tools.getDeviceType() == v[0x751a] ? "1" : Tools.getDeviceType() == v[0x1ee5] ? "3" : v[0x751c],
        ...
        console.log(v[0x7537], params);
        sdkLogin(SDKyyw, params);
      }
    },
    fail: err => {
      wxLogin(SDKyyw);
      console.log(v[0x7538], err);
    }
  });
};
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        console.log(v[0x753a], commonParams.referrerInfo);
        ...
        console.log(v[0x753c], ret);
        // SDK登录回调
        SDKyyw.initLoginCallback(ret);
      } else {
        console.log(v[0x753d], res);
...
const cutGame = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.cutGameCallback != v[0x74bc]) {
    console.log(v[0x753f]);
  } else {
    wx.navigateToMiniProgram({
      ...
      envVersion: commonParams.switchEnv || v[0x7540], // develop trial release
      success(res) {
        console.log(v[0x7541], res);
...
const getCaptcha = function (data, sucCallback, errorCallback) {
  const SDKyyw = this;
  if (typeof sucCallback == v[0x74bc]) {
    ...
    wx.request({
      url: wanBaApi + v[0x7545],
      data: params,
      timeout: 3000,
      success: res => {
        if (res.data.code === 18000) {
          console.log(v[0x7546], res);
          sucCallback(res);
        } else {
          console.log(v[0x7547], res);
          errorCallback(res);
        }
      },
      error: err => {
        console.log(v[0x7548], err);
      }
    });
  } else {
    if (typeof SDKyyw.getCaptchaCallback != v[0x74bc]) {
...
const bindTelephone = function (data, smsCode, sucCallback, errorCallback) {
  ...
  if (typeof sucCallback == v[0x74bc]) {
    ...
    if (typeof SDKyyw.bindTelephoneCallback != v[0x74bc]) {
...
const replenish = function () {
  let orderList = wx.getStorageSync(v[0x752d]);
...
const subscribeMessage = function (tmplIds) {
  const SDKyyw = this;
  if (typeof SDKyyw.subscribeMsgCallback != v[0x74bc]) {
...
const advertisement = function (data) {
  ...
  if (data.type == v[0x754e]) {
    if (typeof SDKyyw.createBannerAdCallback != v[0x74bc]) {
  ...
  } else if (data.type == v[0x7553]) {
    if (typeof SDKyyw.rewardedVideoAdCallback != v[0x74bc]) {
  ...
  } else if (data.type == v[0x7559]) {
    if (typeof SDKyyw.createInterstitialAdCallback != v[0x74bc]) {
      console.log(v[0x755a]);
    } else {
      adData.adType = 3;
      // 创建插屏广告实例，提前初始化
  ...
  } else if (data.type == v[0x755e]) {
    if (typeof SDKyyw.createGridAdCallback != v[0x74bc]) {
      ...
      state.gridAd.onError(err => {
        ...
        console.log(v[0x7563], err);
      });
    }
  } else if (data.type == v[0x7564]) {
    if (typeof SDKyyw.createCustomAdCallback != v[0x74bc]) {
      console.log(v[0x7565]);
    } else {
      // 原生模板广告
      adData.adType = 5;
...
const openData = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.openDataCallback != v[0x74bc]) {
    console.error(v[0x7568]);
  } else {
    if (data.type == v[0x7569]) {
      //好友感兴趣
...
const pay = function (payData) {
  ...
  if (typeof this.onPayCallback != v[0x74bc]) {
    console.error(v[0x756f]);
  } else {
    console.log(v[0x7570], commonParams);
    const params = {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success: payRes => {
            ...
            console.log(v[0x7574], payRes.data);
            wx.hideLoading();
            let ret = payRes.data;
              ...
              switch (commonParams.payType) {
                  ...
                  wx.setClipboardData({
                    ...
                    success(res) {
                      wx.showModal({
                        ...
                        content: v[0x7576],
                        showCancel: false,
                        confirmText: v[0x7577],
                        confirmColor: v[0x7578]
                      });
                    }
                  });
                  break;
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    extraData: Minidata,
                    success: function (res) {
                      console.log(v[0x757e], res);
                    },
                    fail: function (err) {
                      console.log(v[0x757f], err);
                      wx.request({
                        url: api + v[0x7580] + commonParams.gameId + v[0x7581] + err.errMsg,
                        success(res) {
                          console.log(v[0x7582], res);
                        }
                      });
                    },
                    complete: function (com) {
                      console.log(v[0x7583], com);
                    }
                  });
                  break;
              }
...
const getWhatsNewSubscriptionsSetting = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getWhatsNewSubscriptionsSettingCallback != v[0x74bc]) {
...
const requestSubscribeWhatsNew = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.requestSubscribeWhatsNewCallback != v[0x74bc]) {
    console.log(v[0x7586]);
  } else {
...
const getRedPagState = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPagStateCallback != v[0x74bc]) {
    console.log(v[0x7587]);
  } else {
    if (commonParams.showRedPagOn) {
      wx.request({
        url: wxRedApi + v[0x7588],
        data: {
          app_id: commonParams.gameId,
          open_id: commonParams.uid,
...
const redLogin = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.redLoginCallback != v[0x74bc]) {
    console.log(v[0x758a]);
  } else {
    // 初始化锁
      ...
      success(res) {
        if (res.code) {
          ...
          if (!httpLock.httpRedLoginFlag) {
            ...
            wx.request({
              url: wxRedApi + v[0x758b],
              method: v[0x125a],
              data: Tools.buildRedParams(params),
              header: {
                "Content-Type": v[0x758c]
              },
              success(res) {
                let ret = res.data;
                if (ret.status == 200) {
          ...
          console.log(v[0x758d] + res.errMsg);
        }
      }
    });
  }
...
const getWxUserInfo = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getWxUserInfoCallback != v[0x74bc]) {
...
const getRedPackageList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPackageListCallback != v[0x74bc]) {
...
const getRedConfig = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedConfigCallback != v[0x74bc]) {
...
const toReceiveRedPackage = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toReceiveRedPackageCallback != v[0x74bc]) {
    console.log(v[0x7594]);
  } else {
...
const getRedDetailList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedDetailListCallback != v[0x74bc]) {
...
const toWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toWithdrawCallback != v[0x74bc]) {
    console.log(v[0x7599]);
  } else {
    const params = {
...
const toRedShare = function (data) {
  ...
  wx.request({
    url: wxRedApi + v[0x759b],
    data: Tools.buildRedParams(params),
    success(res) {
      let ret = res.data;
      console.log(v[0x759c], ret);
      if (ret.status == 200) {
        wx.shareAppMessage({
...
const getShareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getShareListCallback != v[0x74bc]) {
    console.log(v[0x759d]);
  } else {
    const params = {
      activity_id: data.share_activity_id,
      app_id: commonParams.gameId,
  ...
  wx.login({
    success: res => {
      if (res.code) {
        ...
        wx.request({
          url: wxRedApi + v[0x759f],
          data: Tools.buildRedParams(params),
          success(res) {
            let ret = res.data;
            console.log(v[0x75a0], ret.data, ret.msg);
          },
          fail(err) {
            console.log(v[0x75a1], err);
          },
          timeout: 4000
        });
      } else {
        console.log(v[0x7535], res);
      }
    }
  });
};
...
const getLuckDrawIndex = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getLuckDrawIndexCallback != v[0x74bc]) {
...
const luckyDraw = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.luckyDrawCallback != v[0x74bc]) {
...
const getSquareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getSquareListCallback != v[0x74bc]) {
    console.log(v[0x75a6]);
  } else {
...
const squareClick = function (data) {
  if (!httpLock.httpSquareClickFlag) {
    ...
    wx.request({
      url: wxRedApi + v[0x75a9],
      data: Tools.buildRedParams(params),
      success(res) {
        console.log(v[0x75aa], res);
        wx.navigateToMiniProgram({
          appId: data.wxapp_id,
          extraData: {
            square: {
...
const squareBind = function (data) {
  wx.login({
    success(ret) {
      ...
      console.log(v[0x75ab], params);
      wx.request({
        url: wxRedApi + v[0x75ac],
        data: params,
        timeout: 5000,
        success: res => {
          console.log(v[0x75ad], res);
        }
      });
    }
  });
...
const gameWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.gameWithdrawCallback != v[0x74bc]) {
...
const toCouponReceive = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toCouponReceiveCallback != v[0x74bc]) {
...
const getChangeShellOn = function (appid, appkey) {
  const SDKyyw = this;
  if (typeof SDKyyw.getChangeShellOnCallback != v[0x74bc]) {
    console.log(v[0x75b2]);
  } else {
...
const showAppModal = function () {
  if (commonParams.showAppOn) {
    wx.showModal({
      ...
      content: commonParams.appContent || v[0x75b4],
      cancelText: v[0x75b5],
      confirmText: v[0x75b6],
      success(res) {
        if (res.confirm) {
          wx.openCustomerServiceConversation({
            showMessageCard: true,
            ...
            complete(res) {
              console.log(v[0x75b7], res);
            }
          });
        } else if (res.cancel) {
          console.log(v[0x75b8]);
        }
      }
    });
  }
...
const msgSecCheck = function (msg, callback) {
  const msgIsString = Object.prototype.toString.call(msg).indexOf(v[0x75b9]) > -1;
  const msgIsObject = Object.prototype.toString.call(msg).indexOf(v[0x75ba]) > -1;
  var params = {};
  if (msgIsString) {
    params = {
      gameid: commonParams.gameId,
...
const gameLive = {
  ...
  startGameLive() {
    const SDKyyw = this;
    if (typeof SDKyyw.startGameLiveCallback != v[0x74bc]) {
  ...
  checkGameLiveEnabled() {
    const SDKyyw = this;
    if (typeof SDKyyw.checkGameLiveEnabledCallback != v[0x74bc]) {
  ...
  onGameLiveStateChange() {
    const SDKyyw = this;
    if (typeof SDKyyw.onGameLiveStateChangeCallback != v[0x74bc]) {
      console.log(v[0x75c4]);
      return;
    }
                    ...
                    wx.login({
                      success: res => {
                        if (res.code == null || res.code == undefined || res.code == "") {
                          ...
                          params.code = res.code;
                          wx.request({
                            url: wxRedApi + v[0x75c7],
                            method: v[0x125a],
                            data: Tools.buildRedParams(params),
                            success(res) {
                              console.log(v[0x75c8], res);
                            },
                            fail(err) {
                              console.log(v[0x75c9], err);
                            },
                            timeout: 10 * 1000
                          });
                        }
  ...
  getGameLiveState() {
    const SDKyyw = this;
    if (typeof SDKyyw.getGameLiveStateCallback != v[0x74bc]) {
    ...
    console.log(v[0x75cb], state);
  },
  // 获取小游戏用户当前正在直播的信息（可查询当前直播的 feedId）
  getUserCurrentGameliveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserCurrentGameliveInfoCallback != v[0x74bc]) {
  ...
  getUserRecentGameLiveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserRecentGameLiveInfoCallback != v[0x74bc]) {
  ...
  getUserGameLiveDetails(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserGameLiveDetailsCallback != v[0x74bc]) {
      console.log(v[0x75d0]);
      return;
    }
    if (!data.feedIdList) {
  ...
  getChannelsLiveNoticeInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveNoticeInfoCallback != v[0x74bc]) {
      console.log(v[0x75d3]);
      return;
    }
  ...
  getChannelsLiveInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveInfoCallback != v[0x74bc]) {
  ...
  openChannelsLive(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.openChannelsLiveCallback != v[0x74bc]) {
  ...
  reserveChannelsLive(data = {}) {
    ...
    if (typeof SDKyyw.reserveChannelsLiveCallback != v[0x74bc]) {
      console.log(v[0x75da]);
      return;
    }
    if (!data.noticeId) {
  ...
  getLiveTask() {
    const SDKyyw = this;
    if (typeof SDKyyw.getLiveTaskCallback != v[0x74bc]) {
  ...
  toLiveTaskReceive(data) {
    const SDKyyw = this;
    if (typeof SDKyyw.toLiveTaskReceiveCallback != v[0x74bc]) {
  ...
  getUserLiveInfo(data) {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserLiveInfoCallback != v[0x74bc]) {
      console.log(v[0x75e1]);
    } else {
...
const pushData = function (data) {
  ...
  if (data.type == "1") {
    //登录
    data.logType = v[0x7572];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x75e3]];
    delete data[v[0x75e4]];
    //accountId serverId
  } else if (data.type == "2") {
    //创建角色或者账号
    if (!data.roleId) {
      console.log(v[0x75e5]);
    }
    if (!data.roleName) {
      console.log(v[0x75e6]);
    }

    data.logType = v[0x75e7];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x75e3]];
    delete data[v[0x75e4]];
    ...
    delete data[v[0x75eb]];
    delete data[v[0x75ec]];
    delete data[v[0x75f1]];
    delete data[v[0x7520]];
    delete data[v[0x75ed]];
    delete data[v[0x663d]];
    delete data[v[0x75ee]];
    delete data[v[0x75ef]];
    delete data[v[0x7521]];
  } else if (data.type == 10) {
    data.logType = v[0x75f2]; // 游戏关卡
    data.channelAccid = commonParams.uid;
  } else if (data.type == 11) {
    data.logType = v[0x75f3]; // 广告
    data.channelAccid = commonParams.uid;
  }
  delete data[v[0x66]];
  wx.request({
...
function setLog(str, level) {
  if (state.debug == true) {
    if (level == state.level.error) {
      ...
      console.log(v[0x75f7] + str, v[0x75f8]);
    } else if (level == state.level.debug) {
      console.log(v[0x75f9] + str, v[0x75f8]);
    } else {
      console.log(v[0x75fa] + str, v[0x75f8]);
    }
  }
}
