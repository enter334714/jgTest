...
// 地址
const api = v[0x6fc9];
const wanBaApi = v[0x6fca];
const logApi = v[0x6fcb];
const wxRedApi = v[0x6fcc];

const Tools = {
  getDeviceType: () => {
    const systemInfo = wx.getSystemInfoSync();
    let osType;
    if (systemInfo.system.toLowerCase().indexOf(v[0x6fcd]) > -1) {
      osType = v[0x6fcd];
    } else if (systemInfo.system.toLowerCase().indexOf(v[0x6fce]) > -1) {
      osType = v[0x6fce];
    } else if (systemInfo.system.toLowerCase().indexOf(v[0x1e5b]) > -1) {
      osType = v[0x1e5b];
    } else {
      osType = v[0x6fcf];
    }
  ...
  buildActiveParams: (params, gameKey) => {
    const keys = [v[0x6fd0], v[0x6fd1], v[0x6fd2], v[0x6fd3], v[0x6fd4], v[0x353]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
    signString = signString + gameKey;
    params[v[0x2d10]] = md5(signString);
    return params;
  },
  buildChangeShell: (params, gameKey) => {
    const keys = [v[0x6fd0], v[0x6fd5], v[0x353]];
    let signString = "";
    for (let k in params) {
      signString = signString + params[k];
    }
  ...
  buildLoginParams: params => {
    const keys = [v[0x6fd6], v[0x6fd0], v[0x6fd2], v[0x6fd1], v[0x353]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildBindParams: params => {
    const keys = [v[0x6fd0], v[0x6fd7], v[0x6fd8], v[0x353]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildPayParams: params => {
    const keys = [v[0x6fd0], v[0x6fd6], v[0x6fd2], v[0x1e18], v[0x4920], v[0x353]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildRepublish: params => {
    const keys = [v[0x1e18], v[0x6fd0], v[0x6fd6], v[0x6fd2], v[0x353]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
  ...
  buildCheckMsg(params) {
    const keys = [v[0x6fd0], v[0x6fd2], v[0x6fd6], v[0x353]];
    let signString = "";
    for (let key in keys) {
      let k = keys[key];
      signString = signString + params[k];
    ...
    signString = signString + commonParams.gameKey;
    params[v[0x2d10]] = md5(signString).toLowerCase();
    return params;
  },

  getDeviceTypeId: () => {
    const osType = Tools.getDeviceType();
    let deviceType = v[0x6fcf];
    if (osType == v[0x6fce]) {
      deviceType = 1;
    } else if (osType == v[0x6fcd]) {
      deviceType = 2;
    } else if (osType == v[0x1e5b]) {
      deviceType = 3;
    }
    return deviceType;
  },
  ...
  buildRedParams: params => {
    ...
    stringParams += v[0x10e] + "=" + commonParams.gameKey;
    params[v[0x2d10]] = md5(stringParams);
    return params;
  },
  initLock: locks => {
...
const init = function (gameId, gameKey) {
  ...
  if (typeof SDKyyw.initLoginCallback != v[0x6fd9]) {
    console.error(v[0x6fda]);
  } else {
    ...
    console.log(v[0x6fdb], accountInfo);
    console.log(v[0x6fdc], accountInfo.miniProgram.envVersion);
    ...
    console.log(v[0x6fdd], launchOption);
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.data.code == 1) {
        ...
        let orderList = [];
        wx.setStorageSync(v[0x6fe1], orderList);
        wxLogin(SDKyyw);
      } else {
        state.initStatus = v[0x6420];
        console.log(v[0x6fe2], v[0x6fe3] + gameId, v[0x6fe4] + gameKey);
    ...
    success: res => {
      if (res.code == null || res.code == undefined || res.code == "") {
        ...
        let params = {
          ...
          code: jscode,
          iv: commonParams.iv,
          osType: Tools.getDeviceType() == v[0x6fce] ? "2" : Tools.getDeviceType() == v[0x6fcd] ? "1" : Tools.getDeviceType() == v[0x1e5b] ? "3" : v[0x6fcf],
        ...
        console.log(v[0x6fe8], params);
        sdkLogin(SDKyyw, params);
      }
    },
    fail: err => {
      wxLogin(SDKyyw);
      console.log(v[0x6fe9], err);
    }
  });
};
  ...
  wx.request({
    ...
    success: res => {
      ...
      if (ret.status == 1) {
        ...
        console.log(v[0x6feb], commonParams.referrerInfo);
        ...
        console.log(v[0x1e18], ret.data.uid);
        console.log(v[0x6fed], ret);
        // SDK登录回调
        SDKyyw.initLoginCallback(ret);
      } else {
        console.log(v[0x6fee], res);
...
const cutGame = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.cutGameCallback != v[0x6fd9]) {
    console.log(v[0x6ff0]);
  } else {
    wx.navigateToMiniProgram({
      ...
      envVersion: commonParams.switchEnv || v[0x6ff1], // develop trial release
      success(res) {
        console.log(v[0x6ff2], res);
...
const getCaptcha = function (data, sucCallback, errorCallback) {
  const SDKyyw = this;
  if (typeof sucCallback == v[0x6fd9]) {
    ...
    wx.request({
      url: wanBaApi + v[0x6ff6],
      data: params,
      timeout: 3000,
      success: res => {
        if (res.data.code === 18000) {
          console.log(v[0x6ff7], res);
          sucCallback(res);
        } else {
          console.log(v[0x6ff8], res);
          errorCallback(res);
        }
      },
      error: err => {
        console.log(v[0x6ff9], err);
      }
    });
  } else {
    if (typeof SDKyyw.getCaptchaCallback != v[0x6fd9]) {
      console.log(v[0x6ffa]);
    } else {
      let params = Tools.buildParams({
        yy_pf: commonParams.yy_pf,
        app_id: commonParams.gameId,
...
const bindTelephone = function (data, smsCode, sucCallback, errorCallback) {
  ...
  if (typeof sucCallback == v[0x6fd9]) {
    ...
    if (typeof SDKyyw.bindTelephoneCallback != v[0x6fd9]) {
      console.log(v[0x6ffc]);
    } else {
      let params = Tools.buildParams({
        yy_pf: commonParams.yy_pf,
        app_id: commonParams.gameId,
...
const replenish = function () {
  let orderList = wx.getStorageSync(v[0x6fe1]);
...
const subscribeMessage = function (tmplIds) {
  const SDKyyw = this;
  if (typeof SDKyyw.subscribeMsgCallback != v[0x6fd9]) {
    console.log(v[0x6ffe]);
  } else {
...
const advertisement = function (data) {
  ...
  if (data.type == v[0x6fff]) {
    if (typeof SDKyyw.createBannerAdCallback != v[0x6fd9]) {
      console.log(v[0x7000]);
    } else {
  ...
  } else if (data.type == v[0x7004]) {
    if (typeof SDKyyw.rewardedVideoAdCallback != v[0x6fd9]) {
      console.log(v[0x7005]);
    } else {
      adData.adType = 2;
      // 创建激励视频广告实例，提前初始化
  ...
  } else if (data.type == v[0x700a]) {
    if (typeof SDKyyw.createInterstitialAdCallback != v[0x6fd9]) {
      console.log(v[0x700b]);
    } else {
      adData.adType = 3;
      // 创建插屏广告实例，提前初始化
  ...
  } else if (data.type == v[0x700f]) {
    if (typeof SDKyyw.createGridAdCallback != v[0x6fd9]) {
      console.log(v[0x7010]);
    } else {
      // 格子广告
      adData.adType = 4;
      ...
      state.gridAd.onError(err => {
        ...
        console.log(v[0x7014], err);
      });
    }
  } else if (data.type == v[0x7015]) {
    if (typeof SDKyyw.createCustomAdCallback != v[0x6fd9]) {
      console.log(v[0x7016]);
    } else {
      // 原生模板广告
      adData.adType = 5;
...
const openData = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.openDataCallback != v[0x6fd9]) {
    console.error(v[0x7019]);
  } else {
    if (data.type == v[0x701a]) {
      //好友感兴趣
...
const pay = function (payData) {
  ...
  if (typeof this.onPayCallback != v[0x6fd9]) {
    console.error(v[0x7020]);
  } else {
    console.log(v[0x7021], commonParams);
    const params = {
      ...
      success(res) {
        ...
        wx.request({
          ...
          success: payRes => {
            ...
            console.log(v[0x7025], payRes.data);
            wx.hideLoading();
            let ret = payRes.data;
              ...
              switch (commonParams.payType) {
                  ...
                  wx.setClipboardData({
                    ...
                    success(res) {
                      wx.showModal({
                        ...
                        content: v[0x7027],
                        showCancel: false,
                        confirmText: v[0x7028],
                        confirmColor: v[0x7029]
                      });
                    }
                  });
                  break;
                ...
                case 5:
                  ...
                  wx.navigateToMiniProgram({
                    ...
                    extraData: Minidata,
                    success: function (res) {
                      console.log(v[0x702f], res);
                    },
                    fail: function (err) {
                      console.log(v[0x7030], err);
                      wx.request({
                        url: v[0x7031] + commonParams.gameId + v[0x7032] + err.errMsg,
                        success(res) {
                          console.log(v[0x7033], res);
                        }
                      });
                    },
                    complete: function (com) {
                      console.log(v[0x7034], com);
                    }
                  });
                  break;
              }
...
const getWhatsNewSubscriptionsSetting = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getWhatsNewSubscriptionsSettingCallback != v[0x6fd9]) {
    console.log(v[0x7036]);
  } else {
...
const requestSubscribeWhatsNew = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.requestSubscribeWhatsNewCallback != v[0x6fd9]) {
    console.log(v[0x7037]);
  } else {
...
const getRedPagState = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPagStateCallback != v[0x6fd9]) {
    console.log(v[0x7038]);
  } else {
    if (commonParams.showRedPagOn) {
      wx.request({
        url: wxRedApi + v[0x7039],
        data: {
          app_id: commonParams.gameId,
          open_id: commonParams.uid,
...
const redLogin = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.redLoginCallback != v[0x6fd9]) {
    console.log(v[0x703b]);
  } else {
    // 初始化锁
      ...
      success(res) {
        if (res.code) {
          ...
          if (!httpLock.httpRedLoginFlag) {
            ...
            wx.request({
              url: wxRedApi + v[0x703c],
              method: v[0x11eb],
              data: Tools.buildRedParams(params),
              header: {
                "Content-Type": v[0x703d]
              },
              success(res) {
                let ret = res.data;
                if (ret.status == 200) {
          ...
          console.log(v[0x703e] + res.errMsg);
        }
      }
    });
  }
...
const getWxUserInfo = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getWxUserInfoCallback != v[0x6fd9]) {
    console.log(v[0x703f]);
  } else {
    const params = {
      app_id: commonParams.gameId,
...
const getRedPackageList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedPackageListCallback != v[0x6fd9]) {
    console.log(v[0x7041]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const getRedConfig = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedConfigCallback != v[0x6fd9]) {
    console.log(v[0x7043]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const toReceiveRedPackage = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toReceiveRedPackageCallback != v[0x6fd9]) {
    console.log(v[0x7045]);
  } else {
...
const getRedDetailList = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getRedDetailListCallback != v[0x6fd9]) {
    console.log(v[0x7048]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token
...
const toWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toWithdrawCallback != v[0x6fd9]) {
    console.log(v[0x704a]);
  } else {
    const params = {
...
const toRedShare = function (data) {
  ...
  wx.request({
    url: wxRedApi + v[0x704c],
    data: Tools.buildRedParams(params),
    success(res) {
      let ret = res.data;
      console.log(v[0x704d], ret);
      if (ret.status == 200) {
        wx.shareAppMessage({
...
const getShareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getShareListCallback != v[0x6fd9]) {
    console.log(v[0x704e]);
  } else {
    const params = {
      activity_id: data.share_activity_id,
      app_id: commonParams.gameId,
  ...
  wx.login({
    success: res => {
      if (res.code) {
        ...
        wx.request({
          url: wxRedApi + v[0x7050],
          data: Tools.buildRedParams(params),
          success(res) {
            let ret = res.data;
            console.log(v[0x7051], ret.data, ret.msg);
          },
          fail(err) {
            console.log(v[0x7052], err);
          },
          timeout: 4000
        });
      } else {
        console.log(v[0x6fe6], res);
      }
    }
  });
};
...
const getLuckDrawIndex = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.getLuckDrawIndexCallback != v[0x6fd9]) {
    console.log(v[0x7053]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token,
...
const luckyDraw = function () {
  const SDKyyw = this;
  if (typeof SDKyyw.luckyDrawCallback != v[0x6fd9]) {
    console.log(v[0x7055]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      token: commonParams.token,
...
const getSquareList = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.getSquareListCallback != v[0x6fd9]) {
    console.log(v[0x7057]);
  } else {
    if (!httpLock.httpSquareListFlag) {
      ...
      wx.request({
        url: wxRedApi + v[0x7058],
        data: Tools.buildRedParams(params),
        success(res) {
          let ret = res.data;
          console.log(v[0x7059], ret);
...
const squareClick = function (data) {
  if (!httpLock.httpSquareClickFlag) {
    ...
    wx.request({
      url: wxRedApi + v[0x705a],
      data: Tools.buildRedParams(params),
      success(res) {
        console.log(v[0x705b], res);
        wx.navigateToMiniProgram({
          appId: data.wxapp_id,
          extraData: {
            square: {
...
const squareBind = function (data) {
  wx.login({
    success(ret) {
      ...
      console.log(v[0x705c], params);
      wx.request({
        url: wxRedApi + v[0x705d],
        data: params,
        timeout: 5000,
        success: res => {
          console.log(v[0x705e], res);
        }
      });
    }
  });
...
const gameWithdraw = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.gameWithdrawCallback != v[0x6fd9]) {
    console.log(v[0x705f]);
  } else {
    const params = {
      app_id: commonParams.gameId,
      withdraw_amount: data.withdraw_amount,
...
const toCouponReceive = function (data) {
  const SDKyyw = this;
  if (typeof SDKyyw.toCouponReceiveCallback != v[0x6fd9]) {
    console.log(v[0x7061]);
  } else {
...
const getChangeShellOn = function (appid, appkey) {
  const SDKyyw = this;
  if (typeof SDKyyw.getChangeShellOnCallback != v[0x6fd9]) {
    console.log(v[0x7063]);
  } else {
...
const showAppModal = function () {
  if (commonParams.showAppOn) {
    wx.showModal({
      ...
      content: commonParams.appContent || v[0x7065],
      cancelText: v[0x7066],
      confirmText: v[0x7067],
      success(res) {
        if (res.confirm) {
          wx.openCustomerServiceConversation({
            showMessageCard: true,
            ...
            complete(res) {
              console.log(v[0x7068], res);
            }
          });
        } else if (res.cancel) {
          console.log(v[0x7069]);
        }
      }
    });
  }
...
const msgSecCheck = function (msg, callback) {
  const msgIsString = Object.prototype.toString.call(msg).indexOf(v[0x706a]) > -1;
  const msgIsObject = Object.prototype.toString.call(msg).indexOf(v[0x706b]) > -1;
  var params = {};
  if (msgIsString) {
    params = {
      gameid: commonParams.gameId,
...
const gameLive = {
  ...
  startGameLive() {
    const SDKyyw = this;
    if (typeof SDKyyw.startGameLiveCallback != v[0x6fd9]) {
      console.log(v[0x7071]);
      return;
    }
    wx.startGameLive({
      success(res) {
        ...
        console.log(v[0x7072], res);
      },
      fail(err) {
        console.error(v[0x7072], err);
        SDKyyw.startGameLiveCallback({
          status: "0",
          msg: "",
          data: err
  ...
  checkGameLiveEnabled() {
    const SDKyyw = this;
    if (typeof SDKyyw.checkGameLiveEnabledCallback != v[0x6fd9]) {
      console.log(v[0x7073]);
      return;
    }
  ...
  onGameLiveStateChange() {
    const SDKyyw = this;
    if (typeof SDKyyw.onGameLiveStateChangeCallback != v[0x6fd9]) {
      console.log(v[0x7075]);
      return;
    }
                    ...
                    wx.login({
                      success: res => {
                        if (res.code == null || res.code == undefined || res.code == "") {
                          ...
                          params.code = res.code;
                          wx.request({
                            url: wxRedApi + v[0x7078],
                            method: v[0x11eb],
                            data: Tools.buildRedParams(params),
                            success(res) {
                              console.log(v[0x7079], res);
                            },
                            fail(err) {
                              console.log(v[0x707a], err);
                            },
                            timeout: 10 * 1000
                          });
                        }
  ...
  getGameLiveState() {
    const SDKyyw = this;
    if (typeof SDKyyw.getGameLiveStateCallback != v[0x6fd9]) {
      console.log(v[0x707b]);
      return;
    }
    const state = wx.getGameLiveState();
    // isLive	boolean	是否正在直播
    SDKyyw.getGameLiveStateCallback(state);
    console.log(v[0x707c], state);
  },
  // 获取小游戏用户当前正在直播的信息（可查询当前直播的 feedId）
  getUserCurrentGameliveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserCurrentGameliveInfoCallback != v[0x6fd9]) {
      console.log(v[0x707d]);
      return;
    }
    wx.getUserCurrentGameliveInfo({
      success(res) {
        ...
        console.log(v[0x707e], res);
      },
      fail(err) {
        console.log(v[0x707e], err);
  ...
  getUserRecentGameLiveInfo() {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserRecentGameLiveInfoCallback != v[0x6fd9]) {
      console.log(v[0x707f]);
      return;
    }
    wx.getUserRecentGameLiveInfo({
      success(res) {
  ...
  getUserGameLiveDetails(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getUserGameLiveDetailsCallback != v[0x6fd9]) {
      console.log(v[0x7081]);
      return;
    }
    if (!data.feedIdList) {
      console.log(v[0x7082]);
      return;
    }
    wx.getUserGameLiveDetails({
      feedIdList: data.feedIdList,
      success(res) {
        console.log(v[0x7083], res);
  ...
  getChannelsLiveNoticeInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveNoticeInfoCallback != v[0x6fd9]) {
      console.log(v[0x7084]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x7085]);
      return;
    }
  ...
  getChannelsLiveInfo(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.getChannelsLiveInfoCallback != v[0x6fd9]) {
      console.log(v[0x7087]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x7085]);
  ...
  openChannelsLive(data = {}) {
    const SDKyyw = this;
    if (typeof SDKyyw.openChannelsLiveCallback != v[0x6fd9]) {
      console.log(v[0x7089]);
      return;
    }
    if (!data.finderUserName) {
      console.log(v[0x7085]);
  ...
  reserveChannelsLive(data = {}) {
    ...
    if (typeof SDKyyw.reserveChannelsLiveCallback != v[0x6fd9]) {
      console.log(v[0x708b]);
      return;
    }
    if (!data.noticeId) {
      console.log(v[0x708c]);
      return;
    }
    wx.reserveChannelsLive({
      noticeId: data.noticeId || "",
      success(res) {
        console.log(v[0x708d], res);
  ...
  getLiveTask() {
    const SDKyyw = this;
    if (typeof SDKyyw.getLiveTaskCallback != v[0x6fd9]) {
      console.log(v[0x708e]);
    } else {
      const params = {
        app_id: commonParams.gameId,
        open_id: commonParams.uid,
  ...
  toLiveTaskReceive(data) {
    const SDKyyw = this;
    if (typeof SDKyyw.toLiveTaskReceiveCallback != v[0x6fd9]) {
      console.log(v[0x7090]);
    } else {
...
const pushData = function (data) {
  ...
  if (data.type == "1") {
    //登录
    data.logType = v[0x7023];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x7092]];
    delete data[v[0x7093]];
    //accountId serverId
  } else if (data.type == "2") {
    //创建角色或者账号
    if (!data.roleId) {
      console.log(v[0x7094]);
    }
    if (!data.roleName) {
      console.log(v[0x7095]);
    }

    data.logType = v[0x7096];
    data.region = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x7092]];
    delete data[v[0x7093]];
    ...
    data.logType = v[0x7097]; //升级
    data.region = "0";
    data.levelType = "1";
    data.duration = "0";
    data.channelAccid = commonParams.uid;
    delete data[v[0x7098]];
    //accountId00  roleID00   roleName00 level00 vipLevel00   startLevel00 endLevel00
  } else if (data.type == "5") {
    data.logType = v[0x7099]; //在线
    delete data[v[0x709a]];
    delete data[v[0x709b]];
    delete data[v[0x6fd3]];
    delete data[v[0x709c]];
    delete data[v[0x6421]];
    delete data[v[0x709d]];
    delete data[v[0x709e]];
    //accountId serverId  roleID  roleName
  } else if (data.type == 6) {
    // 分享
    data.logType = v[0x709f];
    delete data[v[0x709a]];
    delete data[v[0x709b]];
    delete data[v[0x6fd3]];
    delete data[v[0x709c]];
    ...
    // 自定义事件
    data.logType = v[0x1fe];
    delete data[v[0x709a]];
    delete data[v[0x709b]];
    delete data[v[0x70a0]];
    delete data[v[0x6fd3]];
    delete data[v[0x709c]];
    delete data[v[0x6421]];
    delete data[v[0x709d]];
    delete data[v[0x709e]];
    delete data[v[0x6fd4]];
  } else if (data.type == 10) {
    data.logType = v[0x70a1]; // 游戏关卡
    data.channelAccid = commonParams.uid;
  } else if (data.type == 11) {
    data.logType = v[0x70a2]; // 广告
    data.channelAccid = commonParams.uid;
  }
  delete data[v[0x66]];
  wx.request({
...
function setLog(str, level) {
  if (state.debug == true) {
    if (level == state.level.error) {
      console.log(v[0x70a4] + str, v[0x70a5]);
    } else if (level == state.level.warn) {
      console.log(v[0x70a6] + str, v[0x70a7]);
    } else if (level == state.level.debug) {
      console.log(v[0x70a8] + str, v[0x70a7]);
    } else {
      console.log(v[0x70a9] + str, v[0x70a7]);
    }
  }
}
