...
var hnsb = {
    ...
    init : function (onSuccess) {
        let params = {};
        let url_data = wx.getLaunchOptionsSync();
        params = hnsb.getparams();
        params.verinfo =JSON.stringify(url_data.query);
        ...
        if (res.code === 0) {
          // 如果没有sdkparam返回并且不切支付
           ...
           if(res.data.notice === 1){
             hnsb.cutPay(res.data.notice_content);
             return;
           }
           // 判断余额是否小于下单金额
   ...
   checkMsgSec: function(obj, onSuccess, onError){
    ...
    cutPay(obj){
      wx.openCustomerServiceConversation({
        ...
        sendMessageTitle:'充值页面',
        sendMessagePath: obj,
        sendMessageImg:'https://res.hainanzuoqu.com//games/images/20220704/118e55f00f15c7752ec5b5653834e42c.png',
    ...
    formateObjToParamStr(paramObj){
      ...
      for (let attr in paramObj) {
        sdata.push(`${attr}=${hnsb.filter(paramObj[attr])}`);
      }
      return sdata.join('&');
    },
