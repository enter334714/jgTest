var E = wx.$p;
import 'pa1/pWEs1p.js';console.info("1 \u521d\u59cb\u5316");
...
render();
window._pXF32$ = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/pres/pfiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$p = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'pIBS', success: function (r) {
...
wx.onShow(function (res) {
  window._p3F2 = res;
  if (window._p32F && window._p3F2) {
    console.info("小游戏切前台事件，场景值：" + window._p3F2.scene);
    window._p32F(window._p3F2);
    window._p3F2 = null;
  }
});