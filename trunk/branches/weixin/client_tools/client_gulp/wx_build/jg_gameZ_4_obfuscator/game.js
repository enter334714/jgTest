var B = wx.$B;
import 'xv2/T5T5E4T5T5.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "T5T5lgrT5T5/T5T522b.png";
...
render();
window.B12_NLS = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/BCes/BCiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
      ...
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            ...
            name: 'T52IT52', success: function (r) {
...
wx.onShow(function (res) {
  window.B1S_ = res;
  if (window.B1L_S && window.B1S_) {
    console.info("小游戏切前台事件，场景值：" + window.B1S_.scene);
    window.B1L_S(window.B1S_);
    window.B1S_ = null;
  }
});