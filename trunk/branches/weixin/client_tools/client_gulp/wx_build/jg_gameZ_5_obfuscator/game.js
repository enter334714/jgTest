var F = wx.$D;
import 'xv2/d22d22E4d22d22.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "d22d22lgrd22d22/d22d2222b.png";
...
render();
window.D1WI24X = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/SDFDCes/SAFACiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$D = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'd222Id222', success: function (r) {
...
wx.onShow(function (res) {
  window.D1XI = res;
  if (window.D14IX && window.D1XI) {
    console.info("小游戏切前台事件，场景值：" + window.D1XI.scene);
    window.D14IX(window.D1XI);
    window.D1XI = null;
  }
});