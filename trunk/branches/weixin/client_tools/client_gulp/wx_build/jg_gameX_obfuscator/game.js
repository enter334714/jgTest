var u = wx.$x;
import 'xv2/xxE4xx.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "wxlogin_atlas/x22b.png";
...
render();
window.x1T$69J = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/XXes/XXiles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
...
wx.onShow(function (res) {
  window.x1J$ = res;
  if (window.x19$J && window.x1J$) {
    console.info("小游戏切前台事件，场景值：" + window.x1J$.scene);
    window.x19$J(window.x1J$);
    window.x1J$ = null;
  }
});