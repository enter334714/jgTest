var u = wx.$x;
import 'xv2/z888z888E4z888z888.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "z888z888lgrz888z888/z888z88822b.png";
...
render();
window.x1GA086 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Z19es/Z19iles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$x = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'z8882Iz8882', success: function (r) {
...
wx.onShow(function (res) {
  window.x16A = res;
  if (window.x18A6 && window.x16A) {
    console.info("小游戏切前台事件，场景值：" + window.x16A.scene);
    window.x18A6(window.x16A);
    window.x16A = null;
  }
});