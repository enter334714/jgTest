var G = wx.$E;
import 'xv2/DD88DD88E4DD88DD88.js';console.info("1 \u521d\u59cb\u5316");
...
var image = new Image();
image.src = "DD88DD88lgrDD88DD88/DD88DD8822b.png";
...
render();
window.E1CDR_6 = setInterval(function () {
  render();
}, 16);
wx.showLoading({ title: "\u6b63\u5728\u52a0\u8f7d" });
wx.y$ = [];var fs = wx.getFileSystemManager();wx.getFileSystemManager().unzip({
  zipFilePath: "/Z60es/Z60iles.zip", targetPath: wx.env.USER_DATA_PATH + "/f/", success: r => {
    "unzip:ok" == r.errMsg ? fs.readFile({
      filePath: wx.env.USER_DATA_PATH + "/f/files", encoding: "utf8", success: function (r) {
        if ("readFile:ok" == r.errMsg && r.data) {
          try {
            wx.$E = JSON.parse(r.data);
          } catch (r) {
            console.error(r);
          }r = wx.loadSubpackage({
            name: 'DD882IDD882', success: function (r) {
...
wx.onShow(function (res) {
  window.E16D = res;
  if (window.E1_D6 && window.E16D) {
    console.info("小游戏切前台事件，场景值：" + window.E16D.scene);
    window.E1_D6(window.E16D);
    window.E16D = null;
  }
});