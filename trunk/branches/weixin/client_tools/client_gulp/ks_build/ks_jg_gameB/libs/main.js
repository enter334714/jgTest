...
console.log("快手基础库版本：" + window.SDKVersion);
...
window.loadProbuf = function () {
    ...
    var loadProbufTask = ks.loadSubpackage({
        name: 'probuf',
        success: function (res) {
...
window.loadMain = function () {
    console.info("subPackage 分包加载");
    var loadMainTask = ks.loadSubpackage({
        name: 'subPackage',
        success: function (res) {
            console.info("subPackage 分包加载成功");
            console.log(res);
            //   if (res && res.errMsg == "loadSubpackage: ok") {
            window.loadMainPkg = true;
            window.initMain();
            window.enterToGame("loadMain");
            //   } else {
            // setTimeout(function() {
            //   window.loadMain();
            // }, 500);
        ...
        fail: function (res) {
            if (!window.loadMainPkg) {
                console.info("subPackage 分包加载失败:", res);
                console.log(res);
                setTimeout(function () {
                    window.loadMain();
                }, 500);
