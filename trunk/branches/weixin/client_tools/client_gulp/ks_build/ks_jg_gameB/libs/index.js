...
window.sdkOnInited = function (res) {
    ...
    sdkInitRes = res;
    console.log("sdkInitRes:", sdkInitRes)
    // res.game_ver = "1.0.86";
...
window.onUserLoginDefaultServers = function (response) {
    ...
    this.console.log("onUserLoginDefaultServers>>>>>")
}
...
window.onUserLoginCheckServers = function (response) {
    ...
    this.updCurServer(response);
    console.log("window.ServerLoading》》》》：", window.ServerLoading)
    console.log("window.ServerLoading.instance.openJumpTipsBtn》》》》:", window.ServerLoading.instance.openJumpTipsBtn)
    if (window.ServerLoading && window.ServerLoading.instance.openJumpTipsBtn) {
        window.ServerLoading.instance.openJumpTipsBtn(sdkInitRes.isShowSdkAge, sdkInitRes.sdk_age_adaptation_icon, sdkInitRes.sdk_age_adaptation_content, sdkInitRes.coordinate_x, sdkInitRes.coordinate_y)
    }
}
...
window.toPay = function (roleId, roleName, roleLevel, roleCareer, productId, price, productName, productDesc, callback, appleprd_id) {
    ...
    console.log("[游戏]Order.order开始下单 请求时间:", Date())
...
window.toPayCallBack = function (data) {
    console.log("[游戏]Order.order 下单返回时间:", Date())
    if (data && (data.errCode === 200 || data.state == 'success')) {
        ...
        console.log("[游戏]调起SDK支付接口 时间:", Date())
        AKSDK.pay({
            ...
            console.log("[游戏]调起SDK支付--返回 时间:", Date())
...
window.send = function (url, data, callBack, retryAmount, errorCB, checkSuccess, reqType, contentType) {
    ...
    if (url.indexOf("Order.order") != -1) {
        console.log("[游戏]retryAmount  :", retryAmount)
        console.log("[游戏]Order.order  window.send开始下单 请求时间:", Date())
        window.reqRecordInfo("Order.order:", JSON.stringify({ date: Date(), url: url }));
        timeout = 3000;
    }
    ks.request({
        url: url,
        ...
        success: function (res) {
            if (url.indexOf("Order.order") != -1) {
                console.log("[游戏]send.success:", JSON.stringify(res));
            }
        ...
        fail: function (res) {
            // if (url.indexOf("Order.order") != -1) {
                window.reqRecordInfo("Order.order:", JSON.stringify({ date: Date(), error: res ,url:url}));
                console.log("[游戏]send.fail:", res);
            // }
            window.sendFail(url, data, callBack, retryAmount, errorCB, checkSuccess, res);
        },
        complete: function (res) {
            if (url.indexOf("Order.order") != -1) {
                console.log("[游戏]send.complete:", res);
            }
